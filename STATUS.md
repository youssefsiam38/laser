...
**Last updated:** Typed creation/Markdown milestone merged · code checkpoint: de49fab3
**Current focus:** Build index receiver crash fixed on main (`c085f82a`), independently reviewed; design123, test types, typecheck, identity and UI build pass. Person retry remains pending. M21-T6 creation/Markdown milestone merged at `de49fab3` with all review corrections; parent391 focused tests, test types/typecheck/identity/UI build pass. Initial opened detail implementation `79a93c48` is correcting parent-found draft/revision/pending-save lifecycle gaps before independent review. Opened detail candidate `535a532b` includes exact-owner save settlement; parent415 focused tests, test types/typecheck/identity/UI build pass. M2 review `2003f074` requests Design shared-draft preservation and Research framing validation; parent also requires field-specific editor errors. One final correction owner is addressing the batch. This second milestone is not yet integrated or accepted. M20-T3 revised spend-restart plan `9e4034e8` is approved and implementing: durable request-scoped child totals, restart/live-update coverage and visible partial subtotals. A separate owner keeps it independent of entity UI. The fix remains in next-release scope. Viewing will render Markdown by default; Preview never saves. Publication is explicitly authorized once the work is finished; review, validation and person-owned acceptance results remain pending. Goal mode is inactive.
