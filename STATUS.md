...
**Last updated:** 2026-09-05 · claude-2026-09-05-d · commit: `f8d58cf` (working tree)
...
recorded exception (D-34). Next: the protocol gaps three finished components
wait on, and a real look at the app in a browser with a live provider.
...
1. **See it.** Nothing in wave 3 was looked at in a browser: the sandbox has no provider credentials, so no transcript, tool row, island or dock was exercised live. Every visual claim rests on the built CSS and on build/typecheck/test.
...
4. A real phone and a real desktop session: push delivery, dictation end to end, and every screen at 390px.
...
- **M11-T4 Settings → Appearance** — presets, hues, fonts, text size, density, corners, contrast, motion and a token editor with a live contrast readout per row.
