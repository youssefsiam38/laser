...
confidence. A visible origin legend counts UTF-16 characters and percentages.
...
Every new source carries `origin`, optional `detail`, and `inline: true` when no
file is behind it. The type and schema allow absent origins solely for retained
