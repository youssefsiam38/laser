...
in the login profile or source `.bashrc` there. Other shells follow their own
interactive login startup rules. Windows keeps its inherited environment.

This imports exports, not aliases, functions, job control or a terminal. Startup
files execute as they do at login, including their side effects. Individual
commands remain non-interactive and do not rerun your startup files.
...
phones and browser renderers cannot update the environment.
...
wins for each unprotected key. Removing an export from a profile does not erase
