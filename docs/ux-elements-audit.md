...
| `components/fleet/FleetPanel.tsx` `TaskOutputBody` | A background command's output | **The one duplicate** (Terminal block row, task T-A) |
...
- **T-B (housekeeping, one line) · Retire the "run island" sentence in
  ...
  mount has been there since D-147. No behaviour change.
