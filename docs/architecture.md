...
| `background-work` | Commands — long commands a conversation left running (`ux-fleet.md`; the wire keeps the word `task`): `bash` with an explicit background flag, promotion to a background task after the foreground timeout, `task_output`/`task_stop` (no waiting tool: every exit wakes the model, D-162; no list: the harness's `inspect_fleet` shows every command in the tree, and `task_output` reads a child's through the worker, D-163), `lasercode/task/update` for the fleet and `lasercode/task-event` | the worker passed `BackgroundWorkOptions` |
