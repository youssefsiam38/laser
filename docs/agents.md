...
| Instructions | `instructions`, `engineInstructions`, `excludeCoreInstructions` | The Markdown body, byte-preserved and interpreted as a Handlebars template, answers "how should this agent work?" The shipped `default` agent starts with `engineInstructions: true`; `excludeCoreInstructions` opts this custom agent out of the otherwise prepended product core prompt |
