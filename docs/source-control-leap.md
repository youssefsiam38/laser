...
- **Changes outside the project's workspace.** A session can edit a file in
  another repository, in `$HOME`, or in `/tmp`; those changes are not tracked
  and not shown. Tracking them honestly needs a second mechanism for paths git
  cannot reach (a snapshot store, a deny list for credentials, consent for
  writing refs into somebody else's repository), and that is a leap of its own.
  The overlay states its boundary in one sentence — these are the changes
  inside this workspace — rather than implying it saw everything.
- Filesystem watchers over the machine.
