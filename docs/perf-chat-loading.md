...
- `pi/session/entries` accepts `window: {tail:40}`, `{before,limit}`, `{beforeEntry,limit}`, `{from:entryId}`, or `{all:true}`; omitting it retains the legacy full route. `{beforeEntry,limit}` is a bounded producer page ending before a named entry on the active ancestry and returns the normal opaque `before` cursor. Cursors bind to session identity and branch lineage: they survive append-only growth and a worker restart, but not a branch change.
