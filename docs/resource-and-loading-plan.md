...
Status: implemented — M18 (RP-1–RP-13, RP-2 repeat baseline; RP-14 dropped by D-268) and M19 (recovery and update activation) are done and audited (`/tmp/m18-final-audit.md`, `/tmp/m19-final-audit.md`); the tree carries the final packaged and quick-fixture gates at the release tip.

Architecture task: MX-T9. Implementation tasks: M18-T1..T19 and M19-T1..T3. Decisions: D-255 through D-270.
...
| RP-2 repeat | M18-T15 | done — two clean unchanged full runs at `21a15a2a` (off-main executable tree tagged `resource-baseline-m18-t15`, main minus the M19-T1 merge) pass every ceiling, coverage, survivor, ranking and slope gate (D-267/D-269); `docs/resource-soak-baseline.md`; integrated `7bfce5e8` | RP-4..RP-8, RP-5b |
...
| RP-8 | M18-T8 | done — calibration D-261, worker controller C `a4a364c2`, companion retention D `87e65720`, host evidence/pass/admission E `d8ec6cae`/`55350823`/`49cc12d9`, renderer pressure F `1b5fbfa7`, heap ceilings and worker-loss recovery G `4f15add0`/`ce455b99`; packaged clean-machine gate, Linux containment probe and exact-SHA quick fixture pass | RP-4, RP-5 |
...
| Recovery/update activation | M19-T1..T3 | done — T1 `23787c27` launch identity/structured failure/bounded repair/safe mode; T2 `d59f74e8` verified-inventory generations, park gate, correlated update transaction (D-266/D-270); T3 `2b9962db` integrity-checked snapshot/restore boundary with durable interrupted-restore marker | M18 |
...
Per-body, image-format, decoding and transport ceilings remain hard. The ordinary 1.5 MiB per-view and aggregate transcript targets are cache targets: inactive/reconstructible views obey them, but current, main-destination, scoped-visible and user-owned draft/unsent/action views may exceed them. Their bytes and protected overflow are reported honestly; maintenance reclaims inactive caches first and never deletes logical rows from the conversation a person is using merely to report compliance. Background-running alone is not visibility. Oversized live and settled bodies still retain bounded excerpts plus canonical source identity, exact omitted-byte evidence and a revision fence. Additional body content is read in bounded ranges from the live owning worker or worker-free host projection without restoring the whole body to renderer state.

A legacy view whose older rows were already released keeps the same **Load earlier messages** path. When its producer-owned cursor is gone — or a cursor is stale — that gesture asks for one bounded producer page ending before the retained anchor (`{beforeEntry,limit}`) and prepends it. It never requests the indivisible anchor-to-live suffix and never replaces the retained window with a recent tail. Revision, generation, hydration, trim-anchor and epoch fences discard stale replies. Reaching the root stops further upward requests without claiming a disjoint retained set is the whole transcript.

Done when active logical history remains reachable and inactive caches still converge to their targets, with protected overflow visible, no silent omission and no second transcript authority.
