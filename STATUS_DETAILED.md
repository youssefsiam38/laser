...
| M14-T5 | Packaged build carries the adapter | in-progress | worker mcp-packaging (01a09264-09f9-73a4-a484-eb29d0307676, `agents/mcp-packaging-766cca38`) | — | ready at `86a8e48`: nothing manifest-invisible (keyring 1.3.0 + its Linux x64 binding ship nested under the adapter), the adapter's `skills/**/*.md` restored by a positive files rule, jiti `fsCache: false`, the clean-machine gate opens a real session with a stdio fixture from the bundled Node on an empty PATH and passes with read-only `node_modules`; `pnpm verify` green; under review |
