...
- 2026-09-15 parent pre-review correction run `run_ecc1a20c` started: add the required opaque `sessionId` tail identity, schedule on content/pin/external-draft transitions, refuse dormant retention of unknown decoded-image ownership, preserve per-view release reasons and guarantee bounded post-paint tail delivery.
- Ownership ledger: renderer session lifetime · `01a0a214-a8d0-76c8-926f-52b824a37031` run `run_ecc1a20c` · M18-T5 parent correction · same T5 UI/tests/browser/docs paths; T3 pixels, T10 persistence, client.ts, T4/T7/planning excluded · base `c0e87c4` · next: inspect corrected diff, independent review, integrate.
