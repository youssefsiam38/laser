...
| M10-T7 | Release pipeline, versions pinned end to end | in-progress | codex-2026-09-06-sandbox | prior 0.1.0 evidence retained; 0.1.1 retry pending | The real-release installer gate had hard-coded 0.1.0 expectations and rejected the valid 0.1.1 artifact. It now derives the version from the workspace manifest; synthetic upgrade fixtures remain intentionally fixed. |
...
| M10-T9 | OS-native update channel | in-progress | codex-2026-09-06-sandbox | public feeds and signatures are valid; corrected native-package registration pending 0.1.2 | 0.1.0/0.1.1 registered the case-sensitive GitHub Pages repository as `/Laser` because electron-builder expanded `${sanitizedProductName}` from the display name. The package hook now takes the exact lowercase repository slug from `product.json`. |
