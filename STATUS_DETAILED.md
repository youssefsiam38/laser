...
| M16-T30 | The hidden window costs nothing | in-progress | perf-desktop (`agents/perf-desktop-7aeed9a9`) | — | see notes |
| M16-T31 | The first conversation loads only what it needs | in-progress | perf-bundle (`agents/perf-bundle-a94c069a`) + orchestrator (host cache header) | — | see notes |
| M16-T32 | Streaming touches only the streaming row | in-progress | perf-streaming (`agents/perf-streaming-8b9c30fe`) | — | see notes |
| M16-T33 | The package carries only what runs | in-progress | perf-packaging (`agents/perf-packaging-325edf4e`) | — | see notes |
