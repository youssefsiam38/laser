...
| M14-T7 | Release M14 as 0.4.0 | in-progress | orchestrator | — | orchestrator started from source `43332b7` with `RELEASE_NOTES.md`; candidate `75fbe3a` on `origin/main`, CI 34657053247; then tag, release workflow, digest/provenance/notes verification (D-223) |
...
| ID | Task | State | Owner | Evidence | Notes |
| --- | --- | --- | --- | --- | --- |
| M15-T1 | Native file opener: file card and format-aware viewer modal | in-progress | worker file-viewer (01a09268-985b-73a4-a484-eb2fd91f4208, `agents/file-viewer-75f2f80e`) | — | implemented (validated tree `a325125`, UI 1204, worker files/server 35, `pnpm verify` green, 20 screenshots); the worker found that a cwd under a child's worktree started a second worker for cwd-routed methods — fixed on `main` by the orchestrator (`3ed6453`, invariant 5, host 228); committed `8f85557`+`6f86506`; review (`/tmp/review-file-viewer.md`) APPROVE WITH FIXES — extensionless text files were shown as binary, the attachment/file source union duplicated the body, nothing bounded a 2 MiB payload at display time (Shiki 1.5 s + 7.5 MiB markup), raw entry casts in `messages.tsx`; fixed at `edf24a5` (extensionless text sniffed for NUL, binaries not shipped, one `ProjectFileContent`-shaped source, display bounded through `preview/display.ts`, entries helpers; UI 1212, worker files 12 — re-run by the orchestrator); parked for the M14 tag |
| M15-T2 | Dictation is always transcribed as English | in-progress | worker dictation-english (01a091e2-ac35-73a4-a484-ea5595d1b475, `agents/dictation-english-7a68babc`) | — | reviewed (APPROVE WITH FIXES) and fixed at `3f8e608` (27 transcribe tests, orchestrator re-ran); waits on its branch for the M14 tag (D-222) |
| M15-T3 | Model fallback chains: settings, runtime, persistence, verification | in-progress | worker fallback-chains (01a091e3-8b36-73a4-a484-ea6b0227d7dd, `agents/fallback-chains-e0b3ad20`) | — | phase 2 complete at `0d88553`: controller on the live engine `a4b719f` (14 real-engine tests; worker 585), transcript record + status line `0d88553` (UI 1211); whole workspace 2465 tests green in the worktree; review (`/tmp/review-fallback-chains.md`) REQUEST CHANGES — settle/release only on prompt/goal paths so an extension-woken turn (child ending, background exit) never settles nor falls back; a refused `setModel` loops forever; text classification too broad (ordinary 400s switched and benched the model); stale failure signal, `position` not written on every switch, missing manual-selection race test, raw ids in replayed records. Fix batch landed at `6d24e62` (settle/release on the one `runInvocation` boundary via `setAfterNativeTurn`, refused candidates fail non-transiently, status-anchored classification with every other 4xx `unknown`, `abortTurn()` on manual selection with the race tests, `engine-port.ts` extracted); worker 593, protocol 101, UI 1212; orchestrator re-ran fallback/seam/agent/admission suites 61/61. Waits on its branch for the M14 tag; browser pass at integration |
| M15-T4 | Sidebar activity indicators: one indicator, owned by the session itself | in-progress | worker sidebar-and-slash-fixes (01a091e1-c6e5-73a4-a484-ea4576dad59a, `agents/sidebar-and-slash-fixes-84cd3548`) | — | reviewed and fixed at `5eb2008`; the person's follow-up rule landed at `8559c96`: a child's failure never marks its parent, the failed child wears a quiet neutral ring ("Finished with an error"), the finished fold shows no failed count (UI 1128, identity exit 0, screenshots inspected); waits on its branch for the M14 tag |
| M15-T5 | Slash completion completes, never sends | in-progress | worker sidebar-and-slash-fixes (same session as T4) | — | reviewed and fixed at `b5dd471` (chorded Tab never selects, real caret restored incl. the suffix case, details-pane Tab test); waits on its branch for the M14 tag |
