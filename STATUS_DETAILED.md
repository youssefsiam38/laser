...
| M16-T23 | One browser-check harness every agent uses | in-progress | worker browser-harness | — | every worker rebuilds its own; recurring flakiness (theme not applied, pointer timeouts, `spawn sh ENOENT`, artifacts landing in the repo) |
| M16-T23 | One browser-check harness every agent uses | in-progress | worker browser-harness | — | every worker rebuilds its own; recurring theme/pointer/PATH flakiness and artifacts landing in the repo |
