...
| ID | Task | State | Owner | Evidence | Notes |
| --- | --- | --- | --- | --- | --- |
| M15-T1 | Native file opener: file card and format-aware viewer modal | in-progress | worker file-viewer (01a09268-985b-73a4-a484-eb2fd91f4208, `agents/file-viewer-75f2f80e`) | — | implemented (validated tree `a325125`, UI 1204, worker files/server 35, `pnpm verify` green, 20 screenshots); the worker found that a cwd under a child's worktree started a second worker for cwd-routed methods — fixed on `main` by the orchestrator (`3ed6453`, invariant 5, host 228); committing on the branch; merges after the M14 tag |
