...
| M13-T89 | Make pre-turn agent choice tentative until first send | in-progress | first-prompt-binding | — | D-188; see notes |
| M13-T90 | Align composer controls and recording affordance | in-progress | composer-layout | — | D-188; see notes |
| M13-T91 | Show the session’s persisted agent in the header after start | in-progress | header-agent | — | D-188; see notes |
| M13-T92 | Replace stale Sending state with the queue state | in-progress | first-prompt-binding | — | backend `67bf567` integrated; UI hydration awaiting review; D-188; see notes |
| M13-T93 | Serialize child completion, engine settle and resume | in-progress | harness-lifecycle | — | implementation `7d99f554`; re-review active; caller adoption outstanding; D-188; see notes |
| M13-T94 | Release the next stable patch | todo | — | — | depends on M13-T89–T93, M13-T95, M13-T96 and explicit permission; D-188, D-189, D-190; see notes |
| M13-T95 | Manual aggregate activity open/close persists | in-progress | activity-disclosure | — | D-189; see notes |
| M13-T96 | Blocked terminal runs are neutral finished work | in-progress | neutral-finished-runs | — | D-189; see notes |
...
- 2026-09-10 checkpoint: protocol/host/UI first-turn contract is implemented and focused tests pass. Agent/thinking choices stay in composer scope, preserve drafts/attachments and create no session; every assistant-ui send path captures `{ agentName, thinkingLevel? }` before landing initialization and sends it on `session/prompt` for the same path. Pristine-admission/serialization helpers and saved-empty record recovery tests exist; shared server/driver integration still awaits the reviewed M13-T92 and harness `promptUser` commits.
- 2026-09-10 takeover by first-prompt-binding: implement the typed `session/prompt` first-turn contract through protocol, host and UI; isolate worker rebind work until reserved files are released. Preserve the existing path/session manager, reject any non-pristine root custom session, and roll back failed runtime preparation without a second writer.
...
- 2026-09-10 dependency update: M13-T95 and M13-T96 join M13-T89 through M13-T93 as release correctness dependencies.
- 2026-09-10 added as the next patch release after dependencies are done and independently reviewed.

#### M13-T95 notes
- 2026-09-10 claimed by activity-disclosure: manual aggregate open/close remains authoritative, and Answers-only never auto-opens errors or streaming activity. Eight files are currently uncommitted, including `ToolRow` and `sessionPreferences`; real-browser review is running. Ownership remains bounded from first-binding, composer and header work.

#### M13-T96 notes
- 2026-09-10 claimed by neutral-finished-runs: render terminal `blocked` as neutral finished work, include it in finished folds, and make newer active work supersede historic failures across fleet, sidebar and inspect words. Seventeen files are currently uncommitted, including worker fleet changes; browser review is running. Preserve live warm `needs_input` as Asking and live descendants.
...
**Consequences.** Disclosure state must survive rerenders and mode changes without error/stream exceptions. Terminal blocked rows lose warm attention and join other ended history, but no live question or live descendant is folded away. Optional Markdown-variable and image-preview enhancements are outside this patch.
...
**Decision.** Pending-delivery acceptance, harness settlement, first-prompt binding and caller integration form one coupled session-lifecycle candidate. Existing non-overlapping owners finish frozen checkpoints. After first-prompt-binding relinquishes source ownership, harness-lifecycle is the proposed continuing integration owner; ownership has not transferred yet. Composer, header, disclosure and neutral-fleet work remain bounded independent lanes.
...
**Consequences.** No optional Markdown-variable or image-preview enhancement enters this patch. No version change, push, tag or publication occurs until the combined source candidate passes its gates and the user gives explicit permission. Frozen owner commits and uncommitted review candidates remain preserved; no task is marked done from partial or isolated evidence.
