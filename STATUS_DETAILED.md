...
| M16-T4 | Release 0.5.1 | in-progress | orchestrator | — | integrated live check `/tmp/m16-check/REPORT.md`: 2 defects (welcome frame when a loaded session is presented — the runtime's empty message list for one frame; single-image bubble wider than a narrow column) fixed by the orchestrator at `4460dd5`, re-proven in the real host at 1360/390 both themes; release running |
