...
| M13-T89 | Make pre-turn agent choice tentative until first send | in-progress | hlc010-admission-owner | — | coupled batch frozen `aba12f7b`, unmerged and approval withheld; D-191; see notes |
| M13-T90 | Align composer controls and recording affordance | in-progress | stabilization-ui-acceptance (validation) | — | frozen `e7a5b89`; accepted staging only; D-188; see notes |
| M13-T91 | Show the session’s persisted agent in the header after start | in-progress | stabilization-ui-acceptance (validation) | — | code-approved `a8715dd`; final browser gate incomplete; D-188; see notes |
| M13-T92 | Replace stale Sending state with the queue state | in-progress | hlc010-admission-owner | — | backend `67bf567` on main; coupled batch `aba12f7b` unmerged; D-191; see notes |
| M13-T93 | Serialize child completion, engine settle and resume | in-progress | hlc010-admission-owner | — | Q-8 authorized; HLC-010 plan-only investigation active at development `120bf303`; see notes |
| M13-T94 | Release the next stable patch | in-progress | stabilization-ci | — | user authorizes commit/push/next stable publication after all original stabilization gates; D-194; see notes |
| M13-T95 | Manual aggregate activity open/close persists | in-progress | stabilization-ui-acceptance (validation) | — | approved `131332b` in accepted staging; full current gates incomplete; D-189; see notes |
| M13-T96 | Blocked terminal runs are neutral finished work | in-progress | stabilization-ui-acceptance (validation) | — | approved `d694b07` in accepted staging; full current gates incomplete; D-189; see notes |
...
| Coupled lifecycle | hlc010-admission-owner / `01a08d0a-9a10-70c8-b0d6-ec794e6de23d` | HLC-010 seam; worker lifecycle/driver/server/Chord and related tests only; planning excluded | `120bf303` | `e1ddc2d` handed off; orchestrator identified pre-review gaps; same owner corrects before independent review |
| Independent frontend acceptance | stabilization-ui-acceptance / `01a08d2f-269a-70c8-b0d6-ec85497c2d18` | read-only source at isolated `ui-acceptance`; own installs/builds/temp browser scripts/artifacts | fixed `91a7759` frontend | active browser/theme/pointer matrix and focused UI tests; no code writes; final HLC-dependent scenarios rerun after candidate |
...
| Current planning ledger | stabilization-ci (orchestrator) | PLAN/STATUS/STATUS_DETAILED in `hlc010-development` only | development merge | central current ledger; worker excludes planning files; original release/development checkouts frozen |
