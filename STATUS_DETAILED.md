...
| M10-T4 | Every Linux package format | in-progress | codex-2026-09-06-sandbox | prior evidence retained; Ubuntu native-package sandbox regression under repair | The installed 0.1.0 `.deb` loads its AppArmor profile but leaves `chrome-sandbox` at 0755. The launcher's external `unshare` probe is separately confined and fails, so it incorrectly reports that neither sandbox path exists. See M10-T4 notes. |
...
| M10-T7 | Release pipeline, versions pinned end to end | done | lane-I, integrator, codex-2026-09-06-release | [release run 34005922135](https://github.com/youssefsiam38/laser/actions/runs/34005922135) succeeded for x64, ARM64, publication, provenance and Pages; [v0.1.0](https://github.com/youssefsiam38/laser/releases/tag/v0.1.0) is stable and Latest | Plain SemVer releases are stable/Latest; SemVer suffixes are prereleases and cannot become Latest. The whole workspace is pinned to 0.1.0. The separate in-app updater remains M10-T10. |
...
| M10-T9 | OS-native update channel | done | codex-2026-09-06-release | public APT `InRelease` and RPM `repomd.xml` signatures verified with key `6E593349C0A1BE57F024CF97AFC91CDE867E302B`; APT advertises Laser 0.1.0 | The `.deb` and `.rpm` register the signed GitHub Pages feeds, so the operating system's normal updater owns discovery, notifications and upgrades. |
