...
| ID | Task | State | Owner | Evidence | Notes |
| --- | --- | --- | --- | --- | --- |
| M15-T1 | Native file opener: file card and format-aware viewer modal | in-progress | worker file-viewer (01a09268-985b-73a4-a484-eb2fd91f4208, `agents/file-viewer-75f2f80e`) | — | protocol `pi/project/read` (`0122859`) by the orchestrator; worker read service (traversal-safe, capped) + file card on write/edit rows + viewer modal (code/Markdown preview-or-source/image/diff/text) + attached-image preview assigned; merges after the M14 tag |
...
| M15-T3 | Model fallback chains: settings, runtime, persistence, verification | in-progress | worker fallback-chains (01a091e3-8b36-73a4-a484-ea6b0227d7dd, `agents/fallback-chains-e0b3ad20`) | — | phase 2 complete at `0d88553`: controller on the live engine `a4b719f` (14 real-engine tests; worker 585), transcript record + status line `0d88553` (UI 1211); whole workspace 2465 tests green in the worktree; design corrected (§2.3: the engine's `after_provider_response` does not fire for a failed request, so classification reads the error text and parses stated reset times); exhaustion is its own durable attention record; under review; browser pass at integration |
