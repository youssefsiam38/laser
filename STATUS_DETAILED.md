...
| M16-T16 | Conversations load like a chat app | in-progress | worker chat-loading (01a0956e-442a-7405-a9c3-60452bc90d24) | A `ae423a5`, B `6cc8572` | review `/
#### M16-T16 notes
...
| M16-T26 | The first click is not a cold start | in-progress | worker session-readiness | — | forensics `docs/session-open-forensics.md`: prestarting only the worker process took a short-session open 744 → 175 ms; engine file open is ~4.5 ms |
| M16-T25 | The best MCP client there is | in-progress | worker mcp-best-practices | — | guide `/tmp/mcp-guide.md` (2026-07-28); progressive discovery default per server, Advanced override; today: fixed 40-tool direct-exposure cap |
| M16-T24 | Click to conversation, accounted for | in-progress | worker session-open-forensics | — | the person: opening a session from local files still takes far too long; find out where the time actually goes |
