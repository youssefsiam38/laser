...
| M13-T102 | Selected agent model follows selection | in-progress | agent-model-selection | — | see notes; D-202 |
...
| M13-T104 | Parent-priority hard interrupt and explicit queueing | blocked | parent-interrupt-control | — | see notes; D-203 |
| M13-T105 | Per-item fleet partition with duplicated ancestor context | in-progress | fleet-section-partition | — | see notes; D-205 |
| M13-T106 | Scrollable built-in model picker lists | in-progress | agent-model-selection | — | see notes; D-206 |
| M13-T107 | Release reviewed subset 0.3.7 | in-progress | orchestrator-release-subset | — | see notes; D-207 |
...
- Ownership ledger: instruction editor + value inspection · `01a08fa5-a95d-7754-a665-f172e3a7d781` (`instruction-highlight`) · investigate then implement one cohesive milestone · UI agent editor/helpers/tests; protocol/worker/host additions only after plan approval · base `728a1f9` · no prerequisite changes · implementing approved UI-only plan · next: ready handoff and review. Planning files belong only to orchestrator.
...
- Ownership ledger: agent/model selection · `01a08fa8-b865-7754-a665-f1785ddcd05e` (`agent-model-selection`) · investigate then fix cohesive selection path · session-preparation, agent/model/reasoning selectors, first-turn helpers and focused tests; worker/protocol changes only after plan approval · base `728a1f9` · independent of agent definition editor and activity timing · implementing approved nullable model intent · next: review then T104 interface handoff.
...
- Ownership ledger: activity elapsed timing · `01a08fa9-0f9f-7754-a665-f17f2ed4342c` (`activity-span-timing`) · investigate then fix aggregate span · tool-group.aui, thread timing/group/message integration, projection timing helpers/tests only as needed · base `728a1f9` · independent of instruction editor and first-turn model path · implementing approved UI-only group clock · next: ready handoff and review.
...
- Ownership ledger: parent interrupt lifecycle · `01a08fb4-f3bf-7754-a665-f194086ffca2` (`parent-interrupt-control`) · investigate/plan only until approved · worker agents harness/lifecycle/tests, companion tool description + protocol agents schema if needed; driver/server writes require handoff from T102 owner first · base `728a1f9` · T102 transaction interface must stabilize before any overlapping implementation · investigation · next: explicit plan and dependency approval.
...
- Ownership ledger: fleet lifecycle projection · `01a08fcb-9d95-7754-a665-f1b16ada6357` (`fleet-section-partition`) · investigate/plan then implement · UI fleet model/panel/subagent-list + fleet tests; docs/ux-fleet rule R2 and inventory subsection only after approved plan · base current `3bdacf5` · independent of instruction templates, first-turn model and interrupt harness · investigation · next: plan approval. Canonical run/task state, map/sidebar and worker inspect_fleet stay unchanged.
...
- Ownership ledger: model-picker modal scrolling · `01a08fa8-b865-7754-a665-f1785ddcd05e` (continuing selector owner) · investigate only while T102 under coordinator review; then one scroll milestone · model-selector/provider picker portal helpers, builtin dialog, focused tests · base `7b5dfe7` ready branch · T102 reviewed/integrated base before source edits · investigation · next: exact cause/plan then release UI-only implementation; no protocol/worker changes.
...
- Ownership ledger: release0.3.7 · orchestrator + prep worker pending · version/isolated gates then orchestrator integration/push/CI/tag/publication verification · version manifests/generated identity metadata and release evidence only · base `4acae2e` plus current release ledger/instruction inventory commit · prerequisites T101/T103 done · claimed · next: isolated versioned candidate. No live host restart or installer action authorized.
