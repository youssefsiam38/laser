...
| M16-T27 | The session the app just created is the session it shows | in-progress | worker catalog-arrival-race | — | `packages/ui/test/runtime/catalog-arrival.test.tsx:203` fails intermittently: prompt routing succeeds but `data-current` stays empty; seen independently by the orchestrator and by the MCP worker on unrelated branches |
...
| M16-T25 | The best MCP client there is | in-progress | worker mcp-best-practices (01a09680-84f2-758e-a103-8e74fa1873f5) | milestones 1–2 merged: `c65d3b1` (`docs/mcp-client.md`), `7ea56ee` (progressive default, 2% budget, frozen tools array, Advanced override) | milestone 2 (progressive default, 2% budget, frozen tools array, Advanced override) on `agents/mcp-best-practices-acdf7608`: two review rounds — packaged-gate breakage and budget starvation fixed; a third round runs on guidance payload displacing search results. Milestones 3–5 (cache correctness, dynamic servers, script isolation) not started |
