...
| M13-T60 | The fleet's command output body is `terminal-block` by hand | todo | — | — | found by M13-T37: add `follow` and `truncatedHead` to `terminal-block.tsx`, mount it in the fleet's task detail, drop the hand-drawn ground and the Command/Exit-code fields |
| M13-T61 | Delete the element files nothing mounts | todo | — | — | found by M13-T37: `agent-status`, `todo-list`, `reasoning.aui`, `reasoning-panel`, `onboarding`, `DiffPreview`, the `AgentHandoff` strip export; each row in `docs/ux-elements.md` then says "not mounted, file removed" |
