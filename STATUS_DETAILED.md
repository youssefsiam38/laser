...
| M5-T6 | Preserve Linux command privileges across update restart | done | linux-relaunch | `PATH=/home/youssef/.nvm/versions/node/v24.11.1/bin:$PATH pnpm verify`; desktop 135 tests; opt-in `test/linux-relaunch-smoke.test.ts` passes real Electron 0→0 vs old 0→1 | see notes; D-228 |

#### M5-T6 notes
- 2026-09-12 claimed: trace the native update prompt through `quit({ relaunch: true })`, reproduce Electron relaunch privilege inheritance in isolated processes, then replace the Linux relaunch path without disabling the renderer sandbox or touching the running app.
- 2026-09-12 discovery: this installed Electron, host, worker and command shell all report `NoNewPrivs=1`, while the user systemd ancestor reports 0. `main.ts` calls `app.relaunch()` after stopping the host. No Laser command approval setting caused the sudo refusal. Existing unrelated deleted research files and untracked scratch/config files are left alone.
- 2026-09-12 checkpoint: isolated Electron 44.2.0 probe under the user service manager confirms `app.relaunch()` changes `NoNewPrivs` from 0 to 1 (`/tmp/relaunch-baseline-result.jsonl`); no real host/session was opened. Implement a stock-Node helper with prepare/commit/cancel handshake, old-process-exit wait and unchanged argv/cwd/environment. Refuse automatic restart before teardown when already restricted, with a full quit/reopen instruction; never pretend this kernel bit can be cleared.
- 2026-09-12 checkpoint: desktop build and 6 relaunch tests pass. Opt-in real-Electron smoke passes both old 0→1 and replacement 0→0 into shell children, same argv/cwd/environment and single-instance lock reacquisition. The outer transient systemd service reported cleanup timeout after Vitest passed; journal reports exhausted inotify watches, no probe processes remain. Full desktop test run exposed unrelated stale compiled-version failure in host-environment; rebuilding the workspace before the final gate. Production renderer/launcher sandbox untouched; only isolated downloaded Electron uses the existing smoke-test no-sandbox convention.
- 2026-09-12 done: stock-Node Linux handoff prepares before host shutdown, commits only after stop, waits for original process identity to exit and preserves argv/cwd/environment; AppImage uses its durable outer path. Already-restricted instances get a person-facing full quit/reopen instruction without stopping work. Desktop build, focused 10 tests, all 135 desktop tests, staged identity and full `pnpm verify` pass (`/tmp/linux-relaunch-verify-node24.log`). First full gate's unrelated runtime-env fixture assumed a conventional npm symlink and failed under the installed bundled Node layout; rerunning with the normal development Node 24 passes. Native smoke Vitest passed independently (outer service cleanup caveat above). No renderer component changed, no production app/host restart or installation, no sudoers/privilege policy change, no push/release; unrelated working-tree deletions and scratch files preserved.
...
| M13-T116 | Release the startup trust fix as 0.3.9 | done | claude-2026-09-11-trust | v0.3.9 public, 12 assets; candidate cbab7f7; CI 34626252419; release 34626602601; checkpoint verified 2026-09-11T17:37Z | see notes |
| M13-T117 | Open a live Chat when the remembered one is gone | todo | — | branch `chat-memory-fallback` (7d121ef) | see notes; D-219 |
| M13-T118 | Release notes required; API-only publication check | done | claude-2026-09-11-trust | 13c066d; `node --test scripts/release/test/*.test.mjs` 39/39 | see notes; orchestrator change awaits the person's review before its first use |
...
#### M13-T120 notes
- 2026-09-11 found from the person's report: eight finished children whose parent had run `remove_agent_worktree` showed the engine's refusal "Stored session working directory does not exist: <project>/.worktrees/<slug>" (reproduced with `laser open <child path>` against the live 0.3.9 host). The host already maps a worktree path to its project worker (`projectRootOf`), so the refusal came from `SessionManager.open` reading the header cwd. Fix in the driver: `removedWorktreeCwd(projectCwd, sessionPath)` returns the project checkout only when the stored directory is missing and lies under `<project>/.worktrees/`; the driver passes it as the engine's `cwdOverride`. Nothing is recreated under `.worktrees` (a plain folder there would pose as a checkout). Real-engine driver test reopens after `rmSync` of the worktree and sees `cwd` = project; the resolver test covers still-present, other-project and never-a-worktree cases. `stable-sdk.agent.test.ts` once failed on an unrelated timing-sensitive case while the UI suite ran concurrently; three quiet reruns are green.
- 2026-09-12 post-release review: the reopen only matched `<project>/.worktrees/` (worktrees live under the git toplevel, so a project below the root never matched) and the recovered child was still told it was isolated in the person's checkout. Fixed on `fix/review-0310`: `removedWorktreeCwd` takes the record's `worktree` (path + `removedAt`, now parsed), the worker recovers the role with `isolated: false` under the same rule; D-224; a real-engine test drives the reopened child through `WorkerServer` and asserts the D-156 sentence in its system prompt.

#### M13-T119 notes
- 2026-09-11 found from the person's report: Chat "+" created and listed the session but the main column stayed on the previous chat. Cause: the controller's late-workspace effect re-resolves any `resolving` `chat-tab` destination once `workspaces.chat` is known; `newSession` begins with that same target, so the effect re-opened the previous chat first and the new session's own `resolveSession` found its intent already settled. Fix: `awaitingChatWorkspace` ref records the one intent that returned for lack of a workspace, and the effect serves only that intent. Fake-host test presses + over an open chat and expects the fresh path; it failed on the shipped code.
- 2026-09-12 post-release review: the late-Chat-workspace effect had no positive test. Added: the agents snapshot arrives after the sessions loaded and the waiting chat-tab intent creates and opens the session; both tests fail with the ref guard inverted.

#### M13-T118 notes
- 2026-09-11 by the person's request after 0.3.9: `--notes FILE` is required for `--publish` (refused when empty; a resume must present the same digest); the notes become the annotated tag's body with `--cleanup=verbatim` (git otherwise strips Markdown headings as comments) and `publish.sh` reads `%(contents:body)` as the release page text, falling back to generated notes for a manual tag. The post-workflow download/attestation/manifest verification is gone: the public release is checked through the API only (not a draft, recorded tag, exact inventory with every asset uploaded, Latest). README and AGENTS updated. Orchestrator changes still receive normal review; nothing has been released through the new path yet.
- 2026-09-12 post-release review (0.3.10): the download drop removed the only source-pinned provenance check and any digest comparison; the tag-body fallback in `publish.sh` read the caller's cwd and would take a lightweight tag's commit message as notes; the notes-digest gate blocked every post-tag resume; the AGENTS.md blocker rule was reworded without a decision. Fixed on `fix/review-0310`: `verifyPublishedDigests` (API digests vs the fetched `SHA256SUMS`, coverage exact), `verifyPublishedProvenance` (one `gh attestation verify` on the manifest with the source pinned), the release body compared to the checkpoint's notes, `--resume` after `tag-pushed` without `--notes`, `publish.sh` reading an annotated tag only from `REPO_ROOT`; D-223; 42/42 release tests.

#### M13-T117 notes
- 2026-09-11 found while verifying T115 on the person's machine: the Chat tab showed "Couldn't open this conversation — no project known for session <path>" with a dead Retry. `lasercode-session-tab-last` remembered a Chat session whose file no longer exists (an empty session retired with its idle worker before durable empties), and `resolveTarget`'s chat-tab branch loaded a remembered path on faith; the host's `workerFor` cannot place a path the catalog does not know. Fix and two fake-host tests (newest Chat, else a new one; teeth checked against the shipped code) are committed on branch `chat-memory-fallback` at 7d121ef, not on main: the person chose to clear the one affected machine with the recovery script and to keep 0.3.9 to the startup fix (D-219). Fresh installs carry no stale memory.

#### M13-T116 notes
- 2026-09-11 measured on the installed 0.3.9 (host 1419012): three renderer startups, navigation → opening screen gone with the last session restored, 234 / 212 / 207 ms (CDP `Page.reload` + `performance.now()` when `[data-slot="startup-restoration"]` leaves); host cold start spawn → ready 1.6 s per desktop.log. 0.3.8 sat on the same screen for 120 s.
- 2026-09-11 done: `node scripts/release/release.mjs 0.3.9 --publish --source 463cb869d2df69e828064b7964c54c8fb5aab053` completed end to end — candidate cbab7f7 pushed to main, exact-source CI 34626252419, annotated tag object a175a5e, release workflow 34626602601, public non-prerelease v0.3.9 with 12 assets at 17:25Z, downloaded-asset verification finished 17:37Z. The 0.3.8 stale lock (the person had stopped that run) was recovered first with `0.3.8 --publish --resume --recover-stale-lock` from a detached checkout at its source; it only re-verified the public assets and marked the checkpoint verified.
- 2026-09-11 scope by the person: 0.3.9 fixes the startup freeze only (T115); the Chat dead end is cleared on the single affected machine by the recovery script and T117 stays parked. Source is the T115 commit plus the `StartupShell` guard commit on main.
...
#### M13-T115 notes
- 2026-09-11 general guard added: `Shell` now composes an exported `StartupShell` (notice, gate, prompts) around the frame, and `packages/ui/test/runtime/startup-trust.test.tsx` drives the real provider, controller and `StartupShell` against a fake host that holds `session/load` behind `pi/project/trust_request` — the dialog is answerable over the screen and both answers bring it down. Removing the gate's prompts makes both cases fail. Browser proof against a sandbox host with a planted `.laser/settings.json`: the question renders over the opening screen at desktop width (dark) and at 375px (light).
...
| M14-T3 | Settings → MCP servers | done | worker mcp-settings-ui (01a091d1-20ed-73a4-a484-ea20d2b4199a); live-review fixes by mcp-settings-fixes | merged `763dd52` + fixes `0bb1a46`; UI 1197 on the fix branch, settings suites 116 on main; browser matrix at 1360/390 both themes (`/tmp/mcp-settings-fixes/shots/`) | see notes; live acceptance in M14-T6 |
...
| M14-T5 | Packaged build carries the adapter | done | worker mcp-packaging (01a09264-09f9-73a4-a484-eb29d0307676) | merged `e4303b5`; Linux x64 pack + clean-machine gate pass (real session with a stdio fixture from the bundled Node on an empty PATH, inspect + call), read-only-resources gate, desktop 117 tests, release 42; review `/tmp/review-mcp-packaging.md` APPROVE WITH FIXES, fixes at `1b32cf2` verified by the orchestrator | see notes |
...
| M14-T8 | Shell environment reaches host, workers, command tool and MCP servers | done | worker shell-environment (01a093c4-9a27-72f2-9371-724ec74d3bc2) | merged `eaddac2` (`2b6a0f0`, `783823a`); review `/tmp/review-shell-environment.md` REQUEST CHANGES (advisory refresh for an older host, end-marker finish despite background pipe holders) → fixed, verified; `pnpm verify` green: desktop 129, host 231, cli 73, protocol 102, worker 643; `docs/shell-environment.md` | Linux process proof only; the installed generation needs a restart to carry it |
...
- 2026-09-12 diagnosis: installed 0.5.0 desktop PID 2665281 adopted host PID 2650511 started earlier; host/worker lack the extension-token variable. Interactive Bash finds it. Adapter 2.33.0 `server-manager.ts:1092` hard-codes `pi-mcp-${serverName}` for all transports. No production processes stopped or credentials printed.
- 2026-09-12 checkpoint/handoff at the person's request: report only, `docs/incidents/shell-environment-and-mcp-identity.md`. Real probes confirm normal `bash -c` and explicit non-interactive `.bashrc` source both miss the exported token; interactive `bash -ic` sees it. Pinned shell operations do not source profiles; `.bashrc` returns early for non-interactive callers. Desktop logs prove same-host adoption after Quit. No source implementation or applied patch; H-11 records next steps. The startup-resolution approach in D-226 is proposed, not validated.
...
- 2026-09-11 feasibility spike (read-only, `/tmp/mcp-explore/spike`, `pi-mcp-adapter@2.33.0` with Pi 0.85.0 through `StableSdkDriver` and the stub provider): stdio Playwright registers 24 direct tools as `playwright_browser_*` (cold cache: connected during session start in ~1 s; warm cache: present at open in 13 ms); Streamable HTTP at `http://localhost:8931/mcp` and SSE at `/sse` both work; the `MCP_STATUS_EVENT` snapshot is `{ version, servers: [{ name, status: connected|cached|failed|needs-auth|not-connected|disabled, listenState, toolCount, directToolCount, resourceCount, disabled }], totalTools, totalResources, connectedCount, disabledCount }`, emitted empty at shutdown; direct results are `{ content: [text|image blocks], details: { server, tool } }`, proxy results carry `details.mode` (`search`/`call`/`status`/…) and `details.mcpResult`; zero configured servers still registers `mcp` and `mcpScript` (so the adapter is loaded only when servers exist); the adapter toasts "MCP: N servers connected" and sets status `mcp` (silenced by settings/module); no errors or stray console output in rpc mode.
- 2026-09-11 done: twelve `mcp/*` methods, `mcp/changed`, feature `mcp`, module `mcp`, `lasercode/mcp/status`, `MCP_KNOWN_SERVERS` (Playwright first), D-221 recorded.
- 2026-09-11 the pre-existing untracked `.laser/settings.json` at the repository root (the person's own project config for this checkout) trips `pnpm identity:check` locally; not caused here, and absent in fresh worktrees and CI.

#### M14-T2 notes
- 2026-09-12 review (`/tmp/review-mcp-engine.md`) REQUEST CHANGES: H1 the inspector keyed the engine manager by `scope:name` while sign-in used the bare name, so OAuth never reached inspect/ping/call (no OAuth test existed); H2 the store re-parsed and rewrote a hand-edited file, destroying entries it could not parse; six mediums (engine wording in `authRequiredMessage`, every notify silenced, `unsupported` vs "imported without", `lastIndexOf("/")`, MCP prompt commands labelled Agents, ephemeral needs-auth draft left open) and twelve lows. Fixed at `ae70016` with an offline OAuth fixture server driving start → complete → connected → logout, and a jiti finding of its own: concurrent jiti imports produced two copies of the engine's credential store, now loaded sequentially. Product decisions: info-level engine notifies are dropped, warnings/errors pass; `unsupported` means refused. Merged into `main` as `b6a59e0`; the lockfile needed the MCP SDK tarballs' integrity hashes (`ac5ec0f`) to install frozen, and the identity scanner now exempts the app's own `.<name>/` project directory.
- 2026-09-11 ready at `c49220a` (`0ee135b` engine + methods, `11f3f0b` merge of `fd2d2d6`, `c49220a` switch-off entry, live counts, docs, tests): `packages/worker/src/mcp/{engine,adapter-config,store,session,inspector,import,service}.ts`, `runtime-env.ts`; the adapter is TypeScript-only inside `node_modules`, so `engine.ts` loads it through jiti (the engine's own transpiler) — the only file that knows the adapter; `pnpm-workspace.yaml` `packageExtensions` declares the adapter's undeclared Pi imports; three upstream gaps in `docs/upstream.md`. Proof: `LASERCODE_MCP_LIVE=1` live test passes (provider request carries `playwright_browser_navigate`); standalone inspect of Playwright 1.63.0-alpha lists 24 tools with input schemas in 767 ms. Suites: protocol 50, pi-extension 112, host 225, worker 537 (+ the pre-existing env-var failure).
- 2026-09-11 claimed by the orchestrator for worker `mcp-engine` at base `9ccbf4f`: pin `pi-mcp-adapter@2.33.0` (+ `packageExtensions` for its undeclared Pi imports), `packages/worker/src/mcp/{store,adapter-config,service}.ts`, driver loads the adapter only when servers exist, companion module `mcp`, worker dispatch, host `CWD_ROUTED`, fixture MCP servers over stdio and Streamable HTTP for offline real-engine tests, opt-in live Playwright test. Spike facts the brief carries: the adapter's `getAgentDir()` reads `PI_CODING_AGENT_DIR` (workers inherit it from `piEnv()`; the worker also pins it from `--agent-dir`); its `input` hook holds the first prompt until init (≤30 s); `mcp-oauth-status` triggers a model turn by itself.

#### M14-T3 notes
- 2026-09-12 handoff: assigned six defects complete; final UI build/typecheck/test (1197), staged identity check, and clean-environment `pnpm verify` pass (2393 workspace tests, 42 release tests; three opt-in worker tests skipped). Source-built host + deterministic stdio fixture tested Add/Edit/Import wheel, touch and keyboard; 390×667/1360×900, both themes, reduced motion; Tools saves/reversal and keyboard focus; Run Markdown heading/link/fence; active tab bounds and 12px typography. Screenshots inspected under `/tmp/mcp-settings-fixes/shots/`; report `/tmp/mcp-settings-fixes-report.md`. No engine/protocol/thread edits; own host/Chrome stopped. Task remains in-progress for orchestrator review/integration.
- 2026-09-12 checkpoint: shared ScrollArea now flex-sizes its viewport (no percentage-height trap); Add/Import hold header and footer outside it. Policy saves re-inspect because the worker closes connections on save; tests cover effective switches, reversal, pending and refresh failure. Run uses MarkdownPreview; imports show full commands/URLs and secret names; MCP labels use the 12px typed token. Built workspace and full UI suite passed (1195); focused settings suite 59 passed including resize restoration. Source-built isolated host/browser proves wheel scrolling, all three policy reversals and Markdown; remaining browser import/touch checks in `/tmp/mcp-settings-fixes/`.
- 2026-09-12 claimed: live-review defects 2/3/5/7/8/9 in the assigned UI area, starting from `8f46c5e`; prior UI implementation merged, live-proof handoff is `/tmp/mcp-live/REPORT.md`. Isolated frozen install completed; inspect form geometry, refresh effective policy after saves, reuse safe Markdown, disclose full imports and restore active tab visibility.
...
- 2026-09-11 review (`/tmp/review-mcp-settings-ui.md`) REQUEST CHANGES: blocker B1 — switching Direct off for the last `only` entry deleted `only`, which means every tool direct; S1 the Tools tab ignored the worker's effective `visibility`/`approval` and globs; S2 un-marking a stored secret wrote `""`; S3 ping detail dropped; S4 sign-in dialog never noticed completion; S5 Remove bypassed disconnect; S6 every `mcp/changed` re-scanned imports and re-inspected; S7 duplicated looser name pattern; S8 ARIA tabs/radios without keyboard; S9 tab unreachable without a project; S10 dialogs could clip on a short phone. All fixed at `46aa88e` on top of the `main` merge `ea6ac19` (optional `transport`; switch-off sends `{ name, disabled: true }`; `overridesGlobal` rows read-only with Turn on / Edit the every-project entry).
...
- 2026-09-11 hydration landed at `4531677`; review (`/tmp/review-mcp-transcript.md`) APPROVE WITH FIXES: Find occurrence misalignment between the rendered Markdown and the raw projection (fence labels, links), resource blocks marking `name` but indexing `uri`, a session server named `task`/`start`/… could capture harness tools, the live aggregate still said the raw tool name, `mcpScript` hid its args; all fixed at `9e47b6f` with a rendered-text projection (`mcpDisplayText`) and the tolerance rule in `docs/search-content.md`. The reviewer replayed 134 real persisted edits through both diff paths: patch-based counts equal the old args-based counts in 132, the two differences are corrections (D-214). Host saved-history search still flattens to text (documented divergence; host owner's note to update).

#### M14-T5 notes
- 2026-09-12 ready at `86a8e48`, review (`/tmp/review-mcp-packaging.md`) APPROVE WITH FIXES, fixes at `1b32cf2`, merged `e4303b5`. Findings: the adapter's keyring 1.3.0 binding was transitive and unasserted (the pack log warns `platform-specific optional dependencies not bundled … keyring-linux-arm64-gnu@1.3.0`), so the six 1.3.0 bindings are exact optionalDependencies of the worker and the build target's binding is asserted for both keyring versions; visibility counts `dependencies`+`optionalDependencies` over the closure reachable from the desktop (peers are not copied by the packager); jiti `fsCache: false` cost ~0.85 s per worker start, replaced by a persistent cache under `<agentDir>/cache/jiti` with a non-writable fallback (1.5 s cold → 0.6 s warm); the adapter's `skills/**/*.md` survive the Markdown exclusion; the clean-machine gate opens a real session with a stdio fixture from the bundled Node on an empty PATH, inspects and calls it. Other platforms remain unproven until a release runner builds them.

#### M14-T6 notes
- 2026-09-12 live proof (`/tmp/mcp-live/REPORT.md`, 73 screenshots, RPC and provider traffic captured): real stdio and HTTP (Streamable + SSE) conversations with Playwright work — 24 prefixed tools in the provider request, screenshot image row in the transcript, no engine slash commands in the picker; import moves an inline token to the 0600 secrets file; project switch-off writes exactly `{ name, disabled: true }` and a new session gets none of that server's tools; keyboard/touch/reduced-motion paths hold; Resources/Prompts empty states are designed. **Nine defects**, three high: (1) the gallery's default `npx` fails because `runtimePathAdditions` prepended npm's internal `bin` whose shim `npx` cannot find Node — and the bundled runtime ships no `npx` executable at all; (2) Add/Edit/Import dialogs clip and cannot wheel-scroll; (3) Tools switches and the visibility sentence stay stale after a successful save; (4) list status ignores Test/Inspect/Ping until a session snapshot; (5) Run prints Markdown source; (6) ENOENT reaches the person verbatim; (7) Import hides args/URL paths and says `transport.headers.Authorization`; (8) 11px scope labels; (9) the active Settings tab clips after narrowing. OAuth browser sign-in not proven live (the offline fixture has no standalone main; covered by `test/mcp/oauth.test.ts`).
- 2026-09-12 engine fixes merged (`c139445` + `8a6e985`): the worker's PATH gets the runtime dir first and, when the bundled npm has no `npx` beside the runtime, generated launchers under `<agentDir>/bin`; the gallery's default `npx -y @playwright/mcp@latest --isolated` now yields 24 tools from the CLI host with only the nvm runtime bin on PATH; Test/Inspect/Ping results are remembered so the list says `ready · 24 tools · 24 direct` right after Add and `mcp/changed` follows every status change; ENOENT/EACCES are explained with the PATH searched. Orchestrator re-packed on `main` and ran the clean-machine gate: every claim held, incl. `npx: 11.19.0` from the bundled runtime with an empty PATH. UI fixes merged at `0bb1a46`.
- 2026-09-12 re-acceptance (`/tmp/mcp-live-2/REPORT.md`): built `main` at `b75ed6a`; default gallery add → Ready · 24 tools · 24 direct; dialogs scroll by wheel and touch at 390×667 with header/footer fixed; Tools switches truthful with focus kept; Run renders Markdown; missing-command copy names the PATH searched; import shows the full transport and says "Authorization header"; 12px labels; active tab stays visible. OAuth proven in the browser against the offline fixture through a redirecting shim: `mcp/auth/start` → loopback callback → the dialog notices and closes → `connected` → inspector lists the fixture's tools → Sign out → `needs-auth`; the pasted-code path also passes; a fresh conversation called `oauth-fixture_whoami`. One low defect (PATH overflowing the failure card on a phone) fixed by the orchestrator at `2de5f9f`.

## M15 · After the MCP release

| ID | Task | State | Owner | Evidence | Notes |
| --- | --- | --- | --- | --- | --- |
| M15-T1 | Native file opener: file card and format-aware viewer modal | done | worker file-viewer (01a09268-985b-73a4-a484-eb2fd91f4208) | merged `2e8a485` (`8f85557`, `6f86506`, `edf24a5`); review `/tmp/review-file-viewer.md` APPROVE WITH FIXES, fixes verified; UI 1255 / worker 625 on main | see notes |
| M15-T2 | Dictation is always transcribed as English | done | worker dictation-english (01a091e2-ac35-73a4-a484-ea5595d1b475) | merged `be1d2fd` (`202f645`, `3f8e608`); review APPROVE WITH FIXES, fixes verified; 27 transcribe tests; `docs/dictation-language.md` | see notes |
...
| M16-T5 | Playwright: where the browser runs | done | worker playwright-browser-choice (01a094d9-e2a3-7513-89ef-89b0b2a2f4af) | merged `f93c337` (`cc6cc6b`); protocol 103, UI mcp 62, worker mcp 98; `docs/mcp.md` | root cause of the extension failure: Scrnli extension steals the tab debugger (`/tmp/pwx/REPORT.md`); Laser ships the choice + a page-open Test |
| M16-T6 | Thinking level survives the conversation | done | worker thinking-level-revert (01a094e5-1270-7513-89ef-89c7c9e1b9db) | merged `07d4222` (`d83c53a`); two resets fixed (fallback `engine-port.ts` re-derived from settings; reload restored the definition default); 8 real-worker regressions; worker 657 | suspect: fallback `engine-port.ts:60` setModel re-derives the level from settings |
| M16-T8 | Every word of the system prompt shows where it came from | done | workers prompt-provenance-ux + provenance-fixes (01a09564-c073-7405-a9c3-603047b9eb42) | merged `94c017f` (`af6cd66`, `8daedda`); review `/tmp/review-provenance.md` APPROVE WITH FIXES (markers leaking into Conversation; indistinct origin tints) → fixed; pi-extension 118, UI 1376 | the person's screenshot: `<inline:lasercode>` labelled "Extension modification", click → "Couldn't open this source file"; a large span "Source not recorded"; sources hidden in a dropdown |
| M16-T9 | Dictation on a fresh project landing | done | worker sidebar-and-dictation (01a094fe-3dbf-7513-89ef-89f4db599050) | merged `2710da6` (`3ec4924`); live check `/tmp/m16b/REPORT.md` pass incl. touch |
| M16-T10 | Sidebar: archive takes the tree, long groups fold | done | worker sidebar-and-dictation | merged `2710da6` (`5ae37f7`); empty session first; live check pass; the person's seven-row rule pinned by coordinator test `5e77d08` — seven ordinary rows, only live work (working/waiting/active run, including a descendant's) exceeds them, a finished-unread old row does not, and the list returns to seven by itself when the work ends; an unstarted row and the open session stay deliberately outside the count |
| M16-T11 | Streaming stays smooth | done | worker perf-batch-1 (01a09510-e7ab-7513-89ef-8a3a28ec56e5) | merged `1a1d3c7`; settled footers 6000→0 per 500 deltas; live check pass; reload usage restored by the orchestrator | risk review `/tmp/review-perf-plan.md`: #1/#4/#2 safe with guards, #6 deferred to batch 2; pre-existing timing-namespace defect fixed alongside |
| M16-T13 | Bidirectional layout for the whole app | done | worker bidi (01a0951b-bd4d-7513-89ef-8a60e2ae5ddd) | merged `267f315` (`60a870b`, `c1ac85b`); LTR pixel-identical (4 diffs zero); 28 RTL screenshots; direction guard in verify |
...
- 2026-09-13 host done on `agents/logstore-budget-76e6b1c1` (`a0f7c6b`): per-session body limit, global byte budget enforced at ingestion with a synchronous backstop past a 5% overshoot, `auto_vacuum = INCREMENTAL` for new stores and one logged VACUUM to convert an older one *after* the budget pass (it then copies the live set, not the whole file), bounded `incremental_vacuum` steps off the request path. `entries.body_released` is an additive migration; a body shared by hash is only dropped when no retained entry references it. Measured (1 MiB bodies, 200 per run × 3): ingest 4.06–4.15 ms → 3.08–3.16 ms mean, store 210 MB → 52.8 MB for the same input; maintenance steps max 11.5 ms / p95 6.5 ms between requests; at the default 1 GiB budget an ingest that releases nothing is unchanged (3.72 vs 3.81 ms).
- 2026-09-13 measured and rejected throttling the WAL checkpoint (it looked cheaper per step): ingestion went 3.2 → 6–9 ms mean with 40–77 ms spikes, because SQLite's own automatic checkpoint then has a backlog to copy on the request path. The maintenance pass checkpoints as soon as a vacuum pass catches up; the comment in `reclaimStep` records why.
- 2026-09-13 UI done (`93c6264`): released bodies answer with their summary in the inspector and the log detail pane (`logs/ReleasedBody.tsx`), and Settings → **This device** gains a Log store section with the size, the body budget in use and a Clear written for a person. Placement decision: This device is the only project-independent, laser-owned settings surface — Advanced is the engine's own per-project settings and a store belonging to the whole machine has no row there — so the section's copy names the computer running the host rather than implying the browser. Browser matrix 1360/390 × dark/light with `scripts/browser-check/test/log-store-settings.mjs`: copy, 12px floor, no sideways scroll, Escape backing out unchanged, keyboard-only clear, file 8.9 MB → 61 kB (1.2 MB → 135 kB on the phone cases).
- 2026-09-13 not covered: an end-to-end browser check of the *released* inspector panel (it needs a host started with a tiny per-session limit); the released path is covered by `packages/host/test/logstore-budget.test.ts` through a real `HostServer` router and by `packages/ui/test/logs/released-body.test.tsx`. Report `/tmp/logstore-budget/REPORT.md`.

- 2026-09-13 done, merged `949d617`. Orchestrator read the maintenance loop: every release/prune/vacuum slice is a `setTimeout(0)` step of ≤16 bodies / 256 rows / 1024 pages, so a 27 GB legacy store is drained across many yields; the one-time VACUUM runs after the releases and is logged with its duration.
#### M16-T35 notes
- 2026-09-13 claimed from `docs/handoff-open-problems.md` P2 for 0.6.2; reproduce red first.

#### M16-T36 notes
- 2026-09-13 from P3; sequential after M16-T32 milestone 2 in the same owner (thread/ files).

#### M16-T37 notes
- 2026-09-13 claimed from P4 for 0.6.2.

#### M16-T55 notes
- 2026-09-14 claimed from the person's direct 0.6.3 report: reproduce sending a streaming prompt while 1,200 px above latest, then make the new turn own latest without disturbing passive reading
- 2026-09-14 reproduced in the built app: send from a 1,200 px history gap; the stream starts but its first line remains unmounted and the browser wait times out. Sending never changes the viewport controller's `following=false` reading place.
- 2026-09-14 checkpoint: the local composer submit/click now owns the latest intent; passive run starts, including a running session revisited after switching away, preserve its saved reading place. Resize observation follows streamed growth without subscribing the whole window to every message delta; wheel/touch/scrollbar and Page/Space keys focused inside transcript rows opt out. Browser proof holds a zero-pixel gap for all 20 streamed samples (`/tmp/browser-check/run-jU9fW6`, 1360/390 × dark/light); focused viewport/settings tests 29/29 and UI build pass.
- 2026-09-14 final review `run_21a166a8`: excluded composer controls from keyboard scroll intent and covered overlay plus left-side RTL scrollbars without treating the transcript gutter as intent; focused viewport 27/27.
- 2026-09-14 done: UI suite 1,668 passed/1 skipped; final stream-follow matrix `/tmp/browser-check/run-mBMlcX`.

#### M16-T56 notes
- 2026-09-14 claimed from the person's direct request: replace Environment with collapsible per-project settings and keep only one pre-Bash command.
- 2026-09-14 semantics confirmed by the person: empty disables the setting; a non-zero pre-command prevents the requested Bash command.
- 2026-09-14 checkpoint: Projects tab lists all known projects, opens the active one, saves only the fixed preface form, and shows blocked/trust/legacy-resolver state without exposing saved arguments. Worker startup accepts preface-only config; `{ setup } || exit $?` blocks every statement in semicolon and multiline requested scripts after setup failure; dynamic bridges make live saves affect the next parent, child or background command. The host's public status projection strips legacy arguments and their reversible approval fingerprint while retaining an argument count. Projects matrix passes 1360/390 × dark/light at `/tmp/browser-check/run-URDS29`.
- 2026-09-14 done: host 316/316; worker project/preface/server/background regressions pass; UI suite 1,668 passed/1 skipped; final Projects matrix `/tmp/browser-check/run-xhSf47`.

#### M16-T57 notes
- 2026-09-14 claimed from the person's direct 0.6.3 report: the session remained in Compacting after compaction had finished. Trace the terminal compaction transition before changing code, then add a red regression at the owning layer.
- 2026-09-14 checkpoint: `compaction_end` now publishes terminal `isCompacting: false` synchronously on success/failure/cancel without a stale deferred re-read; `navigateTree` publishes its own terminal state because branch summaries emit no compaction event. Worker map/real-engine tests and UI reducer test pass; real built app auto-compaction appears and clears at `/tmp/browser-check/run-p3dBP2` (1360/390 × dark/light).
- 2026-09-14 done: worker map/real-engine and UI reducer regressions pass; automatic-compaction matrix `/tmp/browser-check/run-p3dBP2`.

#### M16-T58 notes
- 2026-09-14 claimed from the person's released-build report: agents sometimes stop without any person-owned stop. Forensics first: distinguish process loss from unexplained session abort and preserve the exact initiator before changing lifecycle code.
- 2026-09-14 checkpoint: durable run evidence confirms both failure classes. Multiple live children were failed together as `initiator: harness` when the project worker disappeared; a separate four-agent batch and its immediate retries received `stopReason: aborted` while the host and project worker stayed up. No run records a person-owned stop for either class.
- 2026-09-14 correction after pinned-engine source review: async `steer` notifications only enqueue; the four children contained no task/agent events and agent-core's own abort flag was false (`stopReason: error`). The shared provider transport returned `AbortError`/`Request timed out`; `harness.ts` then permanently failed each child on any assistant error while top-level sessions remain usable. The proposed notification-lane change was reverted before commit. Separate 15:45 failures coincide exactly with an orderly desktop quit/relaunch, whose quit path has no live-run guard.
- 2026-09-14 provider failure fix: transient transport/provider errors now leave the same run live and schedule a transcript-safe continuation with exponential backoff (1 s to 30 s, no terminal retry count); a successful assistant response resets the backoff, deterministic credentials/billing/quota errors still fail, and explicit stop/session closure cancels the timer. Red-first harness cases failed 3/4 before implementation; full harness suite passes 102/102, including repeated failure and explicit-stop races.
- 2026-09-14 shutdown/initiator checkpoint: `session/cancel` and engine extension aborts emit credential-free structured initiator diagnostics into the durable host log. Desktop shutdown reads the typed run/task sources and warns before stopping an owned host; orderly host close stamps live runs as user-owned cancellation before worker exits, while a crash remains harness-owned failure on reload. Desktop 156 passed/1 skipped and host registry 14/14 pass; worker/desktop/host typechecks pass after protocol log-module widening.
- 2026-09-14 final review `run_21a166a8` fixed an attribution inversion: `HostServer.close()` now defaults to harness-owned crash cleanup and only explicit signal/desktop shutdown passes `initiator: user`; retry backoff is visible in fleet activity; cancelling quit with no tray reopens a window.
- 2026-09-14 done: harness 102/102; host registry 15/15 and full host 316/316; desktop 156 passed/1 skipped.

#### M16-T54 notes
- 2026-09-14 claimed: reproduce scroll-up on 0.6.3 bytes with the paced wheel trace (SLOW=1 TRACE=1), then fix red-first
- 2026-09-14 reproduced on the person's current 992-line session: 3872→3372 on the first wheel, then every 70 ms wheel stayed at 3372 and rest jumped back to 3872. A layout commit called absolute `restore()` after browser default scrolling but before its `scroll` event captured the new place. Red unit: mounted anchor + wheel/default-scroll/commit race returned 900→1400. Fix: active reading commits apply structural/measurement deltas only; the absolute restore waits until input settles. Green unit 21/21; same real-session trace now moves on every wheel and has no rest jump. Browser regression added; first 1360/390 × dark/light matrix passed (`/tmp/browser-check/run-cjc91P`, phone cases use touch events).
- 2026-09-14 independent read-only review (`scroll-diagnosis` run `run_97a2d5b0`) found the fix sound and the same race on native momentum/scrollbar/Space paths after the 400 ms input timer. Added a second red test (800 restored to 1100), then made unexpected native scroll events refresh reading while expected programmatic corrections remain filtered; following-tail restoration stays immediate. Focused viewport 27/27 green; final 1360/390 dark/light pointer/touch matrix passed (`/tmp/browser-check/run-kMGTAd`).
- 2026-09-14 done: v0.6.4 published from candidate `c4ee554` and source `bc78bd7`; source CI 34835303688, release run 34835834970, 12-asset inventory verified; https://github.com/youssefsiam38/laser/releases/tag/v0.6.4

#### M16-T42 notes
- 2026-09-14 the person: scrolling up in 0.6.2 "repeats the section". Reproduced with a paced wheel (one notch, 650 ms pause) on the huge fixture: at the top the earlier page arrives (scrollHeight 5713→11436) and scrollTop stays 125 — the view is left at the top of the new page while the section being read moves a page down; every further notch loads another page the same way (top row 983→961→941…). Cause: `TranscriptViewport.restore` deliberately leaves the position alone while the person is reading and the anchor row is unmounted (0.6.1's fix for the viewport fighting the reader), and nothing applied the structural shift of the rows inserted above the anchor; the render after the prepend also computed its window from the DOM top, unmounting the anchor. Fix: `setIds` records the anchor's movement from the inserted rows' estimates, `ranges()` computes that render's window around the anchor, `committed()` applies the shift before paint; `restore`/`measure` then refine as before. Unit test red/green; paced browser trace: 625→5859 at the page arrival, then two checkpoints per notch.

#### M16-T30 notes
- 2026-09-13 claimed: from `/home/youssef/electron-optimization-report.md` priority 1 + main-process startup; worker perf-desktop in `.worktrees/perf-desktop-7aeed9a9`.
- 2026-09-13 first runs of all four perf workers died with the project worker ("The project's worker stopped before this run ended") while a second orchestrator session shared this checkout and reverted the ledger; the person ended it and handed the checkout to this session. Worktrees kept their uncommitted WIP; new runs resume in the same worktrees.

- 2026-09-13 second runs (`-2`) also died with the app restart; the person handed the checkout to orchestrator session `01a09b3e`. Third runs (`-3`, worktree=false, working in the existing worktrees) resume from each branch's committed checkpoint plus its uncommitted WIP.
- 2026-09-13 done: `backgroundThrottling` default restored, secrets ∥ shell env, closed-vocabulary startup timeline in the desktop log; renderer inventory needed no further change (delta backstop timer, `useTick` catch-up, seen guard, prefs wait-for-visible, reconnect probe on desktop path).
#### M16-T31 notes
- 2026-09-13 claimed: report priorities 4/6; full Shiki catalog kept (removing grammars loses highlighting for those languages; they are already lazy chunks). Worker perf-bundle in `.worktrees/perf-bundle-a94c069a`. Orchestrator adds the host `cache-control: immutable` header for hashed assets (`packages/host/src/server.ts`, `test/static-cache.test.ts`).

- 2026-09-13 second runs (`-2`) also died with the app restart; the person handed the checkout to orchestrator session `01a09b3e`. Third runs (`-3`, worktree=false, working in the existing worktrees) resume from each branch's committed checkpoint plus its uncommitted WIP.
- 2026-09-13 done: lazy KaTeX (by delimiter scan), inspector body, agent map + React Flow CSS, protocol dist declared side-effect-free so Handlebars/zod follow their callers; explorer validator fetched beside its listing. Not split: dnd-kit, lobehub icons, onboarding, core conversation.
#### M16-T32 notes
- 2026-09-13 claimed: report priority 3 + phase 1/3 measurement; measure first, fix only from traces. Worker perf-streaming in `.worktrees/perf-streaming-8b9c30fe`.

- 2026-09-13 second runs (`-2`) also died with the app restart; the person handed the checkout to orchestrator session `01a09b3e`. Third runs (`-3`, worktree=false, working in the existing worktrees) resume from each branch's committed checkpoint plus its uncommitted WIP.
- 2026-09-13 milestone 1 on branch (`2d26293`, main merged at `019da6b`): corrected profiler (per-instance records; the inherited alternate-based counts were 2–3× over), fixes in find/Thread/Composer/GoalBar/session-preparation/WindowRow: 625→518.6 renders/delta; production huge: 0 long tasks, memory flat over 10 switch pairs. Found fifteen whole-view `useLaserView()` readers outside the lane (Shell/TopBar/palette/dialogs/elements/telemetry) that make every surface a stream subscriber (experiment 625→278). Orchestrator extended ownership to those files; milestone 2 run_3c711ded targets 0 renders/delta on sidebar/rail/telemetry/fleet/workbench.
- 2026-09-13 milestone 2 done: `samePresentationView`/`currentView` in presentation-state.ts, field selectors across shell/dialogs/elements, tool timeline keyed on tool-call state, stable assistant-ui list ids. Left open: `mobile/Notices.tsx` view read; ~4 settled-row bodies/delta from Radix ref re-registration; map re-renders per batch by design. Milestone 3 = M16-T36 (same owner).
#### M16-T33 notes
- 2026-09-13 claimed: report priority 7; candidates seen in the last unpacked build: `@lasercode/*` src/test/public/tsbuildinfo (~5 MB), `recheck-jar` (23 MB, only reachable when the native `recheck-*` binary is absent), `es-abstract` (11 MB, es-shims — keep), `highlight.js` (9 MB, Pi's — keep), `@esbuild` (Pi's TS loader — keep). Worker perf-packaging in `.worktrees/perf-packaging-325edf4e`.

- 2026-09-13 second runs (`-2`) also died with the app restart; the person handed the checkout to orchestrator session `01a09b3e`. Third runs (`-3`, worktree=false, working in the existing worktrees) resume from each branch's committed checkpoint plus its uncommitted WIP.
- 2026-09-13 done: `d051f50` merged `932d4f3`. Removed with loader proofs: recheck-jar, pi-web-access artwork, `@lasercode/*` src/public/configs, dependency `src/` dirs, hljs styles, handlebars AMD, uglify-js, @google/genai browser bundles, 14 unused web-streams-polyfill builds. Gate: clean-machine all claims pass; recheck native renamed away still passes on the pure engine.
#### M16-T16 notes
- 2026-09-12 claimed: research then implement chat-style loading (paging, tail-first, mounted-row bound).
- 2026-09-12 stage A (`ae423a5`) paged the sidebar catalog. Its own measurements: full route 5.13 ms/58 KB → paged 7.68 ms/30 KB, i.e. 2.55 ms SLOWER on the host, and no open/switch improvement claimed. `pageCatalog()` projects on top of the already-decorated catalog, so it removes no host work.
- 2026-09-12 stage B (`6cc8572`) tail-first anchored history pages: 805 KB → 17 KB on the wire, and — the number that matters — mounted rows 2,000 → 40 (their own probe: 56,512 DOM elements / 2,717 ms → 3,915 / 806 ms).
- 2026-09-12 independent review `/tmp/review-chat-loading.md` (verdict PARTLY MISAIMED): the 2,000-message switch is 4,133 ms of which the entries RPC is 4.8 ms and warm `session/load` 1.3 ms — 99.85 % renderer; A was the wrong thing to do first, B largely subsumes stages C and D. Four correctness/structure findings sent back (dishonest `complete` flag after paging to the top; a Telemetry disclosure silently loading the whole transcript; `shareHistoryBlocks` double-`JSON.stringify` where the block id should come from `message_end`'s `entry.id`; history lifecycle to live in `runtime/history-loader.ts`). C, D and E dropped from this milestone; the re-baseline is handed to M16-T24.
tmp/review-chat-loading.md`: PARTLY MISAIMED — cut to A+B, C/D/E dropped from this milestone; see notes |
| M16-T17 | Per-project environment command | in-progress | workenv-2026-09-12-a | branch `agents/m16-t17-project-env` @ `2794790`; protocol 126, worker project-env 22, host 10, MCP 103; browser pass both widths/themes | see notes |
| M16-T29 | Long conversations stay fast after use | done | recover-transcript-acceptance `01a09a1e-5a0f-70e7-a766-c23969cb5b53` | B `e9c47d9`, `/tmp/sustained-navigation/B-REPORT.md`; full verify/matrices pass; 2k enter 4.62→2.31s, leave 1.66→2.21s; live-growth failure at1240/heartbeat preserved | B fixes2499c93 reviewed/verified, merged1b5f826; coordinator focused291 + merged fullUI1524/tsc/identity pass; mouse/touch final matrices B-fixes-matrix/run-tP6KTT and B-fixes-touch/run-gZSi2m. C approved to same owner after private merge1b5f826: canonical history + bounded transcript/map, integrated anchor/find/selection/approvals per INTERFACE-PLAN; ownership extended narrowly to DialogBody.tsx/tests for session+request presentation values/busy/deadline surviving inline/footer remount, no new submission authority. Owns planned UI callers/tests, no MCP/Composer/projection rewrite. C207c57e+17path WIP, C-RECOVERY-HANDOFF.md: finalUI1546/1skip+tsc/build/identity pass; inherited live2000+nativeSelectAll recovered, huge anchors0px, cross-build2000 median323/p95471 misses target (not final comparison). Three action attempts traced version target lost when layout scroll resets following=true. Follow-state ownership fix red/green incl queued-after-settle and user cancellation; focused34/1skip, build/tsc/identity pass. Desktop action run-qmU0lK +both themes pass. New390dark/touch Previous-after-Edit React185 blank renderer, durable C-phone-actions-failure; phone update-loop and accepted-version defects fixed; production action matrix desktop/phone/themes/touch/reduced-motion pass. UI1558/1skip+tsc/build/identity pass. Same-build240/2000: long↔long324.9/413.3ms resident,432.3/526.3ms real-input,2.34×ratio, anchors0px; misses remain. Bounded trace historyRows O(N²)32.5ms self/39ms folded HistorySection. Same owner run_aac26852 scope extended to components/shell/model.ts historyRows/helpers +TelemetryPanel.tsx HistorySection/direct tests: linear semantics-preserving depth/reuse, no canonical projections rewrite, weak/lifecycle-bound cache only. WIP f241aef2 preserved. D-236 user explicitly changes ordinary re-entry to latest~40/earlier paging, not expanded window/deep position. Same owner interrupted with checkpoint+plan request; permitted UI history-loader.ts/LaserProvider normal-open +related store/history tests, no backend/protocol expansion without approval. Avoid obsolete retained-2k switch benchmarking; retain bounded expanded/live/outgoing work and compare equivalent recent-tail returns. Explicit targets and same-session anchors preserved. Checkpoint26files/2efd8b1; linear shell23tests and fresh-tail loader/window23tests+tsc pass; content desktop pass. C-D236-PLAN/OWNERSHIP-NOTE accepted D-238: per-rendered-owner authoritative history windows, canonical metadata/questions/submission authority, atomic raw-update fanout/epoch reconciliation and bounded lifecycle. D236 implemented/committed c55fdf6 (WIP checkpoint) +27721b3: authoritative tail40 with old window retired before the request, generation/revision fencing of stale or replaced-loader replies, runtime/history-owners.ts per-surface windows folded from the same raw actions before notification, canonical session/questions/queue/drafts, viewport forgets only that surface and resolves a missing place to latest; windowless legacy responses fall back instead of failing the open. UI186files/1578pass/1skip, identity pass, browser D236 run-m2ETC9 wire reads exactly [{tail:40},{tail:40}] with draft preserved, action matrix run-5ByBae desktop+phone both themes. Worker runtime-env failure is the owner's node/npm layout only — coordinator reran it 6/6 at c2fcd84. Phone D236 route passed7569e04 (run-PYDlDj, all four1360/390×dark/light cases: wire reads exactly [{tail:40},{tail:40}], return from checkpoint101, older content re-reachable, draft kept, pending approval still answerable;12/13 mounted rows). Recent-tail timings this build: desktop light240 137.1/208.7 resident and168.5/247.2 actual input,2000 98.9/133.8 and129.3/163.0 — all targets met, dark equivalent,2000 no slower than240. Phone resident meets everywhere (worst155.5/200.6); actual input misses the300ms median only at390-dark/2000 (348.8/365.9, p95 within500), attributed to the motion-enabled path plus closing the Sessions sheet (reduced-motion is187.6 same build). Live-grown return unmeasured; old deep-restore ratios superseded. Integrated review C-REVIEW.md reproduced every claim (UI1578/1skip, tsc, build) and passed areas1–2 — real bounding at40/240/2000/10000 with canonical ids intact, sound height index/overscan, one controller per scope, addressable map, shared question guard, semantics-identical linear historyRows with WeakMap reuse — but requested changes on two reproduced blockers: B1 the re-entry fence is captured before historyReset strips historyRevision, so every ordinary return after the first discards its authoritative tail (the browser assertion of two {tail:40} reads encoded the defect; a single-open Beam scope is left empty), and B2 history-owners replays history actions into peer windows so a canonical re-entry wipes an expanded peer, with no explicit epoch/branch reconciliation. All timings are therefore void and must be re-measured, with one reduced-motion390-dark run to settle the phone attribution (judged credible but confounded). Same owner run_33a68e51 has the batch, plus R1/R2 dead-code deletion, R4 handed-back prompt, N1 map tokens. Corrections e8e5b2e+8f27404: fence retaken after the reset so every ordinary return keeps its tail (reviewer's own repro now passes, including the single-open peer), windows no longer replay another surface's history actions and an epoch or leaf change moves every surface onto it, plus R1 deletion of preserve-reading-position and three duplicated scroll fallbacks, required presentation, jump restored for the version picker with its handed-back prompt, map preview tokens. R3 refused with reasons (cannot split without moving behaviour); hooks fall back to a documented detached controller so harness rows still mount. Final bytes: UI185files/1579pass/1skip, tsc, build, staged identity/diff; D-236 route exactly one {tail:40} per return (run-PoFfm5), action matrix run-EUZYcz. Re-measured: every desktop target met (light2000 128.4/177.6 resident,158.4/207.5 input),2000 no slower than240; phone dark2000 input median316.2/p95337.7 still misses the300ms median, and a same-build motion-off run measures159.4 — the cause is measured, not inferred. Coordinator verified clean tree, focused52 pass and the B1/B2 diff, merged a3f19f9, full verify taskt-527fa44c. Inherited SelectAll/live-2000 not re-verified on this build; live-grown return still unmeasured; phone median miss accepted and recorded. Merged verify passed: UI1583/1skip, worker866/4skip, host285, protocol143, pi-extension121, cli73, desktop135, release42, harness9. Final same-build240/2000 comparison and remaining interaction matrix then one review. No repeat broad baseline campaign Hotfix 0.6.1 `1859e4a` (D-241): black window on scroll-up in a real conversation, viewport fighting the reader, earlier pages not loading at the top; full verify UI1586/1skip, worker866, host285; browser matrix huge fixture 4/4 and the person's real 1,732-message conversation to its first prompt |
| M16-T28 | `@` is a file explorer, not a guess | done | at-explorer `01a098b8-d3f9-74c0-9c74-c7b748bbd5db` → coordinator integrated | merged `5193fc9`; review `/tmp/review-at-explorer.md`, fixes `/tmp/at-explorer-fixes-report.md`; coordinator UI98/host60/protocol137 + identity pass; mouse `run-SNUYpK`, touch `run-mk9cIQ` matrices | single review batch verified; coordinator 60k probe 229ms/max heartbeat gap 5.53ms with tail reachable. F9 public-method deletion deferred; scan memory/queue residual limits documented. Merged workspace `pnpm verify` passed, `/tmp/verify-at-explorer-merged.log`; release gate remains separate. D-232 lands with coordinator planning commit |
| M16-T30 | The hidden window costs nothing | done | perf-desktop-3 `01a09c17-14ae-750e-83c5-64f714dfb7b9` | merged `f8b4b7d` (`025c9c8`, `af3bab1`); real window hidden 390 s: 67 ticks vs 390 unthrottled, correct state 3 ms after show; timeline warm host-ready ≈1.16 s, app loaded ≈1.25 s; desktop 144, UI 1599 | `/tmp/perf-desktop/REPORT.md`; dev command fixed (`electron .`); duration "1m 60s" defect fixed by orchestrator on main |
| M16-T31 | The first conversation loads only what it needs | done | perf-bundle-3 `01a09c17-6949-750e-83c5-64fcbf83d98b` + orchestrator (`7dac643` immutable assets) | merged `dec848e` (`dd4ea0b`, `8d722ef`, `60f79be`); initial JS 2,388,895→1,870,132 B (−21.7 %), CSS −25.4 %, entry evaluate 66→56 ms; UI 1595 pass; matrix run-1Kfd8M 4/4 | `docs/perf-bundle.md`; `/tmp/perf-bundle/REPORT.md`; guard `test/perf/bundle-boundaries.test.ts` |
| M16-T32 | Streaming touches only the streaming row | done | perf-streaming-3 `01a09c17-bac1-750e-83c5-65039e7a6575` | merged `4a22d77` (`63ade32`, `965c6af`, `3f47a4b`); production huge: 625→82.2 renders/delta, 0 long tasks, sessions panel 57,420→4,226 bodies/511 deltas, memory flat over 10 switch pairs; UI 1605 pass; matrix run-1p98Ui | `docs/perf-streaming.md`; `/tmp/perf-streaming/REPORT.md`; profiler `scripts/browser-check/profile-streaming.mjs` asserts per-delta budgets (accepted: literal zero is wrong because turn start/end and catalog changes legitimately wake a column) |
| M16-T33 | The package carries only what runs | done | perf-packaging-3 `01a09c18-05a2-750e-83c5-6508afd8e8c6` | merged `932d4f3` (`d051f50`); dependency tree 208.0→151.4 MiB (−27.2 %), unpacked 698→634M; clean-machine 16/16 incl. new recheck pure-engine row; native-renamed proof passes | `docs/perf-package.md`; `/tmp/perf-packaging/REPORT.md`; refused `.d.mts/.d.cts` by extension |
| M16-T34 | The log store is bounded in bytes and visible | done | logstore-budget `01a09c58-b3a1-750e-83c5-65577891ae38` | merged `949d617` (`a0f7c6b`, `93c6264`); 50 bodies/session + 1 GiB at ingestion, incremental vacuum, released bodies answer a typed summary; 210 MB→52.8 MB same input; maintenance max 11.5 ms; host 297, UI 1607; matrix log-store-settings 4/4 | `/tmp/logstore-budget/REPORT.md`; Settings → This device shows size + Clear |
| M16-T35 | The API request modal does not blink while the session streams | done | inspector-flicker `01a09c58-fedd-750e-83c5-655c0ac7e69f` | merged `0010cfd` (`a9014f3`); red `loaders 2, reloads query+content` → green matrix 4/4 zero mutations (run-BEx0lS); UI 1610; `captures.ts` additive adoption, target frozen at open | `/tmp/inspector-flicker/REPORT.md`; `scripts/browser-check/test/api-request-flicker.mjs`; lesson: worktrees share one stash stack — no `git stash` in agent worktrees |
| M16-T36 | Phone dark 2,000-message re-entry meets its budget | done | perf-streaming-3 (milestone 3) | merged `9b0ee04` (`0e76a9e`); cause: dismissed sheet overlay kept hit-testing through its fade; 20 pairs 390 dark motion-on typed 282.7/283.6 (was 316.2/337.7), typable 70.9/75.4; UI 1605; matrices mouse/touch/touch+reduced | orchestrator applied the same rule to `ui/dialog.tsx` (`d09aab6`) |
| M16-T37 | The release orchestrator resumes after the tag | done | release-resume `01a09c59-39b2-750e-83c5-65625713b3f3` | merged (`34d8c7e`); `pnpm test:release` 45 pass; pre-push exactness unchanged, post-tag tolerance proven from tag ancestry, interrupt guard records the stage and releases the lock | `/tmp/release-resume/REPORT.md`; 0.6.1 stopped because the process was killed inside the workflow wait (no lastError, live lock) |
| M16-T38 | Split the log store and the release orchestrator | todo | — | — | D-246 B2 |
| M16-T39 | One named selector family for the current session | todo | — | — | D-246 M5 |
| M16-T40 | The release orchestrator can abandon an untagged candidate | todo | — | — | first 0.6.2 candidate `287a1b6` failed CI (a burst test over the 5 s default on the runner); no abandon path existed, the checkpoint was archived by hand as `v0.6.2.abandoned-287a1b6-ci-failed.json`; the 0.6.1 stale lock was cleared by finishing that release through the M16-T37 resume |
| M16-T41 | Release 0.6.2 | done | orchestrator | v0.6.2 public: candidate `a000b2e` (source = candidate); source CI 34784203470; release run 34784534493; 12 assets; checkpoint `verified`; https://github.com/youssefsiam38/laser/releases/tag/v0.6.2 | gates on the final bytes: `pnpm verify`, app matrix run-vo1vfQ 4/4, inspector flicker run-aZEM51, packaged clean-machine all claims held (634M unpacked); first candidate `287a1b6` failed CI on a test timeout only |
| M16-T42 | Reading upwards keeps the reader's place when a page arrives | done | orchestrator | `e7a2b0a`; unit red/green; paced browser trace 625→5859 at page arrival then two checkpoints per notch | see notes |
| M16-T43 | Prose after a formula does not crash the window | done | orchestrator | `2f02ea0`; reproduced from the person's own session (reasoning text after a message with `$x$`), `markdown-math.test.tsx` red/green | `useState(katex)` called the plugin as a lazy initializer (0.6.2 M16-T31) |
| M16-T44 | Review batch: worker | done | review-worker `01a09e4e-588b-750c-a4b6-2d6208c765e5` | merged (`790141b`); all 13 findings real and fixed red-first; unhandled-rejection guard `process-guards.ts`; `rekeySessionState` owns the fork list; async keybindings lock (3 s stall → <30 ms); worker 882 (runtime-env npx test environmental) | `/tmp/review-worker/REPORT.md`; follow-up: forking a *parent* session leaves children's `parentPath`/`rootPath` on the old path (M16-T52 todo) |
| M16-T45 | Review batch: host | done | review-host `01a09e4e-9637-750c-a4b6-2d6951e1d5e6` | merged `0f2a8a8` (`3cefe57`); all 11 findings real and fixed with red tests; host 313 | `/tmp/review-host/REPORT.md`; lesson repeated: `git stash` is repository-wide — workers told to prove red by file copy |
| M16-T46 | Review batch: relay, crypto, protocol, companion | done | review-edge `01a09e4e-e4e9-750c-a4b6-2d6e4ddb2516` | merged `d11409d` (`c4694f0`); #3 relay backpressure 1 MiB cap, #26 grant expiry, #27 free slot, #28 echoed subprotocol, #7 kill on quit only, #39 refresh queues, #40 confirmed and fixed (redirect manual), #41 guarded hooks, #42 schema tightened + real-capture proof; relay 20, crypto 43, protocol 144, pi-extension 127 | orchestrator wrapped the CLI expired-pairing error (`packages/cli/src/commands/relay.ts`) |
| M16-T47 | Review batch: UI runtime, dialogs, PWA | done | review-ui-runtime `01a09e4f-315d-750c-a4b6-2d740ffab900` | merged (`df6e478`); #4 #18 #19 #43 #44 #46 #47 #48 fixed red-first, #22 proven not a bug (launcher coalesces creates) with a guard test; UI 1639; matrix run-v9LQ8l 4/4 | `/tmp/review-ui-runtime/REPORT.md` |
| M16-T48 | Review batch: UI components | done | review-ui-components `01a09e4f-7ef3-750c-a4b6-2d7a9e57a67e` | merged (`501878e`); all six fixed red-first incl. `tool-fallback.aui.tsx`; `Hint`/`ControlHint` replace native titles; UI 1658; matrices run-ndMk0W, run-PHJ6Ge, run-FBmlkc, run-4Rwz3Q | `/tmp/review-ui-components/REPORT.md` |
| M16-T49 | Review batch: desktop | done | review-desktop `01a09e4f-c963-750c-a4b6-2d8189505e43` | merged `6ec183b` (`32717a2`); #20 macOS `open -t`, Windows txtfile handler/Notepad, capability flag hides the button elsewhere; #21 failed relaunch restarts the host; desktop 152 | `/tmp/review-desktop/REPORT.md`; macOS/Windows branches pinned by stubbed tests only |
| M16-T51 | A thinking or model change never locks the composer | done | orchestrator | `8cbe4a6`; worker `settingsLane` + test "applies a thinking change at once …"; UI preparation gate scoped by session + test | reported by the person: changing thinking disabled every session's composer — the change waited behind a queued steer's lease for the whole running turn, and the preparation counter was shared across sessions |
| M16-T52 | Forking a parent re-keys its children | todo | — | — | from M16-T44: `record.parentPath`/`rootPath` and `run.parent.sessionPath` of other entries still point at the old path after a parent fork |
| M16-T53 | Settings changes keep their order against session moves | todo | — | — | D-247 M1/M2: per-session settings tail so a model change captured before a fork/navigate cannot report success on a torn-down session; `feature/set` refusal names the project and the child waiting |
| M16-T54 | Scrolling up is steady in 0.6.3 | done | orchestrator | v0.6.4 public: candidate `c4ee554` from source `bc78bd7`; CI 34835303688; release 34835834970; 12 assets; https://github.com/youssefsiam38/laser/releases/tag/v0.6.4 | see notes |
| M16-T55 | A new stream returns to latest | done | orchestrator | UI 1,668 passed/1 skipped; browser `/tmp/browser-check/run-mBMlcX` | see notes |
| M16-T56 | Projects own their Bash setup | done | orchestrator | host 316/316; browser `/tmp/browser-check/run-xhSf47` | see notes |
| M16-T57 | Compaction ends visibly | done | orchestrator | worker/UI regressions; browser `/tmp/browser-check/run-p3dBP2` | see notes |
...
| M16-T25 | The best MCP client there is | in-progress | recover-mcp-integration `01a09a1d-a301-70e7-a766-c2337bb21411` | milestones1–2 merged c65d3b1/7ea56ee; milestone3 reviewed fixes a9a622b merged2efc1e0; durable MCP-FIXES.md: private final verify worker814/protocol143/UI1527, fresh22-file patch, packaged20 executable files+worker modules, clean-machine and16 browser cases pass; coordinator MCP204 pass + identity + patch hash verified | Full review MCP-REVIEW.md: B1 blocking established-record exclusive-lock contention; 183 MCP tests pass independently. One batch to same owner: read-first establishment + firstuse/held-revoke tests (retain coalescer unless cold race proven safe), canonical partition helpers, SDK capability assertion, typed control diagnostics, truthful recovery copy/empty-search consistency. F4 migration memoization deferred; F6 optional local typing. Resumed fix completed; B1/F2/F3/F5/F7/F8 verified, F4/F6 deferred as documented. Merge preserves unrelated project-command716319a. Parent frozen install passed; full verify FAILED concurrent-first-open MCP regression (fixture not available; worker820 pass/1fail). Three worker-only diagnostic runs did not reproduce cooldown; one separate inspector needs-auth refusal observed, cause unknown. Same owner approved one instrumented full pnpm verify and at most one recursive-test repeat (original cross-package workload), plus distinct deterministic deadline/lock/generation tests. Preserve three-file diagnostic WIP and durable MCP-INTEGRATION-FIX.md; no timeout change, flake label or retry-to-green. Existing feature review complete, integration blocker remains. Runtime recovery: live fleet was empty; same MCP session resumed run_3b8a9bf9. Coordinator and resumed MCP owner both confirm bash refused before execution by project environment command exit1. Unblocks via Settings → Projects → Environment test/refresh. No experiment started, no source/settings changes, old worktree processes cannot yet be verified. C owner unreachable through current runtime; preserve207c57e and harness WIP (C-REPORT.md); defer replacement until environment works and stale writers checked. Recovery checkpoint: person disabled hook and restarted; coordinator bash now succeeds, HEAD0ae1e0f, zero processes under either preserved worktree. C207c57e plus14 modified/2 new files and MCP0149e23 plus3 diagnostic files preserved; Old sessions unavailable after restart; ownership transferred to recover-mcp-integration run_2710630b (private0149e23,3 diagnostic files; bounded recursive-load diagnosis) and recover-transcript-acceptance run_9a1ac615 (private207c57e+WIP; acceptance/local fixes). Both admitted running. Backups C-RESTART-WIP.patch (d23b65f) and MCP-RESTART-WIP.patch (57f87fa) plus2new C files in durable recovery directory. Explicit private-path writes only; coordinator owns planning/integration. No Settings bypass. Earlier environment blocker resolved; product gates remain outstanding. Earlier inspector residual remains unexplained. Bounded representative verify+recursive tests both pass821/4skip;49 focused probes pass but historical causes remain UNKNOWN (MCP-INTEGRATION-FIX.md). Five-file diagnostic WIP backed up31043b0; no semantic fix. New scoped integrity milestone run_2ad0fc21: prove/fix establish/write treating unavailable reads as missing/corrupt and rotating valid epochs; approved distinct read outcomes, no mutation on unavailable, all serve/forward fences unchanged, no timeout/retry/reconnect expansion. Nine red integrity regressions reproduced; per-record fix focusedMCP227/2skip, registry/cache68, tsc/identity/diff pass. D-234 sorted-lock multi-identity preflight implemented,6 transaction red regressions, final MCP240/2skip+tsc/stagedidentity pass; clean b5e83a2 (7files,+612/-62 incl diagnostics), no rollback/disk-atomicity promise. Coordinator inspected; independent integrity review review-mcp-integrity `01a09a3c-cca6-70e7-a766-c24aee2acb9e` run_1251f524 targets0149e23..b5e83a2, report MCP-INTEGRITY-REVIEW.md. Review changes requested M1 advisory-read fatality;81 independent tests/typecheck pass. Same owner run_e1e71184 gets one batch: allow advisory unavailable into single locked preflight/coalescer, session regression; N1 EOF-bounded short reads; narrowed types/explicit mutation modes; lock-release failure tests, honest unfenced-account naming; remove forensic instrumentation preserving observable regressions. D-235 clarifies authority; fixes696cede addressM1/N1–N9,249 normal MCP tests pass, diagnostics removed. Coordinator diff+104 focused tests/typecheck pass; merged a0dc6c4, main normal pnpm verify taskt-5640d819 PASSED: worker866/4skip, UI1527/1skip, host285, release42/browserharness9; MCP-INTEGRITY-MERGED-VERIFY.log. MCP5 plan MCP5-PLAN.md accepted with D-237 refinements; same owner run_f94ca797 backend restricted QuickJS/broker/schema/permission seams approved. Exact0.31.0 core+release-sync, bundled cross-platform/no download, existing permission defaults and successful call envelopes preserved; typed failures throw, explicit run grants. UI approval files remain C-owned; full MCP5 waits dependent presentation and packaged/security gates. MCP5 run_f94ca797 failed on provider cybersecurity restriction. Coordinator verified private tree clean atc2ca190 (parent merge only), no MCP5 source changes/report and zero private processes. Plan preserved. Person authorized continuation; resumed same owner run_d8d397e1 with the same approved D-237 scope described as ordinary product engineering (provider suggested rephrasing). No model swap, restriction bypass or scope change; a further block means checkpoint and report the exact step. Backend landed privately36c05e2/9c26bac/5b089fe (19files,+2116/-37): QuickJS-on-Wasm guest with JSON-only helpers, per-call policy evaluation and typed mcpApproval payload/UiBridge entry, typed failure rejections keeping success envelopes, structural schema bounds. Worker895/4skip, MCP278/2skip, protocol143, desktop packaging15, tsc/identity pass; MCP5-REPORT.md. Exercised linux-x64 from source only; arm64/packaged/Windows-macOS unproven, approval UI absent, browser/verify/security review pending. D-239 declines the SDK validation rewrite. Dedicated review MCP5-REVIEW.md reproduced every number and found interpreter boundary, dispatch-once, per-call policy, approved-bytes, cancellation/replay fencing, result shapes and schema bounds MET; changes requested on4 blockers. Same owner run_e48987f9 batch: B1 ceilings-vs-defaults in one shared limits module so120s is reachable, B2 bounded marked argument excerpt in the approval title until the renderer lands, B3 delete the unvalidated ticket and correct messages.ts:376, B4 charge return/error/refused emits against the transfer budget and check guest length before extraction, plus N7 binding grants to schema revision. Unchanged by decision: permissive defaults, refuse-not-queue concurrency, D-239. Corrections537a4c9 (c2ca190..537a4c9,4commits/19files): shared mcp-script-limits.mjs with DEFAULTS vs CEILINGS so a requested deadline reaches120s and the parent backstop is tested, marked500-character argument excerpt in the approval title with full JSON on the payload, ticket deleted and messages.ts fence comment corrected, emit/return/failure bytes charged with guest length checked before extraction, grants bound to schemaRevision; over-promising wording fixed in docs/skill/tool description. Coordinator verified: clean tree, focused35 pass, limits/budget diff inspected; worker reported MCP285/2skip, typecheck, packaging15, staged identity. D-240 holds this on the branch, unmerged, until the approval renderer and packaged/arm64 proof. One review cycle complete. Transcript owner remains working. Next MCP5-REPORT.md only after authorized resumption. C owner told to avoid controlled timings during gate. One review cycle complete, no second review; historical causes still UNKNOWN. Worker handoff MCP-REGISTRY-INTEGRITY.md. Milestone4 dropped; milestone5 outstanding |
| M16-T24 | Click to conversation, accounted for | done | worker session-open-forensics | merged `d9b7b7a` + corrections `7942fb5`; review `/tmp/review-forensics.md` SOUND WITH CAVEATS: engine file open ~4.5 ms, findings 1–2 were stage B's own win measured pre-B, only worker readiness (562 ms) was new | the person: opening a session from local files still takes far too long; find out where the time actually goes |
| M16-T23 | One browser-check harness every agent uses | done | worker browser-harness | merged `3b49071` (`fdd8247`); target-neutral engine + `targets/app.mjs` example, 8 fixtures, `--matrix`, artifacts outside the checkout, tests in verify; two real matrix runs | every worker rebuilds its own; recurring flakiness (theme not applied, pointer timeouts, `spawn sh ENOENT`, artifacts landing in the repo) |
| M16-T23 | One browser-check harness every agent uses | done | worker browser-harness | merged `3b49071` (`fdd8247`); target-neutral engine + `targets/app.mjs` example, 8 fixtures, `--matrix`, artifacts outside the checkout, tests in verify; two real matrix runs | every worker rebuilds its own; recurring theme/pointer/PATH flakiness and artifacts landing in the repo |
| M16-T20 | Wide audit, host and worker lane | done | worker perf-host-worker | merged `e0d4bf3`; F01 1,025→52 ms at 32 MiB, F02 40,000→8 realpath calls; F21 rejected with evidence | F01/F02/F08/F09/F10/F11/F20/F21 |
| M16-T21 | Wide audit, renderer lane | done | worker perf-renderer | merged `c19da05`; 32→1 store publications, shell/fleet renders →0, map 2,000→14 geometry reads; F17 needs a host method, F13 math deferral rejected | F04/F05/F13/F14/F15/F17/F23/F24; F06 belongs to M16-T16 |
| M16-T22 | Wide audit, protocol/delivery/startup lane | done | worker perf-delivery | merged `6d42ec0`; F03/F22/F07/F16/F18/F12 done, F19 profiled and rejected | F03/F07/F12/F16/F18/F19/F22 |
| M16-T19 | Open any file on the machine | done | worker open-any-file | merged `42e3151` (`766e94b`); paired-device policy written into `docs/security.md` | `files.ts` read refuses outside `cwd`; the person: Laser must open any file on the machine |
...
> Implement native model fallback chains in Laser, including bounded attempts to recover earlier models when the active fallback fails. Discover the relevant code yourself and follow the repository's AGENTS.md, architecture, UI, and testing requirements. Build the complete feature: settings, runtime behavior, filesystem persistence, and verification.
>
> **Settings.** Add a fallback-chain setting, initially empty. Do not create default chains. Provide an **Add fallback chain** action. Users select an ordered list of models from connected providers. The first model activates the chain; subsequent models are its fallbacks. Allow any number of fallback models, including mixed providers. Support adding, removing, reordering, editing, and deleting. Allow only one chain per starting model. Prevent duplicate models within a chain. A model may appear in multiple chains and have its own separate chain.
>
> **Chain selection.** A chain applies only when its first model is the session's selected model. Given ChatGPT → DeepSeek → OpenRouter and DeepSeek → Gemini: a session starting with ChatGPT uses only the first chain. Switching to DeepSeek does not activate DeepSeek's separate chain. A session starting with DeepSeek uses the second chain. Never merge or recursively enter chains.
>
> **Existing retries and fallback triggers.** Preserve the existing robust retry mechanism. The active model gets its normal retries before fallback is considered. Fallback applies only when a model-access failure would otherwise stop the agent after those retries finish. Examples include provider downtime, connection failures, exhausted credits, and exhausted subscription allowance. Do not trigger fallback for user cancellation, tool failures, ordinary task errors, safety refusals, or successful completion. Prefer structured failure information; avoid broad matching that misclassifies unrelated errors. Each newly activated fallback model retains the normal retry policy. The bounded return attempts described below are the explicit exception.
>
> **Returning to earlier models.** When an active fallback exhausts its normal retries: 1. Check eligible earlier models in the original chain's priority order. 2. Make at most one bounded model request to each eligible earlier model during this failover event. Do not restart its full retry cycle. 3. If an earlier model successfully resumes work, select it and continue. 4. Otherwise, advance to the next remaining fallback. Example: ChatGPT exhausts retries → DeepSeek takes over. DeepSeek later exhausts retries → attempt ChatGPT once, if eligible. If ChatGPT succeeds, continue with ChatGPT. If ChatGPT fails or is ineligible, continue with OpenRouter. Return attempts must perform the pending agent work through the normal request path. Do not send a separate synthetic health-check prompt or discard a successful response. Safeguards: apply a finite cooldown to recently failed models across switches and persist it (choose and document a conservative default consistent with existing retry conventions); respect known provider retry times and allowance reset times; do not retry exhausted-credit or other clearly non-transient failures automatically within the current chain activation unless there is evidence the condition changed (manual model selection can start a fresh activation); treat unknown recovery times explicitly — do not invent reset timestamps or permanently blacklist transient failures; a reachable endpoint alone is not recovery — require successful completion of a normal model response, including a valid tool-call response; bound every failover event: each candidate can be attempted at most once, with finite request timeouts; do not allow successful switching alone to reset failure history and create a rapid loop between models. Track the active position, candidate attempts, and failure eligibility explicitly. Restoring an earlier model must not lose the remaining chain or accidentally enable repeated traversal.
>
> **Task continuity.** Continue the same session and task with conversation context, tool results, and agent state intact. Do not replay completed tools, duplicate messages, lose accepted work, or issue concurrent model requests while switching. Handle partial responses and cross-provider context/capability differences through the existing architecture. If a candidate cannot be used, record a clear reason and proceed safely. If no eligible candidate remains, preserve the task and present an actionable error. Do not wait indefinitely or schedule recovery work. Apply the behavior consistently wherever agents execute model calls, including child agents where applicable.
>
> **Persistence and manual selection.** Persist through the established filesystem-backed session storage: the selected model; the active chain snapshot and traversal state; failure classifications, cooldowns, and known retry/reset times. Reloads and restarts must preserve chain identity and eligibility. They must not resolve a new chain from the replacement model, replay completed work, or bypass cooldowns. Saved chain edits affect future activations. Existing sessions retain their active chain snapshot. A manual model change starts a fresh activation using the chain whose first model matches the newly selected model, if one exists. Fence stale requests and pending failover transitions so they cannot override the user's selection. Sessions without a matching chain retain their existing behavior.
>
> **No background recovery.** Do not implement background probes, recovery jobs, periodic retries, or automatic return while the current model is working. An earlier model is reconsidered only when the active fallback fails. A successful replacement remains selected until the user changes it or a later failure triggers this policy.
>
> **User experience.** Clearly explain that only the first model activates a chain. Explain that earlier models may be retried when a fallback fails, subject to cooldowns and failure eligibility. Show concise switching status and the model that becomes active. Keep the model selector accurate. Preserve a useful record of transitions without exposing credentials or raw provider errors. Follow project theme tokens, accessibility requirements, and assistant-ui guidance.
>
> **Verification.** Add meaningful tests covering: empty settings, unmatched models, chain editing, validation, and persistence; existing retries finishing before fallback; eligible failures versus cancellation, tool failures, and other excluded conditions; cross-provider continuation without duplicated tools, messages, or lost context; independent chains and non-recursive traversal; earlier-model recovery succeeding, failing, and being skipped; priority order when several earlier models are eligible; bounded return attempts without full retry cycles; cooldowns, known reset times, and non-transient failures; repeated failures without cycling or unbounded requests; complete chain exhaustion; reload/restart preserving active selection, traversal, and eligibility; manual selection resetting activation and winning races against stale fallback work; no background checks or return attempts while the active model works. Run the relevant checks. Inspect the settings and fallback experience in the browser at desktop and phone widths, in both themes. Update required planning/status records and report implementation results, verification, and any remaining limitations. Proceed with implementation. Ask only about a consequential ambiguity that cannot be resolved from these requirements or the repository.

#### M15-T3 notes (live)
- 2026-09-12 live pass: with the stub answering 429 + stated reset past a two-retry budget, the session switched to the fallback (status line, `chain 2/3` badge, one transition record), continued with tools, returned to the original once it answered, exhausted with the attention record and no background requests for 60 s, survived F5, and the person's manual choice won during a switch. Findings: the stopped-run explanation collapsed to 5 px beside the reason pill (fixed with a flex basis), chain controls were 28 px on touch (`pointer-coarse:size-11`), the intro copy implied a fallback switch starts the replacement's chain (reworded), and the add-only picker showed the first model as if chosen (empty value → placeholder).

#### M15-T3 notes (phase 2)
- 2026-09-11 design approved at `d0f5fe7` (`docs/model-fallback-chains.md`, 773 lines; plan `/tmp/fallback-chains-plan.md`). Decisions: `@earendil-works/pi-ai@0.85.0` becomes a direct exact-pinned worker dependency so the continuation reuses the engine's `isRetryableAssistantError`; chains are global-scope only; the driver defers `agent_settled` while a failover is in flight; no project-scoped chains; transition records are not collapsed. Mechanism: `session.setModel()` → drop the trailing error assistant from agent state (the engine's own idiom, `agent-session.js:1856`/`:2300`) → `agent.continue()`; trigger from `agent_end { willRetry: false }` + the companion's forwarded provider response status/headers; twelve-class failure table, `unknown` never switches; 5-minute default cooldown; one append-only session entry for persistence.
- 2026-09-11 reviewer runtime (Codex) hit its usage limit on three reviews at once; independent reviews now run as read-only worker sessions distinct from the authors.
- 2026-09-11 hygiene fix landed on the engine branch at `2905a78`: the true cause was provider credentials the engine reads straight from the environment (`OPENAI_API_KEY` in a Laser-spawned shell) on top of the two directory variables; `test/engine-env.setup.ts` in worker/pi-extension/host strips `PI_*`, `*_API_KEY`/`*_AUTH_TOKEN`/`*_OAUTH_TOKEN` and twelve named credential variables before any test loads, and the worker pins both engine directories from its flags. Orchestrator re-ran `stable-sdk.agent.test.ts` in this shell: 17/17 in 2 s (was 1 failed after 33 s of real provider calls).
- 2026-09-11 test-hygiene incident found while confirming a pre-existing failure: a Laser-spawned shell carries `PI_CODING_AGENT_DIR`/`PI_CODING_AGENT_SESSION_DIR` pointing at the person's real agent dir, and the pinned engine resolves parts of a session from that variable rather than the driver's `agentDir`; `stable-sdk.agent.test.ts > keeps no-signal candidate…` then reaches the real default model (`openai/gpt-5.5`) with real credentials and fails on `main` (also at `576ee66`) while CI, with the variables unset, passes. Fix assigned to the worker package owner (M14-T2): vitest `setupFiles` stripping both variables in worker/pi-extension/host, and `main.ts` pinning the variable to `--agent-dir`.

#### M15-T4 notes
- 2026-09-11 ready at `87c675f` (T4) and `0bd7be9` (T5); UI 1125 green; screenshots `/tmp/sidebar-indicators/`, `/tmp/slash-completion/`. T5 root cause: assistant-ui's trigger keyboard resource treats Tab like Enter (select the highlighted item); the `/` picker's action items execute on selection with `removeOnExecute`, so `/compa` + Tab stripped the draft and ran `actions.compact()` — the fix intercepts Tab with a new `onComplete` that writes the completed token and never selects. T4 choice: one leading slot for every state; folded-branch mark is `attention` only (the orchestrator is asking for `danger` back for a hidden failure).
- 2026-09-11 review REQUEST CHANGES: a product-name literal in a test title fails `identity:check` (the worker's "clean" claim was wrong — the `-F ui test` filter does not run the gate); a failed-and-unread child showed the unread mark instead of the failure; `danger` restored as a static folded-branch mark (D-186, DESIGN.md); modifier+Tab still selected-and-ran; caret not restored after completion. Fix batch assigned to the same owner.
- 2026-09-11 environment hazard reported by the worker: a worktree whose root `node_modules` is a symlink into the main checkout rewrites the main checkout's root workspace links on `pnpm install`; the orchestrator restored the links and told the sharing workers to give their worktree its own root before any install.
- 2026-09-11 from `image.png`: "Integrate Meridian into Lazer" shows the primary pre-name dot and a trailing spinner (duplicate); its child "meridian-audit" shows two dots (one on the tree connector, one before the name); the parent's dot runs because the child runs. Wanted: one indicator per row, before the name, running only when that session's own agent is running.
...
#### M16-T17 notes
- 2026-09-12 **moved off main at the person's instruction**, after the work was wiped a second time: `88f714c` ("revert: drop in-flight project-environment sources committed by mistake") removed from the working tree the files a previous lane had committed by mistake, so 14 of 15 integration edits vanished again. Recovered from the stash patch and now lives on `agents/m16-t17-project-env` in a worktree, committed at `2794790`. Main keeps only this ledger note. Do not re-apply the sources to main; merge the branch.
- 2026-09-12 migration done and verified. 21 files, 374 variables into two Infisical projects (`kwentra`, folder per repository; `personal`, folder per repository plus `/_shared`). `kwentrakit` and `janan_tech/dhakira` excluded at the person's instruction: archived projects, and neither was plain dotenv. Verification compares by digest and reported **everything matches**. Two defects the verifier caught: the store refuses an empty value and refuses the whole batch with it (now reported by name and left in the file), and `ACCOMODATION_TOKEN` was a quoted single space that the store trims to empty, so whitespace-only is now treated as empty too.
- 2026-09-12 switchover done: the `BASH_ENV` line and the `context.bash` source removed from `.bashrc` (backup `/tmp/workenv2/bashrc.before-switchover`); `.kwentra-local` and every `project.env` left on disk, unreferenced. The tool-identity variables that its `cd` hooks used to set (`KUBECONFIG`, `CLAUDE_CONFIG_DIR`, `CODEX_HOME`, `GIT_CONFIG_GLOBAL`, …) are now the `kwentra` profile's non-secret layer, so nothing regressed. Proven end to end with real credentials: a fresh terminal loads `personal`; `workenv use` in `hms` gives 76 variables and removes the personal ones; switching back removes every Kwentra name.
...
- 2026-09-12 implemented. Protocol: `packages/protocol/src/project-env.ts` (contract, bounds, validation, fingerprint) + four methods `pi/project/env/{status,set,test,refresh}` and `pi/project/env/changed`, schemas and inventory samples. Host: `packages/host/src/project-env.ts` (machine-local store keyed by canonical project root, `0600`, approval fingerprint recorded on save) and `projectEnvForWorker` in `server.ts` gating on **trust** then **approval** before the non-secret config reaches `ENV.projectEnv`; `projectRootOf` first, so a worktree inherits its owning project's binding. Worker: `packages/worker/src/project-env.ts` holds values in memory and decorates spawn environments through the engine's `spawnHook`; `process.env` is never written, which is what keeps a project from repointing the engine's own model auth. Extension: `ProjectEnvironmentBridge` in `agents-bridge.ts`, applied in `background-work` for foreground, promoted and background commands, with a refusal before execution when a required hook has not resolved. UI: Settings → Environment (`packages/ui/src/components/settings/EnvironmentTab.tsx`) with Save/Test/Refresh, names-only status.
- 2026-09-12 evidence: protocol 122 pass (15 new), worker `project-env` 14 pass + `project-env-live` 3 pass, host `project-env` 9 pass; `pnpm identity:check` green; protocol/pi-extension/worker/host/ui all build. Pre-existing unrelated failures on `main` at the time (confirmed by stashing every change of this task and re-running): `test/runtime-env.test.ts > runs real npm and npx…` and `test/server.test.ts > dispatches project reads…` — both owned by other in-flight lanes.
- 2026-09-12 defect found and fixed during testing: killing a hook on timeout waited on `close`, which a grandchild holding fd 3 can hold open indefinitely (a `sleep` under `bash` kept the project waiting). Now spawns in its own process group, kills the group, and settles on `exit`.
- 2026-09-12 hazard confirmed on this machine, and the reason provider keys are refused by default: `~/projects/kwentra/*/project.env` set `ANTHROPIC_API_KEY` with **two distinct values** across repositories. Per-project opt-in exists for a project that really wants one.
- 2026-09-12 `docs/project-environment.md` written. Not done in this session: the browser pass over the new Settings tab at desktop and phone widths in both themes (the repository requires it before the task is marked done), and MCP stdio servers still receive the worker's environment rather than the project's — recorded as a limitation, not an oversight.
- 2026-09-12 MCP coverage added on the person's approval: a stdio server is started by the engine, not by the shell tool, so it never passes the spawn hook. `toAdapterConfig` now takes an optional decorator (`packages/worker/src/mcp/adapter-config.ts`), threaded through `mcpSessionConfig`/`mcpSessionSetup` and `commonOpen()` so every session of the project is covered. Three rules, each with a test: `inheritEnv: false` is left untouched; an inheriting server has its environment taken over in full (an `env` map can add a name but cannot remove one, and removing an inherited credential is half the point); the server's own `env` wins. HTTP/socket servers start no process and are untouched. `packages/worker/test/mcp-project-env.test.ts`, 5 tests; existing MCP suites still 103 pass.
- 2026-09-12 live proof on a real host (own port 41999, own state dir, the person's running host on 41441 untouched): two projects, each with its own hook, configured through the real protocol. `pi/project/env/test` returned `ready` with names only and **no values anywhere in the payload**. `/proc/<pid>/environ` of the two spawned workers showed each carrying only its own project's `LASERCODE_PROJECT_ENV`, `SHARED_NAME` absent from both (values stay in memory), and the launching terminal's `INHERITED_TOKEN` present at worker level — which the project's `unset` removes at spawn.
- 2026-09-12 browser pass done at 1440×900 and 390×844, dark and light: tab renders, live status truthful, no overflow at phone width, failure state shows "The command exited with code 3." and **does not leak the hook's stderr**, which in the test deliberately contained a password. Two defects found and fixed: the font drew `--profile` as an em dash in the argument placeholder (ligatures disabled on both inputs), and the failure state said what broke but not what it meant — it now says whether commands are refused, and to run the program in a terminal because its output is deliberately never shown.
...
| M18-T3 | RP-3 Advanced resource diagnostics | done | orchestrator-01a0a030 + resource-diagnostics/renderer-pressure owners | `a0111f9`, `66ac605`, `4ba58bd4`, pressure subsection `1b5fbfa7`, ceilings `4f15add0`; final matrix `/tmp/m18-t3-closeout/run-GoZ2AA` 4/4 zero survivors; UI settings/pressure 759 | see notes; depends on M18-T1 |
...
| M18-T8 | RP-8 memory-pressure policy and safety ceilings | done | orchestrator + memory-pressure-policy/host-pressure-controller/renderer-pressure-plan/heap-ceilings-plan owners | `ce455b99` (C `a4a364c2`, D `87e65720`, E1 `d8ec6cae`, E2 `55350823`, E3 `49cc12d9`, F `1b5fbfa7`, G1/G2 `4f15add0`, G3 `ce455b99`); host 842, worker 1028/4 skipped, protocol 317, CLI 82, desktop 171/1 skipped, browser-check 97; packaged gate 17/17; containment probe pass; exact-SHA quick soak nine scenarios | see notes; depends on M18-T4/M18-T5 |
...
| M18-T14 | RP-14 Electron/Tauri decision gate | dropped | — | — | dropped by D-268 |
| M18-T15 | RP-2 post-containment repeat baseline | done | soak-repeat-baseline `01a0aa7f-a32f-714d-99c8-18eecb4786d1` + orchestrator | `7bfce5e8`; `/tmp/m18-t15-final-full` A/B pass, comparison pass; `docs/resource-soak-baseline.md`; browser-check 104 | see notes; created by D-257 |
...
| M18-T19 | Consume environment capabilities in client affordances | done | capability-affordances `01a0aa10-9cff-714d-99c8-188e4f0fa006` | `76f475fa`; UI 2260/1 skipped; narrowed-policy matrices pointer + touch/reduced motion | added by D-264; depends on M18-T13 |
...
- 2026-09-14 Electron lane checkpoint: same main/host generation, all expected renderer/main/GPU/utility/host/worker roles and zero survivors are proven. A new narrow CDP shortcut helper/test passed, but its single lane attempt stopped before dispatch because it incorrectly required a textarea to take OS focus; captured state was visible main thread with BODY active and no survivors.
- 2026-09-14 correction verified: BODY-active main-thread CDP shortcut now proves visible/focused search DOM; the Electron lane passes same main/host generations and zero-survivor teardown. Slow-consumer pacing now stops at the first real server queue, and inspector close is awaited.
- 2026-09-14 quick-run checkpoint: three cleanly torn-down attempts advanced past Electron and slow-consumer checks but stopped at natural worker retirement. The cause is localized to a worker-inspector discovery race: a connected debugger client is leaked when `queryInstances` runs before `WorkerServer` exists; debugger-wait lines match the three workers that fail retirement. No owned survivors remain.
- 2026-09-14 inspector-race correction passed 12/12 focused tests but the single authorized quick run still timed out at worker retirement with zero survivors. Raw attach/end log counts are inconclusive because registrations can duplicate one endpoint and Node may log only final disconnect; no further run is authorized until the actual retirement guard is identified.
...
- 2026-09-14 done at `ad8c188` after reviewed correction and comparator fix: browser-check 67/67, build/release/identity/diff/redaction gates pass; `pnpm verify` only has the known unrelated bundled-layout npx failure. Corrected full A completed scenarios 1/2/3/6 with 18/18 complete coverage phases, then refused B before large tool heap capture at renderer PSS 1,977,168,896/private 1,940,013,056 B against the unchanged 1,610,612,736 B ceiling; zero survivors and raw heaps. `docs/resource-soak-finding.md` attributes unbounded renderer view retention to T5/T8. D-257 preserves all nine unchanged scenarios and category/slope repeatability in T15 after containment.
...
#### M18-T14 notes
- 2026-09-16 dropped by D-268 at the person's direction: no Electron/Tauri comparison or migration decision; `/tmp/m18-t14-plan.md` is retained as history only.
...
#### M18-T16 notes
- 2026-09-15 created by D-258 after three no-heap-snapshot full fixture runs proved one current/running view retained a 3.15 MiB assistant block, rendered 254k DOM nodes and crossed the fixed renderer ceiling at 1.97–2.67 GiB PSS. Existing T5 LRU correctly bounds unpinned views but its approved pin exemption makes the per-view limit only an eviction trigger. T5 remains done; T16 blocks T8's renderer gate and T15.
- 2026-09-15 plan v2 accepted at `/tmp/m18-pinned-view-plan.md`: every settled hydrated view hard-bounded to 1,572,864 B and total to 4 MiB; all live/final text/reasoning/tool/image paths fold to bounded excerpts/references; revision-fenced authoritative ranges are ≤64 KiB and authorize before read without spawning inactive workers; UI paging remains bounded and unchanged scenarios 4–5 must stay below renderer warning/ceiling. Implementation waits for T10 integration; its additive DTO-marker compatibility question was sent to T10.

- 2026-09-15 first RP-5b candidate `c20d017b` plus soak correction `3a123a4e` contains scenarios 4–5 below renderer ceilings with 12 images and zero survivors, but parent source inspection withheld review/integration: exact A1/A8/A10 workloads were reduced or split; structured reducer bodies still allocate a whole JSON string before excerpting; find/PageUp/copy/anchor/focus/action acceptance is incomplete; image reconstruction retains unbounded chunk arrays, does not fence late clear or revoke on row unmount, and reports decoded surfaces as zero; range replies lack complete client validation and 32 MiB reconstruction efficiency. The same owner is correcting the full batch without changing fixtures or ceilings.
- 2026-09-15 candidate `acc265a2` restores the exact workloads and passes focused/browser gates plus one unchanged scenarios-4/5 soak, but parent inspection again withheld review: copy-all reconstructs 32 MiB in JS; range digests and Unicode offsets are not fully validated; image refcounts/held admission/wire-elided surface accounting are unsafe; several live/history/custom/tool/structured write paths bypass bounds; attachment regions are not reconstructible; cache delta accounting grows after tails are dropped; mounted anchor/focus and post-trim cursor rehydration are not proved; old-path anchors leak. Owner run `run_0686d70e` is correcting the batched findings before the one review cycle.
- 2026-09-15 correction checkpoints `a6c0fb36`, `64dedf68` and `0a640e5f` pass focused protocol/host/UI builds and tests (worker has only the known clean-base bundled-npx failure): shipped 32 MiB reads, bounded structured projection, whole-body digest verification, image reservations/races, settled entry/digest identity, memo expiry, honest row copy, ToolFallback overflow and accounted image open/copy/save are implemented. Milestone remains unaccepted and unreviewed while the same owner completes canonical attachment regions/code-copy, mounted trim preservation and guarded tail/cursor reconciliation; parent inspection also returned copy-feedback and FileOpener hold-cleanup fixes.
- 2026-09-15 checkpoints `d3ecb051` through `7772e675` add canonical authority-produced attachment regions, ≤64 KiB revision/digest-fenced reads, strict-old-authority bounded scanning, measured one-page attachment UI, generation-fenced edit headroom, honest code copy and corrected decoded-attachment sends. Protocol 254 and UI 2,109/1 skipped pass with builds/identity/direction clean; the unchanged browser matrix passes but the real attachment interaction fixture is still owed. The same owner is now implementing mounted trim preservation and two-read guarded reconciliation; no review/integration yet.
- 2026-09-15 exact-tip `4991e47e` completed unchanged quick scenarios 1–6 through scenario 5 with zero survivors at `/tmp/m18-t16-soak-exact-4991`; its report is intentionally partial (`pass:false`) because scenarios 7–9 were not requested. Independent review `/tmp/m18-t16-review.md` rejected merge: structured excerpts can cease being canonical prefixes, reconciliation can overwrite overtaking live/optimistic state, editor handback enters before admission and releases its draft reservation, and three monoliths require cohesive extraction. Parent additionally requires result `entryId` echo plus safe-integer/authority/continuity validation. Original owner resumed as `run_14c22cd1` for one batched correction; exact-tip full soak waits for that tip.
- 2026-09-16 post-review corrections `21d42c0c` and `75ed7707` supply one incremental serializer, canonical reconciliation fold, draft-lifetime accounting, required entry echoes and safe reply validation, bounded worker editor handback, and cohesive source splits. Parent real WorkerServer testing then caught the raw `editorText` surviving the bounded spread; `2fca59d3` strips it before both navigate/fork replies and proves the actual wire result.
- 2026-09-16 done and integrated at merge `42344a58` (implementation `2fca59d3`): protocol entry-range 47, UI 2123/1 skipped, host 655/656 with one unrelated five-second E2E timeout, focused real WorkerServer 39, builds/identity/direction and browser large-body/attachment matrices pass. Exact-tip unchanged full-mode scenarios 1–5 at `/tmp/m18-t16-soak-full-2fca-final` completed the 5-project/50-session, 12-image and 200-command workload with complete process coverage, no safety refusal and zero survivors; intentional `--until` makes the report partial rather than the final baseline. Ownership released to T8/T18.
...
- 2026-09-15 reviewed harness repair integrated at `cec6c07a`: strict scenario-8 load, socket-object identity, exact RP-4/RP-6 counters and unavailable-row propagation pass browser-check 96/96 and an exact-SHA scenarios-1–8 run with zero survivors. `2430622b` then corrects only the stale discovery assertion: a readable terminal `TailBuffer` count of zero proves cleanup. The unchanged quick run at `/tmp/m18-t15-soak-nine-integrated2` passes all nine scenarios on exact SHA `2430622b` with zero survivors. Final full A/B remains gated only by T8/T16.
- 2026-09-15 T16 soak triage: two of four unchanged scenarios-4/5 runs completed large content/images with 12/12 images and zero survivors, then scenario 5 refused `session/prompt` for a safely unloaded runtime. This is an RP-4/T15 action-after-retirement integration defect, not evidence against RP-5b retention; do not weaken or add loads to the fixture. After T16 releases its overlapping host router path, T15 must make the host re-establish mutation authority for a safely unloaded session and prove the unchanged 200-command workload.

- 2026-09-16 first post-containment full run A on `350d17be` (`/tmp/m18-t15-baseline/run-a/report-partial.json`, owner `01a0aa7f-a32f-714d-99c8-18eecb4786d1`, branch `agents/soak-repeat-baseline-37f0c500` `4e4057fb`): scenarios 1–6 and 8 complete, heap captures under the 256 MiB cap, zero survivors, but scenario 9 timed out because one product connection held two attached transcript paths before and after traversing all 54 retained views (expected ≤1); scenario 7 not reached; run B refused by design. The same run exposed a harness coverage hole — a Linux process row missing PSS/private-resident beside `coverage.complete: true` — now refused by `sampling.mjs` with a regression test (browser-check 98/98). Product leak assigned to fresh owner `dormant-attach-leak` `01a0aa92-d4bb-714d-99c8-19159b81c1d7`; T15 reruns the unchanged full A/B from the corrected SHA afterwards.

- 2026-09-16 attachment leak narrowed by `dormant-attach-leak` (`/tmp/m18-t15-attach-leak.md`): inspector counting is correct; the two survivors are default-owner (`view`) holds on the product connection for small Beam/Chat workspace transcripts re-attached by non-`openSession` loads (reconnect resume, recovery reopen); two stamp-patching candidates still failed the exact full run, so that branch was discarded (one diagnostic commit). Reassigned to fresh owner `attach-membership-derive` `01a0aac0-20f2-714d-99c8-192d72fb236e` with a derived design: `HostClient` observes every default-owner hold it sends (load, new/fork admission) and every release, and the provider's eager-detach effect re-runs from that held set — no per-path stamps.

- 2026-09-16 attachment leak fixed at merge `98d12881` (`e8d58c60`, review `/tmp/m18-t15-attach-review.md` F1–F12 corrected): `HostClient` observes every default-owner hold and release it sends, `transcript-membership.ts` reconciles from that held set with an exhaustive `holdsTranscript` pin policy, symmetric landing holds for new/reused/fork, one detach per hold, registered real-host membership check. At the fix SHA two consecutive complete unchanged full runs (`/tmp/m18-t15-derive-full-complete`) pass every ceiling, completion and survivor gate (9/9, 0 survivors, attached paths 1→0→1→0). `comparison.json` fails only the predeclared repeatability policy: worker Spearman 0.714 among six structurally identical ~110 KB owners, three renderer heap slopes with CV > 0.25 from 3–5 samples. T15 owner now investigates measurement validity (forced-GC sampling, tie-aware ranks at declared resolution, more post-GC samples from existing steps) with thresholds, fixtures and workloads unchanged, then reruns A/B and writes the baseline document.

- 2026-09-16 done at merge `7bfce5e8` (harness `85c6af67`, `a690b4b3`, `202ab3da`, `f719bd97`): two consecutive complete unchanged full runs at executable SHA `21a15a2a` (`/tmp/m18-t15-final-full`) pass every predeclared gate — nine scenarios each, complete Linux PSS/private coverage (strict refusal of incomplete rows), fixed ceilings, 256 MiB snapshot cap (largest 198,401,584 B), zero survivors, all eleven retained-owner categories with tie-aware ranks at `max(8 B, 1.25 %)`, five post-GC slopes (host retirement and heavy payload as equivalent nulls per D-267; pagination mixed-resolution passing the original sign/CV gate per D-269 at 188,384 vs 177,217 bytes/page, CV 4.3 %). Measurement corrections recorded: renderer slopes were previously fit from pre-instrumentation, pre-GC counters; slopes now carry SE/R²/n; `--compare-only` recomputes a pair from retained reports; the final verdict came from that recomputation under D-269 after the same pair had failed D-267's mixed-resolution clause — no rerun, no threshold change, and the original T2 gate is the one applied. Executable tree tagged `resource-baseline-m18-t15`. `docs/resource-soak-baseline.md` records both runs, gate-by-gate verdicts, the D-267/D-269 record, the T2 comparison, unsupported metrics and limitations. Browser-check 104/104, verify, identity.
...
- 2026-09-15 done and integrated at merge `890e6a86` (implementation `93104fda`): one token-safe bounded naming attempt per session, no-model intents park without pinning, actual naming remains a safety pin, and fork/rekey/close/disposal/model-removal/overlap/late-completion races are fenced. Focused worker tests 45/45, real-host E2E 4/4 and full host 636 pass. Ownership released; T15 scenario 9 passes on the integrated exact SHA.

#### M18-T18 notes
- 2026-09-15 added by D-260 after two unchanged full runs reached scenario 5 only to route `session/prompt` into a runtime a legitimate RP-4 unload had just removed. Read-only investigation confirmed a host check-then-act race across `workerFor` and `WorkerPool.unloadSession`; plan at `/tmp/m18-t15-session-reopen-plan.md`. Implementation waits for T16's overlapping host router work and must fail closed rather than route through a timed-out release gate.
- 2026-09-16 claimed: implement the approved fail-closed per-session route lease in host routing/lifetime only, preserve every saved-session and worker safety guard, prove zero writes on gate timeout/failure, then rerun the unchanged full scenario-5 workload.
- 2026-09-16 independent review `/tmp/m18-t18-review.md` rejected `55abfb12`: four live read fast paths bypassed the lease, move/file and load/fork authority bookkeeping escaped the critical section, and the failed-release E2E actually tested timeout without proving zero worker writes. One correction batch `c0754b3e` leases every live-owner decision, keeps bind/rekey inside the route, makes move exclusive over source+destination through rewrite/state transfer, and distinguishes failure from timeout with zero load/prompt writes.
- 2026-09-16 done and integrated at merge `916e30fa`: parent verified lease/router/move 31/31, real host+built-worker unload/reopen 9/9 and host typecheck; owner full host 687, protocol 264 and builds/identity/direction pass. Three consecutive unchanged exact-SHA full-mode scenarios 1–5 at `/tmp/m18-t18-soak-{1,2,3}` completed all 200 command calls with complete coverage, no error log and zero survivors. Ownership released to T8/T15.
...
- 2026-09-14 plan v2 `/tmp/m18-t4-plan.md` approved to wait behind T6: T4 consumes T6's single membership/store contracts, owns one worker session-safety predicate shared by unload and retirement, enforces hard global replay floors and performs safe idle/over-cap LRU unload before worker retirement. No duplicate attachment registry or public retained method.
- 2026-09-15 implementation started as run `run_228e23cb`: rebase on integrated T6/tracking base `f970a73`, then implement the approved shared safety predicate, safe unload/retirement and hard replay floors.
- 2026-09-15 implementation `e391ddf` plus parent safety correction `551e747` ready: shared worker predicate, fail-closed complete safety coverage for unload/stop/restart/retirement, safe idle/over-cap host policy, hard global replay floors and typed retained counters. Package/build gates pass except the known clean-base bundled-npx worker environment test; unchanged quick base/after soaks both reach scenario 8's pre-existing buffered-byte timeout, with post-fix sessions/replay reductions recorded under `/tmp/m18-t4-soak-*`.
- 2026-09-15 independent review `/tmp/m18-t4-review.md` requests changes: worker retirement has a check/kill admission race; fork leaves a phantom old pool path; pinned oldest candidates starve later safe LRU rows; reopenability trusts only in-memory identity; disabled-Namer intent is dropped; lifecycle ownership is overgrown. Existing soaks are correctly only partial base evidence, not final-head convergence evidence.
- 2026-09-15 correction design run `run_ac95f9f0` started with original owner: define worker-owned atomic retirement/admission, pool rekey, fair bounded unload, typed durable reopen preparation, preserved naming intent, extracted lifecycle owner and exact-head focused convergence evidence before source edits.
- 2026-09-15 done and integrated at merge `eb18e147` (implementation tip `8dcfbab`): worker-owned atomic release/retirement fences, fail-closed complete pin coverage, fair bounded idle/over-cap unload, canonical flush/reopen proof, host fork rekey, hard global replay floors, categorical privacy-safe refusals and retirement lease recovery. Independent review findings and parent replay/privacy/timeout corrections are closed. Parent exact-tip verification passed protocol 40, worker 44, host 46, host/worker typechecks and diff-check; `/tmp/m18-t4-convergence.json` records 50 sessions → 4 loaded, replay 0 B, bounded post-GC growth. Full unchanged two-run soak remains T15.
- Ownership ledger: worker session lifetime · completed by `01a0a1f8-57b0-76c8-926f-52a825867a5f` · stable unload/safety/replay interfaces handed to T8/T3 · integrated `eb18e147` · status done.
...
- 2026-09-15 implementation `c0e87c4` ready with 50 light identities, six-view/4 MiB/1.5 MiB hydrated bounds, pin and epoch fences, frozen tail sink, local counters, 4/4 browser matrix at `/tmp/browser-check/run-5kc75o`, and partial calibration under `/tmp/resource-soak-t5`; broad gates pass except the known clean-base bundled-npx worker environment test.
- 2026-09-15 parent corrections through `5fe6c98` ready: required opaque session identity, whole-state/external pin scheduling, bounded growth passes, non-retainable unknown decoded images, exact reasons, reducer revalidation, one post-paint/fallback tail queue bounded to 32 full DTOs/2 MiB exact UTF-8 with deterministic shedding. Final UI 1,860/1 skipped, typecheck/diff/identity/direction and 4/4 browser matrix pass. Exact release-behavior calibration at code `c724cad` is explicit partial/non-pass with zero survivors/raw heaps under `/tmp/resource-soak-t5-final`; later commits only document and correct pending DTO accounting.
- 2026-09-15 independent review `/tmp/m18-t5-review.md` requests changes: tail records can pair stale revisions with newer entries; streamed measurement rescans light records and re-encodes cumulative text instead of owner-proportional deltas/stable counters; pinned views incorrectly consume the six unpinned LRU slots and accepted reads/sends/forks do not touch recency; pending tails key by mutable path rather than opaque environment/session identity.
...
- 2026-09-14 plan rev 2 approved: one canonical bounded direct/relay membership registry and read-only T4 view; bounded terminal task records plus per-task/session/worker durable rolling logs; remote `resource/snapshot.stores` with honest live-worker coverage; exact-pinned Pi spawn callback patch/upstream; RP-1 helper/command registration; updated soak assertions without fixture/ceiling changes. Detach cancels in-flight owner generation; log ownership never deletes another live runtime.
- 2026-09-15 implementation through `180ee25` passed package/build gates and real 200-command/live-log probes; upstream guarded Pi `onSpawn` PR: https://github.com/earendil-works/pi/pull/9604. Independent review accepted the membership/race direction but found write-failure disk escape, stale ranged offsets, create-time global delivery, per-chunk scans/duplicate retention policy, inexact counters, lock integrity drift, silent scope refusal and synchronous/non-progressing sweeps. One batched correction is active before integration.
- 2026-09-15 done and integrated at `a256efe`: canonical direct/relay `SessionMembershipView`, bounded create reservations and duplicate-load settlement, compact task records, hard-bounded live tails and async durable segmented logs, retained-store coverage, safe RP-1 registrations and Pi `onSpawn`. Review corrections gate every worker spawn on host crash cleanup and reduce task-log failures to bounded categorical codes so private paths cannot cross diagnostics or client boundaries. Independent review plus final parent diff audit passed; root verification passed build/typecheck/identity/direction, pi-extension 168, host 530, protocol 186, CLI 75 and browser-check 69. One relay timing assertion flaked once under the parallel gate and passed both focused and complete reruns.
- Ownership ledger: task and delivery lifetime · completed by `01a0a1f8-57b2-76c8-926f-52ac71acb877` · stable interfaces handed to T4/T5/T7/T3 · integrated `a256efe` · status done.
...
- 2026-09-15 implementation and two parent correction passes ready at `6890cd7`: byte-exact bounded decoder, producer-redacted chunked captures with one pressure-aware terminal outcome, per-chunk and aggregate durable integrity, hard direct/paired connection queues, hard durable-output bounds, exact renderer pending bytes, visible S9 omission evidence and typed retained counters. Broad package/build gates pass except the known clean-base bundled-npx worker environment test; real desktop/phone × dark/light matrix is 4/4 and bounded-memory evidence is in `/tmp/m18-t7-report.md`.
- 2026-09-15 independent review `/tmp/m18-t7-review.md` rejects the slice: depth-capped redaction fails open; concurrent captures exceed the global cap; relay frame/shaper caps silently drop state; Noise/chaff/stuck/parse-error accounting is incomplete; S9 abbreviates evidence and browser coverage omits release states/reduced motion; counter/generation/migration bounds and coordinator structure need correction. Exact focused tests pass but some codify the wrong relay policy; submitted report is stale and lacks final-head E2E/PSS evidence.
- 2026-09-15 correction implementation rebased onto RP-4 at `f813651`: fail-closed storage redaction, actor-scoped host reservations, authoritative relay fencing, exact Noise/chaff/stuck/parse accounting, atomic extracted content store, exact S9 ordinary/reduced browser states and real near-ceiling E2E all pass. Mandatory footprint evidence is complete but identifies capture ingestion PSS/private growth 163→355 MiB for the unchanged 20×12 MiB workload.
- 2026-09-15 parent verification passed 22 protocol + 87 host + 31 T4 worker focused tests and found final gaps: detached concurrent producer bodies are not worker-global bounded and response can overtake capture completion; worker pipe frame counts report zero beside queued bytes; getter/proxy redaction and newer-schema LogStore usage still need fail-closed handling; the >2× capture resident regression must be removed rather than documented. Final run `run_ee057e49` owns this batch and exact-workload remeasurement.
- 2026-09-15 correction `5ae3b06` closes worker-global producer reservations, response ordering, exact bidirectional pipe frame counts, getter/proxy and future-schema lockout, and reduces unchanged capture-ingestion footprint from 163.3 to 50.2 MiB PSS with zero survivors; broad gates and final ordinary/reduced browser matrices pass. Parent code inspection found the streaming defense still uses an incomplete 512-character overlap, UTF-8 chunking still creates a whole Buffer/array despite its claim, future-schema bytes render zero, sink failures can escape, and successful response waits leave timers armed.
- 2026-09-15 correction `ac1b2be` implements bounded streaming JSON-key scanning, allocation-free incremental UTF-8 chunks, honest future-schema metrics, guarded streaming store failures and cancelled response timers; worker evidence reports all gates and improved 39 MiB capture PSS. Parent exact-head focused rerun passed 95/96 but the required real near-ceiling E2E falsely released a healthy 15.3 MiB capture as `link-busy`: the 80 ms drain allowance is shorter than real bounded SQLite processing.
- 2026-09-15 done and integrated at merge `0542a58e`: bounded linear decoder; exact direct/worker/Noise/chaff/relay queue accounting and authoritative fencing; compiler-declared diagnostic shedding only; process-global producer and actor-scoped host reservations; fail-closed depth/key redaction; atomic bounded body storage; exact S9 omission evidence. Final real near-ceiling drain fix `4c95de9` passes five consecutive worker→host E2E runs plus parent focused protocol/pi-extension/host 117/117. Ordinary/reduced browser matrices, privacy scan, zero-survivor fixed workloads and `/tmp/t7-footprint-after6.json` prove decoder 120.1 MiB and capture 38.8 MiB PSS against the unchanged 188 MiB capture bound. Report: `/tmp/m18-t7-report.md`.
- Ownership ledger: transport pressure · completed by `01a0a214-a8cc-76c8-926f-52b4b3d8408e` · integrated `0542a58e`; T7 interfaces released to T3/T8/T10 · status done.
...
- 2026-09-14 ownership transfer: original T3 owner stopped after reviewed `8b5a269`; its clean merged worktree was removed. Browser acceptance now belongs to `01a0a217-2916-76c8-926f-52bf76c097a0` run `run_9f46f82d`, shared with T13B because both require the same real-host desktop/phone/theme/touch/reduced-motion harness. Counter wiring still waits T4-T7.
- 2026-09-15 final browser acceptance and review merged as `a0111f9`: real-host Resources and environment matrices each pass desktop/phone, dark/light, pointer/touch; Resources proves loading, coverage/trends/history, safe routing, disconnect/reconnect, visible-only polling, reduced motion and redacted export. Phone SpecSheet/ticker/touch defects and dead pre-environment fixture seeding were fixed; reviewer independently reran both 4/4 matrices. Root browser-check 69/69 and focused UI 23/23 pass.
...
- 2026-09-15 T4/T5/T6/T7 retained-store producers are integrated; checkpoint `4ba58bd4` replaces light-view guesses with T5's exact hydrated bytes/count and labels the T7 queue authority “Host / workers”. Focused UI 22/22 and typecheck pass. T8 retains its explicitly transferred pressure subsection; final T3 review/browser evidence follows T8.
- 2026-09-16 closeout on the finished RP-8 stack: the pressure subsection (`PressureDiagnostics.tsx`, F3 review `/tmp/m18-t8-f3-review.md`) renders the D-265 headline, one role-card projection with configured-vs-measured heap ceilings from G, honest coverage and the journal; T4–T8 counters are all consumed. Final real-host Resources matrix at `ce455b99`: `/tmp/m18-t3-closeout/run-GoZ2AA` desktop/phone × dark/light × pointer/touch, summary/pressure-critical/process/no-trend/reduced-motion/disconnected/disconnected-mount states, visible-only polling, zero survivors; contact sheet inspected. Done.
- Ownership ledger: diagnostics UI · orchestrator `01a0a030` · every area released after closeout `ce455b99` · status done.
...
- 2026-09-15 plan v1 `/tmp/m18-t8-plan.md` maps existing T4/T5/T6 release seams and proposes role thresholds/controllers, but needs correction before approval: workers must self-act/report rather than wait for host-self pressure; renderer physical freshness and actor-specific ordering need exact contracts; aggregate worker admission plus lower ceilings must contain multi-worker runaways; internal RPCs need explicit client refusal; wire events need enums rather than arbitrary detail; task-output addressability, host ceiling proof, history amplification and actual heap evidence need stronger gates. T8 receives only the new pressure subsection in T3 UI.
- 2026-09-15 plan v2 `/tmp/m18-t8-plan.md` approved for implementation after T7 integrates: one self-sampled controller per process; owner-local ordered subsequences with worker→host feedback; host/machine admission plus atomic T4 release paths; renderer-local T5 release/refusal; one enum-only journal/summary; task-output addressability; ceilings last with harness-attributed recovery; T8 owns only the new pressure subsection in Resources UI. Implementation must bind worker reports to the sending `WorkerClient` at host ingress (host assigns event IDs/time/project; worker supplies only action results), add a static aggregate worker-ceiling reservation cap so non-Linux hosts remain bounded without `MemAvailable`, use heap-only honest degradation outside Linux rather than importing host collectors, and run ordinary plus reduced-motion browser matrices.
- 2026-09-15 RP-7 integrated; milestone A calibration resumed as `01a0a397-d49e-76c8-926f-532d5ea2d1da` run `run_b186cd2e` on exact base `722c7ecc`. It owns scratch-process largest-history/multi-agent PSS/private/heap evidence and `/tmp/m18-t8-calibration.{md,json}` only; no production source or planning writes until T10 releases the narrow protocol-resources overlap.
- 2026-09-15 milestone A complete at exact base `722c7ecc`: `/tmp/m18-t8-calibration.{md,json}` records complete natural PSS/private/heap evidence for largest-history and quick multi-agent phases, zero survivors/raw heaps, and no A1/A2/A3 threshold adjustment. Host peak 200.6 MiB, worker 408.6 MiB, renderer 601.8 MiB at measured scale; full scenario 4 is unavailable/inconclusive after snapshot contamination, never zero. It also found the T15 slow-socket/counter prerequisites and the pinned-view audit now owned under T5.
- 2026-09-15 current-interface production plan `/tmp/m18-t8-final-plan.md` supersedes v2: it preserves the calibrated actor-local subsequences/ceilings, consumes T10/T7 store and transport contracts, adds T16 host/worker body-range memo release, routes pressure unload only through T18's lease, and specifies focused, scratch-process, unchanged-soak, browser and packaged acceptance. Source work remains dependency-blocked on final reviewed T16 and T18 integration.
- 2026-09-16 source milestone B0+B claimed on final dependency base `a575ee85`: re-run the unchanged quick/through-scenario-5 calibration rules, then implement only the compiler-complete enum-only protocol contract and explicit host router refusal from `/tmp/m18-t8-final-plan.md`; no controllers, release behavior, UI or ceilings yet. Isolated owner worktree fast-forwards from clean `722c7ecc`; report `/tmp/m18-t8-report.md`.
- 2026-09-16 B0 on `58b47eba` completed unchanged quick nine-scenario and full through scenario 5 with complete harness PSS coverage and zero survivors (`/tmp/m18-t8-b0-{quick,full}`); calibration A1/A2/A3 fired at full scale, so source stopped before B. D-261 adopts host 512/768 MiB unchanged, worker and renderer 1280/1920 MiB, the mechanically sufficient 1728 MiB worker clamp floor, and 1280 MiB non-Linux worker reservation. The fixed 1536 MiB renderer harness ceiling remains unchanged; focused injected-level tests prove critical behavior because unchanged normal fixtures must not reach pressure. Resume B from the held number-free patch `/tmp/m18-t8-b-contract.patch`.
...
- 2026-09-16 milestone C reached `0bd1cf49`; parent focused verification passed protocol 310, worker pressure/replay/git 62 and host client/pool 32 with no unhandled errors. Independent review `run_7f130988` requested one correction batch: consume/drop private worker pressure reports before generic direct/relay broadcast; reset candidate streak on every unknown observation; and prove the ≤16 task-pressure recipient cap with real session-access recency. Existing owner resumed in `run_92a191b2`; no D/E/F/G work had started.
- 2026-09-16 milestone C integrated at `a4a364c2` after corrections `b51ca651` and `c781c426`: private reports take a dedicated host-only road and reach neither direct clients nor a proven-live relay listener/log; unknown evidence breaks every candidate streak; true session access drives the bounded 16-session task-pressure order. Post-merge build, protocol 310, worker focused 72, host focused 35, identity, direction and diff checks pass. Worker-local actions remain steps 1/3/4 only; companion D is still intentionally absent.
- 2026-09-16 milestone D plan approved: one-shot terminal-only bounds are warning 50 records/15 minutes/512 KiB, critical 20 records/5 minutes/128 KiB, always preserving the newest five. Normal bounds, running tasks/live tails and durable logs stay unchanged. Worker claims a release only from synchronous before/after retention deltas; an observed no-change is `nothing_to_give`, missing comparison evidence is `unavailable`. Exact counter deltas are the acceptance proof, with a bounded deleted scratch GC/PSS probe as non-gating physical corroboration.
- 2026-09-16 milestone D integrated through `87e65720`: terminal-only compaction publishes synchronously, TaskIndex remains the row/log authority, and deterministic deltas drive the worker outcome. Parent inspection corrected live-excerpt accounting before review. Independent review `/tmp/m18-t8-d-review.md` then found terminal records could be forgotten while durable writes still rotated segments; the single review-fix batch now retains publication ownership until drain and proves the former stale-segment failure with real `TaskLog` → `TaskIndex` → `readLogTail`. Parent exact-tip verification passed pi-extension retention/background 31/31 and worker pressure/TaskIndex 61/61; owner full suites passed pi-extension 199/199 and worker 1,026/4 skipped plus build/typecheck/identity/direction.
- 2026-09-16 milestone E plan `/tmp/m18-t8-e-plan.md` assigned to fresh owner `01a0a965-2d5f-714d-99c8-17d52027fea4`. Host-only E1/E2/E3 interfaces are approved: process-local journal/epoch, Linux 2048/1024 MiB machine thresholds, 1280 MiB static worker reservation with a half-RAM aggregate cap, narrow cold-project admission, renderer-excluded host decisions, body-range-only host cache release, exact export trimming, intermediate fail-closed probes, generation allowlists and four-worker concurrent directive stages. Revision 3 restores heap-only non-Linux action and records uncancellable late directive results exactly once without resuming an ended pass.
- 2026-09-16 milestone E1 claimed by `01a0a965-2d5f-714d-99c8-17d52027fea4` on base `088d2259`: implement host sampling/serialization, generation-bound ingress, bounded journal/summary, local-only publication and snapshot/export evidence only. E2 actions and E3 admission remain non-goals until E1 is reviewed and integrated.
- 2026-09-16 milestone F plan `/tmp/m18-t8-f-plan.md` by fresh owner `01a0a97f-12fe-714d-99c8-17ed413cc297` approved as option A (D-265): renderer-local controller, `ProjectFileCache` + `ViewCache.releaseUnder` release, whole-transcript-only refusal, bounded-tail migration of three windowless callers, diagnostics inside the existing Resources surface. F0 packaged-Electron measurement probe precedes F1 controller code; F write set is `packages/ui/**` plus the shared browser script in F3, disjoint from E's `packages/host/**`.
- 2026-09-16 milestone E1 integrated at merge `d8ec6cae` (implementation `9cc47de5`/`f6e4a51b`, review fixes `89b6ca83`): host self-sampler, combined-probe hysteresis, bounded process-local journal, generation-bound worker ingress, window-only renderer presence, direct-local-only publication, snapshot summary and exact-bounded export. Independent review `/tmp/m18-t8-e1-review.md` found four defects (per-role hysteresis, escaping callbacks, oversized export, tooling sockets counted as windows); all corrected in the single fix batch. Parent post-merge: host pressure/resources 170/170, workspace typecheck, identity, direction. Report `/tmp/m18-t8-e-report.md`. E2 actions and E3 admission next under the same owner.
- 2026-09-16 milestone F1 ready at `9f4af1af` (implementation `61b97615`, parent corrections: preload-local `desktop.memory.process()` private-resident bridge proven in the packaged sandboxed renderer, no zero timestamp stand-in). Independent review `/tmp/m18-t8-f1-review.md` APPROVED with nonblocking findings now assigned as one fix batch (host-directive ageing, owned-refusal copy only, stable selector, one policy across the three whole-history affordances, dedup). Behaviour change to note: TopBar fork-from-last-prompt now reads a bounded tail filtered to the active branch. Carry-forward for E3: `history-loader.ts` `ensure()` still requests `{window:{all:true}}`; host-side `whole_transcript` refusal must exempt it or fork/navigate/edit break under host warning. F3 diagnostics follow on the same branch.
- 2026-09-16 milestone E2 integrated at merge `55350823` (implementation `d8336695`, review fixes `0c822394`): host step 1 body-memo release, ≤4 concurrent generation-bound worker directives under an observed 5 s bound with once-only late results, per-pass allowlists, lease-protected single session unload and atomic single worker retirement, cadence-only hysteresis with read-only intermediate probes. Independent review `/tmp/m18-t8-e2-review.md` findings (refused directives authorizing, fabricated step-5 refusal, probe-driven hysteresis, file growth, held reasons, cooldown/epoch gaps) all corrected; pass machinery lives in `pressure/pass.ts`. Parent post-merge: host 799/799, workspace typecheck, identity. E3 admission next under the same owner.
- 2026-09-16 milestone F integrated at merge `1b5fbfa7` (F1 `61b97615`/`9f4af1af`/`4cd4236d`, F3 `c5e98236`/`43178a80`): renderer-local controller with honest private-resident/heap evidence through the preload-local `desktop.memory.process()` bridge, `ProjectFileCache` and `ViewCache.releaseUnder` passes preserving pins/drafts/history, whole-transcript-only refusal presented consistently across four affordances, bounded-tail fork callers, epoch-fenced and 15 s-aged host publications, and the pressure subsection in Resources (`PressureDiagnostics.tsx`, D-265 headline, one role-card projection, honest coverage). Reviews `/tmp/m18-t8-f1-review.md` (approved) and `/tmp/m18-t8-f3-review.md` (B1–B4/N1–N5 corrected in `43178a80`). Browser matrix `/tmp/m18-t8-f3-browser-2/run-DBtIfy` 1360/390 × dark/light × pointer/touch with in-case reduced motion, zero survivors. Parent post-merge: UI settings/pressure/thread 759/759, UI+desktop typecheck, identity, desktop bridge-smoke green against a rebuilt preload (a stale `dist/preload.cjs` first produced a 60 s Electron timeout — build before that suite). Report `/tmp/m18-t8-f-report.md`.
- 2026-09-16 milestone E3 candidate `12cec9c5` reviewed (`/tmp/m18-t8-e3-review.md`): guard positions and zero side effects correct; policy defects returned as one batch — CLI `entries` still windowless, reservation fallback replaced the level policy on non-Linux, no startup probe made the fallback live for 20 s on Linux, refusal rows invented a level, plus param-validation order, warm-worker presence and unknown-cwd fallback.
- 2026-09-16 milestone G plan `/tmp/m18-t8-g-plan.md` by fresh owner `01a0aa00-05e9-714d-99c8-187dd64a979c` approved: host old-space ceiling 448 MiB (`ceil64(2 × 200.6 MiB)`), worker 1728–2048 MiB per D-261, explicit argv only with `NODE_OPTIONS` stripped, absent flag reported as no configured ceiling (never zero), `MemoryPressureCeiling` on host/worker rows, OOM classified by fatal marker plus abnormal exit and attributed `harness`, one private bounded `pi/worker/recover-agent-failures` request so a successor delivers the parent's canonical `lasercode/agent-event`, packaged clean-machine gate and Linux containment probe with the 2 GiB safety floor. Host-loss activation/reconciliation is recorded as an M19 finding. G1a (ceiling module, worker-client, CLI/desktop launchers, worker parsing, protocol field) proceeds now; G1b/G2/G3 wait for E3 integration on shared host files.
- 2026-09-16 milestone E3 integrated at merge `49cc12d9` (implementation `bf7d8d74`, review fixes `bf5571ee`): four guard-first admission refusals with zero side effects, reservation fallback supplementing the settled level, immediate startup probe, honest refusal rows at the decision level, bounded CLI `entries` read, warm-worker presence and validation-before-admission. Independent review `/tmp/m18-t8-e3-review.md` F1–F7 all corrected. Parent post-merge: host 821/821, CLI 77/77, workspace typecheck, identity. Host pressure orchestration E is complete; remaining RP-8 work is ceilings G and the T3 pressure closeout.
- 2026-09-16 G1a/G1b/G2 checkpoint: explicit host/worker old-space launches and truthful configured/measured rows landed through `babfd1a8`; harness-owned worker-loss attribution, canonical successor delivery and real OOM/convergence proof landed at `5f01e123`. Independent review `/tmp/m18-t8-g12-review.md` requested one correction batch; it now extracts and directly tests bounded recovery (including overflow, later incidents and successor re-filtering), centralizes argv/environment parsing, makes undersized capacity a project refusal, and closes the listed launcher/lifecycle/dedup findings. G3 remains unstarted pending parent verification.
- 2026-09-16 milestone G1+G2 integrated at merge `4f15add0` (G1a `d9f92de0`, G1b `babfd1a8`, G2 `5f01e123`, review fixes `3ee170ca`): explicit `--max-old-space-size` argv for host (448 MiB) and worker (1728–2048 MiB, 64 MiB quantum, `min(totalmem, constrainedMemory)`, unknown→floor, undersized machine → project-level refusal at spawn), `NODE_OPTIONS` scrubbed by one protocol helper, one protocol `heap-ceiling` parser shared by host and worker, configured-vs-measured `MemoryPressureCeiling` on host and worker rows (absent flag stays absent), OOM classified by fatal marker plus abnormal exit inside a 4 KiB stderr window, harness attribution of every live run, bounded `AgentFailureRecoveryQueue` (`packages/host/src/agents/failure-recovery.ts`) delivering the parent's canonical `lasercode/agent-event` through the exact reopened successor via compiler-complete private `pi/worker/recover-agent-failures`, real-child OOM and built-worker convergence E2E, desktop owned-host restart accounting with healthy reset, CLI foreground re-exec with signal forwarding. Review `/tmp/m18-t8-g12-review.md` B1–B5/N1–N10 all corrected. Parent post-merge: host 842/842, worker 1028/4 skipped, CLI 82, desktop 171/1 skipped, build, identity. G3 (packaged gate + containment probe) remains.
- 2026-09-16 milestone G3 candidate on tracking base `0a5d0b09`: packaged Linux clean-machine gate passes from bundled Node with empty PATH, all default features, one model, host 448 MiB configured/640 MiB measured, worker 2048 MiB configured/2240 MiB measured, clean SIGTERM and zero survivors; finite 384 MiB worker containment passes OOM/abnormal exit, one canonical successor, revision/leaf/entries convergence, 15.31 GiB minimum available and zero survivors. G1's intentional `NODE_OPTIONS` scrub exposed the RP-2 inspector's inherited preload; the harness now uses explicit host argv and worker-wrapper import without changing fixture/workload. Unchanged quick all nine scenarios then passes with zero survivors; largest-history and multi-agent role aggregates remain below D-261 warnings. Browser-check 97/97 passes; sanitized evidence is `/tmp/m18-t8-g-{packaged,containment,report}.{json,json,md}`.
- 2026-09-16 milestone G3 integrated at merge `ce455b99` (`fe36b4ad`): packaged clean-machine gate (bundled Node, empty PATH, no `NODE_OPTIONS`, explicit 448 MiB host argv, real session with all default features, one model, `resource/snapshot` host row 469,762,048 configured / 671,088,640 measured and worker row 2,147,483,648 / 2,348,810,240, clean SIGTERM, zero survivors, decoy unchanged — 17/17 claims, `/tmp/m18-t8-g-packaged.json`); Linux containment probe `scripts/browser-check/resource/heap-ceiling-containment.mjs` (384 MiB scratch worker, heap OOM classified abnormal, one successor, revision/leaf/entries converge, minimum `MemAvailable` 15,168,602,112 bytes ≥ 2 GiB, zero survivors — `/tmp/m18-t8-g-containment.json`, reproduced by the parent post-merge); exact-SHA RP-2 quick fixture nine scenarios pass with largest-history and multi-agent totals below D-261 warnings (`/tmp/resource-quick-g3-final`); the soak harness now passes its inspector preload as explicit argv because production launchers scrub `NODE_OPTIONS`. Root browser-check 97/97. M18-T8 done; report `/tmp/m18-t8-g-report.md`.
- Ownership ledger: pressure policy · ceilings owner `01a0aa00-05e9-714d-99c8-187dd64a979c` released after integrated G3 `ce455b99`; every M18-T8 area released · status done. `01a0a397-d49e-76c8-926f-532d5ea2d1da` released after reviewed/integrated D `87e65720`; host orchestration owner `01a0a965-2d5f-714d-99c8-17d52027fea4` owns approved host E3 paths on base `55350823`; host owner `01a0a965-2d5f-714d-99c8-17d52027fea4` released after integrated E3 `49cc12d9`; ceilings owner `01a0aa00-05e9-714d-99c8-187dd64a979c` owns G1–G3 and stops after one G3 commit; renderer owner `01a0a97f-12fe-714d-99c8-17ed413cc297` released after integrated F `1b5fbfa7` · next handoff parent inspection.
...
- 2026-09-16 done at merge `76f475fa` (T19a `e69d2bb0`, corrections `0ccf00f8`, T19b `1b793baa`, denied-affordance tests `9aaf3f37`, final fixes `732a80dd`): one `capabilityFor`/`useCapability` decision (discriminated union) over the authenticated descriptor and the protocol's `methodPolicy`/`reachAllows`, `POLICED_METHODS`-pinned capability families, full descriptor stored atomically and cleared on reconnect/downgrade, `HostClient.request` preflight as the zero-frame backstop, shell/settings/agents/MCP/providers/projects/features/resources/logs/push controls hidden or explained per DESIGN.md with reads preserved, composer/approval/question/lifecycle/dictation/file/Beam/fleet/prewarm denied before invocation with every alternate path (shortcut, slash, palette, double-click, drag) closed. Reviews `/tmp/m18-t19a-review.md`, `/tmp/m18-t19b-review.md` (all findings corrected). Browser: seeded narrowed-policy target `scripts/browser-check/test/environment-affordances.mjs` desktop/phone × dark/light, pointer (`/tmp/browser-check/run-ojkghk`) and touch + reduced motion (`/tmp/browser-check/run-u6GUxi`), contact sheets inspected. Parent post-merge: UI 2260/1 skipped, both tsconfigs, build, identity, browser-check 97/97.
...
- 2026-09-14 resume checkpoint `/tmp/m18-t13-resume-checkpoint.md`: all 30 preserved files were staged and complete; continuation kept them byte-for-byte, expanded validation and committed milestone A as `b676796`.
- 2026-09-14 A evidence: `environment/describe` ep1, compiler-complete request/notification policy, pre-parse authorization, narrow-only policy/grants, bounded redacted audit and synthetic Noise paired E2E. Protocol 182/182, host 496/496, relay 21/21, CLI 73/73, desktop 160 + 1 skip, build/typecheck/identity/direction pass.
- 2026-09-14 security review changes requested: nonloopback direct sockets were misclassified as paired; optional Router authority failed open; trust-request replay/missing-actor notification egress bypassed policy; audit reserves/summary claims and dictation chunk volume need correction; direct MCP/project-env execution needs its own scope; launcher policy docs overclaimed.
- 2026-09-14 milestone A integrated as `a4bb51a` + review correction `00db89f`: nonloopback direct sockets refused; Router authority required; notification egress fail-closed; bounded refusal reserves/stream summaries; distinct execution scope; truthful launcher docs. Integrated build, protocol 184/184, host 507/507, CLI 74/74, identity and direction checks pass. The first host test run before rebuilding protocol dist failed on the stale dist export; rebuild plus rerun is green.
- 2026-09-14 milestone B plan `/tmp/m18-t13b-plan.md` approved with full key inventory: version→descriptor handshake before open/resume; one disabled-by-default environment storage authority; purge-not-rekey legacy/foreign state; cache-bounded drafts; synchronous changed/downgraded reset and module rehydration. Legacy project import is removed; fleet-read state is environment-scoped; no permanent global test seam.
- 2026-09-14 B implementation `ce8bb6b`: 52 UI/protocol/docs/test files; UI 1,745 pass/1 skip, protocol 184, CLI 74, desktop 160/1 skip, workspace build/typecheck/identity/direction pass.
- 2026-09-14 independent review changes requested: first activation never rehydrates archive/pins/folds/Beam/fleet and the next write loses them; an old handshake timer can close/reset a healthy replacement; draft byte/expiry bounds and missing/corrupt fingerprint invalidation need correction; phone hides the failure reason. One batch also extracts environment lifecycle, uses discriminated activation, remounts component-local environment state, hardens socket generations/storage failure and improves test fidelity.
- 2026-09-14 review batch `cb2ae9e` closes first-activation rehydration, socket generation timers, UTF-8 draft bounds, fingerprint schemas, persistent environment notice, lifecycle extraction/remount and test fidelity; UI 1,774/1 skip plus protocol/CLI/desktop/build/typecheck/identity/direction pass. Self-verification found current-namespace/content removals were not verified and the advertised clear-data recovery could not clear >5,000 keys; narrow fail-closed follow-up active.
...
- 2026-09-15 plan v2 `/tmp/m18-t10-plan.md` approved after correction: a bounded async environment preparation gates connection-open long enough to guarantee a warm local-first `peek` while cache failure still permits authoritative loading; production never falls back to memory; hostile IndexedDB scans/purges/clears are numerically bounded, cooperative, verified and fail closed; recovery awaits deletion before reload; opaque session ID is the intentional cache key. T5 was asked to land the stable `installViewTailSink` seam. T10 owns the `deviceCache` typed producer; T3 owns its row/pixels.
- 2026-09-15 T5 and T7 contracts are integrated and released; implementation resumed as `01a0a2fe-219e-76c8-926f-52f9c3310b62` run `run_40a964d1` on exact base `722c7ecc`. T10 owns its approved tail-cache/runtime/client/device-storage/desktop-key bridge/settings/narrow protocol-store interfaces and tests; T3 pixels, T8 pressure and T11 provisional transcript paint remain excluded.
- 2026-09-15 implementation through `0419eab4` passes parent focused cache/readiness/settings/key tests, package gates and real pointer/touch browser matrices, but independent review `/tmp/m18-t10-review.md` requires changes before integration: serialize write/delete/clear races, cancel never-settling readiness and validate every cold row, make durable age/LRU exact, seal all content-derived metadata, complete B2/B10–B14 and before/after resource evidence, and decompose the 1,005-line authority. Accepted correction plan `/tmp/m18-t10-review-fix-plan.md` also removes the unapproved fake-IDB dependency and all durable path locators; original owner is implementing one complete correction batch.
- 2026-09-15 reviewed correction completed through `2b9db770` and integrated at `c7375791`: one lifecycle/mutation authority; session-ID-only durable identity; sealed content metadata; bounded readiness, scans, transactions, controls, retries, touches and deletion; full-row authentication/validation; revision-fenced supersession; no production memory fallback or fake IndexedDB. Independent review `/tmp/m18-t10-review.md` received one batched correction cycle; parent re-inspected every later correction and passed exact-tip tail-cache 113, readiness 18, UI test types and build. Browser lifecycle/race matrices remain 4/4 each at `/tmp/m18-t10-last-long/run-9uWw3h` and `/tmp/m18-t10-last-projects/run-rFpGUT`; accepted pointer/touch and same-workload cache memory artifacts are indexed in `/tmp/m18-t10-report.md`.
- Ownership ledger: device tail cache · M18-T10 done through merge `c7375791` · owner stopped, branch released after integration · review cycle complete · RP-11 consumes `peek`/`prime`/`supersede`; RP-5b consumes the bounded DTO/cache seam.
...
- 2026-09-16 implementation `fa68dd02` ready for independent review: synchronous warm-cache provisional paint, assistant-ui send-only fence, canonical delta/replace fold, truthful offline retry and no blank frame. UI 2,151/1 skipped, builds/typechecks/identity/direction and real pointer/touch/reduced-motion desktop/phone/theme matrices 12/12 pass; artifacts `/tmp/m18-t11-bc-{pointer3,touch3,reduced3}`. Review target is exact commit `fa68dd02` over `6d8ded11`.
- 2026-09-16 independent review `/tmp/m18-t11-review.md` rejected `fa68dd02`: fleet stop and thread-list mutations escaped provisional authority, cache supersession targeted whichever revision was held after the race, the browser gate admitted wrong offline/frame/authority outcomes and did not prove focus/anchor preservation, and the consumer trusted `peek` identity implicitly. One correction batch returned to the same owner; parent also required late-promotion fencing and exact-session validation.
- 2026-09-16 review correction `eddc9f38` centralised provisional authority, fenced session-owned fleet/sidebar mutations without blocking unrelated work, compare-deleted only the painted revision, rejected foreign-session cache rows and made the real browser gate prove an offline warm paint, deterministic mismatch, focus/draft/reading-place preservation and authoritative recovery. Parent verification then bounded retained paint capture and serialized exact-revision deletion behind accepted newer writes in `bae977d9`; final direct fence-through-storage fix `78b22c83` prevents the same stale revision from being re-admitted while deletion awaits. Focused 189/189 plus UI full 2,161/1 skipped, host 687, workspace build/typecheck/identity/direction and three corrected pointer/touch/reduced-motion matrices 12/12 pass with zero survivors (`/tmp/m18-t11-fix4-{pointer,touch,reduced}`). Done; ownership released to T8.
...
- 2026-09-14 implementation `36b2443` reviewed: inactive `authority:any` projection/no-spawn/identity/torn-tail behavior is sound and full protocol 165/165 plus host 452/452 passed; cold 20k was 144.09 ms with 40 bodies and zero spawns. Review found shared `baseRevision` planning breaks `before`/`from`/`all`, live replacement bounds diverge from durable, oversized durable `all` silently changes semantics, and the decrement/replan loop can block ~620 ms.
- 2026-09-14 correction `ac45371` integrated with implementation as `0e2fa48`/`e9e3d8d`: deltas only serve live-edge tails; anchored/all shapes are exact replacements; suffix/empty-delta semantics are documented; live/durable answers share complete-turn 200-row/1 MiB bounds including context; binary-search planning replaces the blocking decrement loop; one delta discriminant and one durable ownership gate remain. Added request-shape/base-state, authority-parity and pathological bounds tests.
- 2026-09-14 final integrated verification: protocol 167/167; focused host revision/projection/index/router/E2E 106/106; focused worker 44/44; workspace typecheck, identity, direction and diff checks passed. Cold 20k integrated scan 256 ms under parallel load with 0.16 ms p99/62.15 ms observed max slice; pathological 20k/tail:200 projection 195.19 ms under parallel load and no linear retry loop. T12 done.
- Ownership ledger: worker-free reads · integrated through `e9e3d8d` · M18-T12 done · review cycle complete · next handoff: T13 consumes authority/capability policy; T10/T11 consume environmentKey/revision and delta-or-replace without spawning inactive workers.
...
| M19-T2 | Immutable generation activation and correlated updates | done | m19-recovery-plan `01a0aa8f-8f82-714d-99c8-190890feefc7` | merge `d59f74e8` (T2A `b09a6189`, T2B `b2c84f96`, T2C `1c2f3da2`, validation `8c127c8d`; D-270 review-fix batch and `/tmp/m19-t2-report.md` ; D-270 fixes `ee71f677`); host 874, desktop 177/1 skipped, worker 1034/4 skipped, UI 2279/1 skipped, CLI 88; .deb fixed-root upgrade cycle `/tmp/m19-t2-deb-upgrade.txt` | see notes; added by D-264; depends on M19-T1 |
| M19-T3 | Migration snapshot and interrupted-restore recovery | in-progress | m19-recovery-plan `01a0aa8f-8f82-714d-99c8-190890feefc7` | — | added by D-264; depends on M19-T2 |
...
- 2026-09-16 implementation owner `m19-recovery-a0b859e3`: one review after all three T1 commits. D-266's runtime generation inventory amendment belongs to T2 and replaces the plan's byte-copy proposal.
...
- 2026-09-16 T1B complete: CLI and desktop mint exact 128-bit host identities; `host.json` moves atomically from `starting` to `ready`; health, adoption and worker first-frame readiness require the exact launch identity. Evidence: focused CLI/desktop/host launch tests and rebased commit `58b0ea1f`.
- 2026-09-16 T1C complete: bounded durable two-retry repair, corruption pause, persistent non-destructive safe mode, existing-element recovery actions, and load-time host-loss agent failure delivery. Initial evidence: `/tmp/m19-t1-report.md` and `/tmp/m19-t1-*`; soak intentionally not rerun.
- 2026-09-16 review fixes claimed on rebased base `ea11910a`: preserve competing/legacy hosts, unify production/test retry policy, fence worker frames, drain startup failures, centralize recovery UI/copy, and close every non-blocking finding in `/tmp/m19-t1-review.md`.
...
- 2026-09-16 integrated at merge `d59f74e8` after review `/tmp/m19-t2-review.md` (B1/B2 fixed-root refusal and unreachable rollback → D-270 manifest-in-root semantics; B3 pending tray admission; bounded verification 15,283 files/537 ms cold → 3 files/78 ms warm; `METHOD_POLICY`-derived new-root set; one copy source). Parent post-merge: host 874, desktop 177/1, worker 1034/4, UI 2279/1, CLI 88, build, identity. Owner continues into M19-T3.
...
- 2026-09-17 T3B complete at `fb75bf61`: shared CLI/desktop pre-host helper emits versioned secret-free JSONL, binds the migration engine to durable T2 phases before fixed-root selection, blocks daemon bind while a marker exists, and exposes exact retry/restore progress through desktop IPC, startup and the existing update notice. Synthetic migration, correlated restore, daemon refusal, protocol copy and UI action tests pass; protocol 323, CLI 91, host migration/transaction 16, desktop 179/1 skipped except one corrected startup assertion subsequently focused green.
- 2026-09-17 T3C complete at `877ed4f6`: real child processes were SIGKILLed at snapshotting, verified-snapshot/migrating, partial-step, migrated-before-selection and partial-restore points; every restart converged to exact bytes/modes in `packages/host/test/migrations/process-recovery.test.ts` (5 tests). A fixed extracted `.deb` root exercised prior install → overwrite → successful migration/exact launch commit, a failed migration with no host bind and correlated JSONL, exact data rollback, and retry to success; `/tmp/m19-t3-deb-cycle.txt`, `/tmp/m19-t3-deb-package.txt`, `/tmp/m19-t3-zero-survivors.txt`. Focused host 21, CLI 4 and desktop 4 tests plus builds and identity pass; one T3 review and the final full gates remain.
- 2026-09-17 review-fix batch implemented from `/tmp/m19-t3-review.md`: migrated marker now survives exact launch readiness and drives production recovery/rollback; disk update identity owns pre-host restore; old host only parks and migration runs in a worker-thread preflight; canonical selection handles install-root moves; forward step-index resume, streamed hashing, progress, categorical private diagnostics and one copy source are pinned by focused tests. The fixed-root cycle harness is committed at `packages/desktop/scripts/migration-deb-cycle.mjs`; final full gates follow the single fix commit.
- 2026-09-17 final review-fix evidence: full build and tests pass (protocol 323, crypto 43, pi-goal 8, pi-extension 200, relay 21, UI 2280/1 skipped, worker 1034/4 skipped, host 895, CLI 92, desktop 181/1 skipped); `pnpm verify` 104/104, release 45/45, identity and direction pass. Fresh `.deb`, clean-machine and committed fixed-root success/failure/rollback/retry harness pass at `/tmp/m19-t3-deb-package.txt`, `/tmp/m19-t3-clean-machine.txt`, `/tmp/m19-t3-deb-cycle.txt`. Reduced-motion pointer and touch matrices pass at `/tmp/m19-t3-browser-pointer/run-8vUl8d` and `/tmp/m19-t3-browser-touch/run-Eph686`; zero survivors recorded in `/tmp/m19-t3-zero-survivors.txt`. Consolidated evidence: `/tmp/m19-t3-report.md`. Ready for parent verification; M19-T3 is not marked done pending integration.
...
Working tree at checkpoint: only this milestone’s UI/tests/docs; final commit and clean status are in the report. No changes to worker/protocol/thread/store or the parent checkout.
Decision: re-inspect after policy saves; worker `service.save` closes the connection, so a held-connection/fast-refresh assumption was false. Correct worker-owned name/pattern semantics win over UI duplication; switches stay activation-blocked but keyboard-focusable until refreshed. Scoped MCP headings use `typed` (12px); unrelated global 11px category eyebrows remain unchanged outside ownership.
Next: orchestrator reviews/integrates these UI fixes alongside the separate engine findings. Re-run combined acceptance; default gallery, status propagation, missing-command copy, live OAuth and packaging are not this branch’s claims. Own isolated host/Chrome stopped; fixture artifacts retained.
...
### H-11 · M14-T8 · 2026-09-12 · env-mcp-01a093aa
State of the work: investigation complete; implementation not started. The person requested a report to pass to another coding agent: `docs/incidents/shell-environment-and-mcp-identity.md` contains code paths, safe live probes, confirmed host adoption, independent client-branding defect, uncertainties and acceptance plan.
Uncommitted: own PLAN/STATUS/STATUS_DETAILED/report only; no application/dependency/config changes. `/tmp/mcp-identity-patch` is an unedited pnpm patch preparation, never committed/applied. Preserve all pre-existing deleted discovery docs and untracked personal files.
What is broken: command tools use non-interactive Bash, `.bashrc` has an early-return guard, the adopted host retains a missing export, and MCP clients advertise engine branding. Normal Quit does not stop an adopted host.
Next concrete step: next owner reads the report, chooses the shared exported-environment contract, reproduces with fake HOME and a real MCP fixture, and implements product-derived handshake identity for every client path. Do not assume a Bash-tool-only change updates MCP subprocesses.
Do not: print the screenshot token, edit personal shell/config files, restart installed host/agents without consent, stage unrelated files, or call this fixed.
...
### D-231 · 2026-09-13 · Sustained navigation is a release requirement
Decision: Add M16-T29 to the next release. M16-T16's tail-first loading remains; its deferred mounted-row optimization is reconsidered against the person's report of slow switching after scrolling back or growing a conversation live.
Why: initial 40-message acceptance does not test sustained use; the existing renderer mounts every loaded message, and the prior report records roughly 57,000 elements after full loading.
Consequences: first reproduce and attribute outgoing/incoming switch work, then approve the implementation with one continuing owner. Preserve canonical history and reading position; do not substitute clearing sessions or a cosmetic loading delay for bounded rendering. Coordinator owns planning; investigation owner may write only its external report until approval. Base `5bcc4fa`; unrelated local STATUS edits and docs deletions preserved. No overlap with MCP cache or composer explorer ownership.

### D-232 · 2026-09-13 · Mentions explore the filesystem, not just project search
Decision: Correct M16-T28's coordinator brief to the person's original global explorer request: `@/` means filesystem root; relative paths start at session cwd; absolute paths, parent traversal and readable outside symlinks use the same OS-account boundary as the general file viewer. Ignored/generated entries such as node_modules remain available. Onboarding defaults remain unchanged.
Why: the initial project-root interpretation and git exclusions contradicted the person's examples; this was caught during integration inspection of `8e8d427`, before review or release.
Consequences: same at-explorer owner revises resolver, opt-in browse, tests and browser acceptance; coordinator owns PLAN/STATUS updates. Next handoff is independent review of the full revised feature. No live user files or credentials are read for acceptance.

### D-233 · 2026-09-13 · Enabled MCP transports may connect lazily without model management
Decision: D-230 forbids model-directed server configuration, enabling/disabling, installation/removal and sign-in. It does not forbid the client's internal transport establishment needed to discover or call tools on an already person-configured, enabled and trusted server. Reuse existing manager lazy-connect policy; no new model-facing connect verb or management framework.
Why: milestone3 acceptance at5195eec exposed an on-demand discovery dead end: model connect correctly refused, but ordinary discovery never established the enabled connection and advised the forbidden verb.
Consequences: one narrow correction by mcp-cache; generation checks before connect/serve/forward and across racing revocation remain mandatory; disabled/removed servers never connect, authentication needs person-owned Settings, provider tool arrays stay frozen. Test real cold discovery without inspector prewarming, script and direct routes, negative management cases, and final tracked-patch/package bytes. This clarifies D-230, not a reversal.

### Recovery checkpoint · 2026-09-13 · active owner transfer after runtime loss
- User requested resuming stopped work. This runtime has no child agents and cannot address former sessions. Read-only saved run-registry inspection confirms navigation `run_d1036919` and MCP `run_e6e9ca01` failed at `2026-09-13T06:52:51.830Z`; no processes have either private worktree as cwd. Prior writers are stopped; no user session/configuration was modified.
- M16-T29 C transfers from `01a09911-416f-74c0-9c74-c80b0e35990e`: `.worktrees/sustained-navigation-d1e0492b` is clean at `1b5f826`; C was not implemented. DialogBody presentation-state extension remains approved. Transferred to resume-transcript-window `01a099bd-9797-7379-b27a-ad343e6855ca`; private UI-only C scope, next durable `.git/coordination-recovery/C-REPORT.md` then independent review.
- M16-T25 milestone3 correction transfers from `01a098ba-15d8-74c0-9c74-c7bcb0a1e085`: `.worktrees/mcp-cache-fe11b683` at `5195eec` has five uncommitted on-demand/fixture/patch/lock files. Preserved all bytes; backup `.git/coordination-recovery/mcp-on-demand-wip.patch`, SHA256 `2bcfc61267d29b11a9fdf6286d389242971fb1874315f08a999a161c7ddcef86`. Transferred to resume-mcp-discovery `01a099be-556b-7379-b27a-ad3befbb5011`; finish this preserved correction, do not restart MCP implementation; next durable `.git/coordination-recovery/MCP-REPORT.md` then full milestone review.
- Prior `/tmp/sustained-navigation` reports and `/tmp/mcp-cache-report.md` are missing in this runtime. Historic results remain reported evidence, not available executable proof; reproduce required final gates, do not fabricate the lost artifacts. New handoffs use repo `.git/coordination-recovery/` for durable recovery copies. Coordinator retains planning ownership; each continuing owner receives only its private area.

### D-234 · 2026-09-13 · Unavailable authorization reads never become missing records
Decision: M16-T25 registry reads distinguish present, proven missing/corrupt, temporarily unavailable and locked. Establishment/mutation refuses unavailable reads without rewriting a valid generation; cached serving and forwarding remain fail-closed. Multi-identity revocation acquires the deduplicated sorted identity/CAS lock set and validates every record/expected generation before any write. No retries or timeout increases.
Why: nine deterministic regressions showed EIO/deadlines rotating valid epochs during establish/bump/account lookup and CAS falsely reporting revocation. Earlier nonreproduced integration failures remain separately unexplained.
Consequences: same owner implements bounded shared lock/read/write primitives and multi-identity tests, then one independent integrity review. Preflight failure changes no records and calls no mutation callback. Post-preflight write or callback failures retain conservative advanced generations; no rollback or claim of multi-file disk atomicity. No general reconnect policy or credential retry after true revocation. Evidence: `.git/coordination-recovery/MCP-REGISTRY-INTEGRITY.md`.

### D-235 · 2026-09-13 · Advisory authorization reads do not decide session availability
Decision: In M16-T25 establishment only, a failed unlocked fast-path read may join the existing establishment coalescer or enter one locked authoritative preflight. A held external mutation still refuses, and any unavailable authoritative read refuses without record changes/callback. Public current/serve/forward checks remain fail-closed; no timeout increases or retry loops. Credential-account lookup semantics remain unchanged.
Why: independent integrity review M1 showed rejecting a preliminary read permanently removes MCP tools from the conversation even if the authoritative read would succeed. The integrity boundary is the complete locked preflight, not the fast-path optimization. This does not establish the historical failures' causes.
Consequences: real session regression must retain discovery after an advisory-only failure; authoritative failures keep byte-preservation/no-forwarding guarantees. Complete bounded reads must reach EOF before classifying malformed content as proven corruption. Temporary diagnostic instrumentation is removed after its evidence is preserved. Clarifies D-234's establishment refusal boundary; no rollback or broader reconnect-policy change.

### D-236 · 2026-09-13 · Ordinary session re-entry starts at the recent tail
Decision: The person explicitly confirmed ordinary session switching back should reopen the latest ~40 messages, with older messages loaded again by scrolling up, rather than retain an expanded loaded window/deep reading position. Explicit search/navigation/action targets override this default; same-session reading anchors and same-row no-op remain. Preserve all durable canonical history, drafts, optimistic sends, live events and approvals; no data deletion disguised as optimization.
Why: user: after full-history scrolling and switching away/back, it should behave with limited messages; confirmed coordinator's latest-~40 clarification. This replaces cross-session deep-position restoration, not within-session anchor preservation or bounded live rendering.
Consequences: M16-T29 continuing owner plans explicit normal-reentry tail lifecycle with truthful cursors/completeness and stale-request/epoch fences; existing tail40 protocol preferred. Ownership extends to UI history-loader, normal-open lifecycle and directly related store/history tests. Existing bounded transcript/map and measured linear history projection remain relevant to live/current expanded use and outgoing cleanup. Performance evidence must separate these from reset-tail re-entry; do not claim old retained-history budgets passed through a changed test. PLAN acceptance wording updated; final integrated C review covers both policies.

### D-237 · 2026-09-13 · Portable MCP script runtime and bounded backend contract
Decision: Approve MCP5 backend implementation from MCP5-PLAN.md using exact QuickJS core/release-sync0.31.0, retaining adapter broker and bundled Node. User requires cross-platform Laser: Windows/macOS/Linux and supported x64/ARM64, no global install/CDN/download fallback/native-ABI runtime dependency. Package loading/execution proof is required per supported target; portable JS+Wasm alone is not test evidence.
Why: model-generated scripts need a restricted guest and enforceable resource boundaries, but this must not compromise distribution portability or silently change ordinary MCP permissions/API successes.
Consequences: preserve current person-owned approve policy/defaults and successful `{ok:true,data}` responses; throw bounded typed guest errors for failed calls. Per-call policy/revision checks and explicit same-args or tool-for-this-run grants only, never annotations or blanket script consent. Optional typed MCP approval payload reuses existing portable select/request lifecycle; worker validates exact invocation/ticket/arguments and cancels exact questions. C owns UI presentation until handoff; backend cannot claim full feature readiness without that consumer. Approve bundled-Node interruptible schema helper with pinned SDK seam tests, no network refs/coercion/main-thread timer-only validation. Plan numerical source/heap/stack/output/frame/transfer/trace/call limits accepted; no wire-call replay after admission, no dangling work after return, conservative partial-side-effect trace. Backend owner has adapter/worker/protocol/asset/test/docs scope only; dedicated security/approval review after integrated milestone. Guest limits are not whole-process RSS or a sandbox for powerful authorized MCP servers. Preserve historical failure uncertainty separately.

### D-238 · 2026-09-13 · Loaded history belongs to its rendered owner
Decision: Implement D-236 through owner-local authoritative loaded-history windows, not a global reset of shared open[path]. Main and Beam may display the same session independently. Canonical metadata/session structure, question/submission/draft authority and raw event ordering remain shared; entries/blocks/pagination metadata and pending-window replay belong to the rendered owner. No second full AppState/permission writer or protocol change.
Why: current scoped store changes only current/destination; resetting its shared session window would disturb an expanded peer. Same-path independence requires an internal ownership seam, not more scroll-loop exceptions.
Consequences: continuing C owner may add a focused history-owner helper plus narrow internal action fanout. Root and owner reductions complete before notification; root no-op cannot lose pending-owner replay. Same-epoch/same-leaf reset affects only requested owner; shared branch/epoch changes reconcile peers before readiness. Scope-local history requests/generations reject stale responses; disposal cancels local acceptance, not host work or canonical questions. Only current owned windows/bounded pending data retained, no per-visited-path strong cache. Tests cover same/different-path peers, epoch/seq/branch changes, live tools/questions, stale loader replies, send guards, disposal and no old-history flash. Final integrated C review includes this seam; checkpoint commits are not acceptance.

### D-239 · 2026-09-13 · Keep the pinned SDK's own output validation
Decision: Do not suppress the pinned MCP SDK's synchronous output-schema check per call to move validation into a bounded helper process. Ship the structural bounds (depth, node count, local references only, no remote `$id`, per-tool refusal) and validate against the schema captured with the call. The interruptible 250ms helper boundary stays unimplemented and documented as such.
Why: the worker checkpointed this as the "rewriting SDK validation architecture" case D-237 required approval for. Replacing a pinned dependency's validation path for every call is a large seam with its own failure modes, for a bound no observed workload needs yet.
Consequences: `docs/mcp-client.md`'s 250ms interruptible validation target remains an open requirement, not a shipped claim; a pathological schema can still occupy the worker for longer than that inside the SDK's own validator, bounded only by the structural limits. Revisit with a measured case rather than pre-emptively. No change to captured-schema/revision handling or per-tool isolation.

### D-240 · 2026-09-13 · The script runtime waits for its renderer and its packaged proof
Decision: Keep the reviewed MCP5 backend on `agents/mcp-cache-fe11b683` (537a4c9), out of main and out of the next release. Merge only once the approval renderer exists, the packaged and arm64 gates pass, and a person can see and answer a gated call in the transcript. The next release ships the already agreed scope without it.
Why: the slice is reviewed and corrected, but it adds an interpreter and dependency proven only on linux-x64 from source, and its per-call consent has no surface yet. Shipping it early would either claim a capability nobody can use or risk an unproven runtime in an installer.
Consequences: no MCP5 claim in release notes; the branch stays preserved and unmerged, with reports in `.git/coordination-recovery/MCP5-{PLAN,REPORT,REVIEW}.md`. The renderer is scheduled after the transcript milestone, using the typed payload from D-237 contract B, and is a follow-on milestone with its own acceptance, not a reopening of this review. Rebase or re-verify against main before that merge.

### D-241 · 2026-09-13 · A transcript row is a plain root
Decision: The transcript's message rows use a plain element carrying `data-message-id`, not `MessagePrimitive.Root`, and the app root has an error boundary that keeps the person in the app with a reload. The desktop copies renderer console errors into its log.
Why: 0.6.0 blanked the window when reading upwards in a real conversation: `MessagePrimitive.Root` dispatches "not hovering" into the thread state on unmount, the bounded transcript unmounts many rows per commit, and the cascade exceeded React's nested-update limit — an uncaught throw with no boundary. The synthetic fixtures never reproduced it; the person's real conversation did every time. Two further defects surfaced on the same path: commits re-placed an unmounted anchor from estimates and pushed the viewport back under the wheel, and at the top no scroll event fires so earlier pages never loaded.
Consequences: hover for actions stays CSS (`group/message`); assistant-ui's hover state is not consumed anywhere. While the person is reading, layout may shift the view by the change above their anchor and never re-place it (`reading` window, 400 ms). Wheel/swipe/↑ at the top requests earlier pages. `scripts/browser-check/test/scroll-up-blank.mjs` is the regression (huge fixture, matrix, touch) and takes `SCROLL_SESSION` for a real conversation. Shipped as 0.6.1 from `1859e4a`; published and verified by hand (Latest, 12 assets, all nine installer digests equal the published `SHA256SUMS`, provenance present) because the orchestrator's checkpoint stopped at `tag-pushed` and resume refuses a source remote main has passed.

### D-242 · 2026-09-13 · What the log store keeps is a person's disk, and must be bounded in bytes
Decision: `logs.db` retention gains a byte budget on retained request bodies, reclaims space (incremental vacuum, not a file that only grows), and shows its size with a Clear in Settings. Row and age limits stay, but they are not the binding constraint.
Why: this machine's store reached **27.7 GB in 8 days** — 25,148 `provider_request` rows holding 27.2 GB of bodies, ~1.08 MB each, because every turn re-sends the whole conversation and the whole prompt is retained. Retention is 200,000 rows / 14 days; at 138k rows and 8 days neither had ever fired, and SQLite would not have returned the space if they had. Growth was 4–10 GB/day, invisible to the person.
Consequences: the inspector keeps recent requests in full and older ones as summary only (exact policy to confirm with the person: proposed last ~50 per session). Next release after 0.6.1. Measured: stats queries over this store cost ~40 ms and run every 1.5 s while following, which is a separate thing to bound.

### D-243 · 2026-09-13 · The Logs page appends; it does not reload
Decision: The Logs page's filter identity contains only the project that is actually filtering, and the browser check `scripts/browser-check/test/logs-flicker.mjs` fails if the list is replaced, a loader appears, the view jumps on its own, or the page re-queries the store while rows stream in.
Why: a streaming turn moves the current session's cwd, which rebuilt the filters object and reloaded the page from scratch with nothing changed. Found by reading the code while chasing a reported flicker.
Consequences: fixed in `068be78`. **The reported flicker on a 27 GB store is not reproduced and this is not claimed as its cause**; the stub provider now streams in many deltas (`chunks`/`chunkDelayMs`, `fixture-stream`) so a live turn can be watched from another surface. Renderer errors now reach the desktop log (D-241), so the next occurrence can be read rather than guessed.

### D-244 · 2026-09-13 · Optimize inside Electron, from the report's remaining items
Decision: Adopt `/home/youssef/electron-optimization-report.md` as the plan for M16-T30..T33: restore hidden-window throttling, lazy-load what a first conversation does not need, prove streaming isolation with a profiler, and package only what runs. Keep Electron. Keep Shiki's full grammar catalog (each grammar is already a lazy chunk; dropping grammars loses highlighting). Defer the report's phase 4 (an Electron-served shell that loads independently of the host) and phase 6 (destroying a long-hidden renderer) to open question Q-9: both change hosting/CSP/PWA or renderer lifecycle contracts and need the person's go-ahead.
...
### D-246 · 2026-09-13 · 0.6.2 review triage
Decision: Independent review `/tmp/review-062/REVIEW.md` on `6438864` requested changes. Fixed on main by the orchestrator: B1 pre-tag resume (M16-T37's rule refused the very resume its interrupt guard prints; a `--resume` with the source contained in remote main is allowed again, the journal stage then demands main == candidate; test corrected), B3 command palette forking from a frozen view (now reads entries and leaf from the host at command time, like TopBar), M1 `summaryModel` guessing a count as a model, M3 a released row keeping its "summary only" flag after the same body returned by hash, M6 tool timeline key now includes result presence. Not changed, with reasons: M4 Clear runs the one-time VACUUM after the rows are gone, so it copies an almost empty live set (measured 63 ms at 210 MB) — the multi-GB conversion happens on the maintenance timer after releases and is logged; M2 a shared-hash summary may carry a twin's timing — the row's own `at`/summary are already on the entry; M7 the stable-list ref cache in a selector is a memo, not state; M8 the inspector's target is frozen for the dialog's lifetime by design (reopen = fresh target). Deferred to follow-up tasks: B2 file growth (`logstore.ts` 1381 lines → split; `release.mjs` 1061), M5 a named selector family for `s.open[s.current]` reads.
Why: the release is behaviour-preserving and the deferred items are structure, not correctness.
Consequences: M16-T38 (logstore/release split) and M16-T39 (selector family) added to PLAN as todo.

### D-247 · 2026-09-14 · 0.6.3 review triage
...
### D-249 · 2026-09-14 · A distinct scroll race gets a focused hotfix task
Decision: add M16-T54 and release 0.6.4 for the mounted-anchor wheel/commit race reproduced on the person's 0.6.3 conversation; do not reopen M16-T42, whose earlier-page structural shift remains correct and independently covered.
Why: M16-T42 fixed where an earlier page lands. This defect is a separate browser ordering race: a layout commit can restore the old mounted anchor after default scrolling changes `scrollTop` but before the native `scroll` event captures it.
Consequences: M16-T54 owns the one controller change, red unit, real-session trace and both-widths/both-themes browser regression. The user explicitly authorized the hotfix release.

### D-250 · 2026-09-14 · Starting a turn chooses latest
Decision: add M16-T55 to the 0.6.4 hotfix. An explicit Send from an earlier reading position returns that conversation to latest and follows its new stream; passive incoming output still does not pull a person away from history.
Why: the person reported that new streams stay off-screen in 0.6.3, and the browser reproduced it at a 1,200 px history gap. Sending is a new navigation intent, unlike output arriving while the person only reads.
Consequences: the local composer submit/click chooses latest; run state only continues an existing following intent. Passive remote starts and switching back to an already-running session preserve the reader, and ResizeObserver handles streamed growth without re-rendering the window on every delta.

### D-251 · 2026-09-14 · Project setup is one visible setting
Decision: add M16-T56 to 0.6.4 and replace Settings → Environment with Settings → Projects. Every known project is a collapsible and exposes exactly one optional Bash pre-command; the resolver form remains readable for compatibility but has no visible controls.
Why: the person asked for project-oriented settings rather than an environment subsystem. One command matches how a developer already prepares a project shell and keeps provider-specific machinery out of the product.
Consequences: empty removes the config; saving a command approves a fixed required preface config; pre-command failure blocks the requested foreground or background Bash command; parent sessions, children and worktrees share it through the project's worker.

### D-252 · 2026-09-14 · Compaction has a terminal visible transition
Decision: add M16-T57 to 0.6.4. Compacting is live state only: every successful, failed or cancelled terminal compaction path must explicitly clear it in the canonical session state and UI projection.
Why: the person observed a completed compaction whose visible state never ended. A live-state label may not rely on a later unrelated update to repair itself.
Consequences: the fix belongs at the first missing terminal transition, with reload coverage so persisted truth and live truth agree.

### D-253 · 2026-09-14 · Unowned agent stops block 0.6.4
Decision: add M16-T58 to 0.6.4 and treat any child agent becoming terminal without an explicit owner or an unrecoverable explained failure as a release blocker. Diagnose from the durable run/session/log evidence before changing lifecycle behavior.
Why: the person reports recent releases sometimes stop agents without their action. The machine records both simultaneous project-worker loss and unexplained engine aborts with no person initiator; silently losing delegated work violates the harness contract.
Consequences: every terminal path must retain its actual initiator; transient aborts must not masquerade as a person's stop or discard a live run; worker retirement/replacement must preserve the invariant that a project with live runs is not idle.

### D-254 · 2026-09-14 · Shutdown asks first and records its owner
...
### D-219 · 2026-09-11 · The stale Chat memory is cleared on the one affected machine, not migrated
**Decision.** 0.3.9 ships the startup trust fix (T115) only. The Chat dead end on a vanished remembered session is cleared on the person's machine by running their recovery script (backup, then remove `lasercode-session`, `lasercode-session-tab-last`, `lasercode-beam-session`, reload). The code fallback (T117) stays on branch `chat-memory-fallback` for a later release; no storage migration is written.
**Why.** The beta is installed on one machine; a fresh install has no remembered session, so nobody else can hit the stale memory. The person asked for the smallest release that opens the app fast.
**Consequences.** A remembered Chat session that vanishes still dead-ends until T117 lands. The recovery script is a one-machine step, recorded here, never a product surface.

### D-220 · 2026-09-11 · A child whose worktree is gone opens in the project checkout
**Decision.** A stored session whose directory was a worktree of this project that no longer exists opens with the engine's `cwdOverride` set to the project checkout. It is a normal session there, not a read-only view; nothing is recreated under `.worktrees`. A missing directory that was never this project's worktree is still left to the engine's refusal.
**Why.** Removing a merged child's worktree is the parent's job (D-157) and its transcript is history a person reads back; refusing to open it made cleanup look like data loss. The checkout is where that child's work went, so anything it does next belongs there. A read-only mode would be a new surface for one case.
**Consequences.** A reopened child's `cwd` is the project root, and the sessions list keeps it under the project. If a person prompts it again, the agent works in the checkout, which its transcript no longer describes; the role block for new children is unchanged.

### D-221 · 2026-09-11 · MCP servers are a Laser feature over the exact-pinned upstream adapter, configured only from Laser's own files
**Decision.** M14 adds `pi-mcp-adapter` as an exact-pinned worker dependency driven solely through `createMcpAdapter({ config })`. Configuration lives in `<agentDir>/<data dir>/mcp.json` (every project), `<project>/.laser/mcp.json` (this project, shareable), and a 0600 secrets file; the adapter's own file discovery, host-config imports and terminal panel are never active. Existing `.mcp.json`, `~/.config/mcp/mcp.json` and other tools' configurations are importable from Settings by explicit choice, never read automatically. The adapter is loaded into a session only when the project has at least one enabled server. Configuration changes apply to sessions started afterwards. The default exposure is direct for servers with at most 40 tools, on-demand above. Playwright's curated entry defaults to `--isolated` and a visible browser window.
**Why.** Invariant 6b: Laser owns settings and experience, Pi-native packages own logic; the adapter already implements every transport and sign-in method and is maintained upstream. Automatic reading of foreign files would run commands a person never approved in Laser; explicit import keeps the trust boundary visible. A model must not see an `mcp` tool that has nothing behind it. A running conversation's tool list is part of the conversation. Playwright's persistent profile allows one browser at a time; a fresh profile per conversation lets several sessions work, and watching the browser is the point of a desktop app.
**Consequences.** `.pi/mcp.json` remains unsupported. Secrets in `.laser/mcp.json` are references, never values. A person who wants a change now uses the project restart the host already offers. The catalog and the 40-tool threshold live in the protocol and can be revisited without touching the engine mapping.
...
### D-223 · 2026-09-12 · Post-publication verification checks digests, provenance and notes without downloading installers
**Decision.** `verifyPublicRelease` fetches two small assets — the published `SHA256SUMS` and `provenance.jsonl` — and checks: the manifest covers exactly the inventory; every asset's API-reported SHA-256 equals the manifest's; the bundle verifies once against the manifest with `--source-digest`/`--source-ref` pinned to the tag's commit; the release page body equals the notes the tag carries; Latest promotion. Installers are not downloaded. `--resume` after `tag-pushed` needs no `--notes` (the checkpoint and the tag carry them); before it, the same file is required. `publish.sh` reads a tag body only from an annotated tag and always from the repository root.
**Why.** 0.3.10's orchestrator (M13-T118) dropped the twenty-minute download and with it the only source-pinned provenance verification and the only post-publication byte check; the AGENTS.md release-blocker rule was reworded in the same commit without a decision (AGENTS.md §3.6). A clobbered asset keeps its name, upload state and a plausible size; only its digest gives it away, and the API reports one. A failing bundle would fail every `install.sh`, so verifying it once here is the cheap guard. The notes-digest gate blocked every post-tag resume once the scratch file changed (0.3.8).
**Consequences.** Post-publication verification costs a few kilobytes and seconds. A release page that is not the tag's notes, a manifest that does not cover the inventory, a digest that disagrees, or a bundle that does not verify all fail the checkpoint before it is marked verified. `SHA256SUMS`, `SHA256SUMS.sig` and `provenance.jsonl` are the three files the manifest never lists. Supersedes the "downloads nothing / checks the public inventory only" wording in M13-T118's AGENTS.md edit.

### D-224 · 2026-09-12 · A child whose worktree was removed is recovered as not isolated, from its own record
...
**Why:** Desktop launch and adopted hosts do not inherit later shell changes; the adapter currently advertises its engine identity to external servers.
**Consequences:** Regressions must inspect real subprocess environments and initialize requests, not only configuration strings; installed processes need a deliberate restart to load changed code.

### D-225 · 2026-09-12 · Only a top-level session interrupts a person outside the window
**Decision.** A child agent's session never produces a desktop notification or a phone push. The host tags `pi/session/attention` with the session's `agent` (from the run registry or the catalog) and skips the `pi/ui/request` push for a child; the desktop fleet model remembers a session's kind from a listing or an event and `shouldNotify` refuses a child. A session the host cannot classify keeps the top-level rule.
**Why.** The person asked for it: the top-level agent is the one they are talking to, and a child's finished report or question already reaches its parent inside the conversation. A banner for every finished subagent interrupts someone working in another window for news that is not theirs to act on.
**Consequences.** A child blocked on a question is visible in the sidebar (attention mark), the fleet and the live map, and its parent is told; nothing on the OS says so. If a person wants to be interrupted for a specific child, the answer is the parent's own question to them, not a child notification.

### D-228 · 2026-09-12 · Linux update restart must preserve process privileges
...
### D-260 · 2026-09-15 · A runtime may retire; mutation authority may not race it
**Decision.** Add M18-T18 before M18-T15. The host serializes a path-routed action's ensure-open and request with safe runtime unload through a bounded per-session route lease, and rechecks the complete saved-session guard after worker replacement. A release gate that does not settle is a fail-closed actionable refusal, never permission to route into uncertain authority.
**Why.** Two unchanged full runs completed RP-5b scenario 4, then failed during scenario 5 because `workerFor()` reused an `open` snapshot across awaits while the lifetime policy legitimately unloaded that runtime. The worker refused before delivery, but the host had already lost the authority it promised the action. RP-4b made real unloads reachable and exposed this latent RP-4 race.
**Consequences.** No possibly delivered mutation is retried; worker pins, release/admission fences, explicit close and missing-transcript refusals remain authoritative. T15 keeps its 200-command fixture and all ceilings unchanged and must prove repeated unload/reopen actions before the final full baseline.

### D-261 · 2026-09-16 · Full-scale pressure calibration preserves the safety arithmetic
**Decision.** Adopt B0's mechanically derived warning/critical pairs: host 512/768 MiB unchanged, project worker 1280/1920 MiB, and desktop renderer 1280/1920 MiB. Raise the worker old-space clamp floor to 1728 MiB, the first 64 MiB multiple at or above twice the exact 881,456,128-byte peak; keep its 2048 MiB cap. Use 1280 MiB as the non-Linux static aggregate reservation per worker. Keep RP-2's fixed 1,610,612,736-byte renderer ceiling unchanged.
**Why.** The final unchanged full fixture measured 840.6 MiB worker and 813.3 MiB renderer peaks, firing A1/A2/A3. The 1.5× warning and critical arithmetic preserves headroom so normal work does not trigger destructive actions. Lowering the renderer pair merely to fit a test-harness containment ceiling would weaken that calibration rule without making normal fixtures a valid critical-pressure workload. A proposed 1792 MiB worker floor exceeded A3's stated next-64-MiB requirement; 1728 MiB is the exact mechanically sufficient value.
**Consequences.** The unchanged RP-2 harness can naturally exercise at most renderer warning, never critical; focused controller/browser tests inject critical state to prove behavior without weakening fixtures or ceilings. Milestone E uses these values and records unavailable evidence as unknown, never zero. Milestone G proves ceilings last; no fixture, workload, or RP-2 ceiling changes.

### D-262 · 2026-09-16 · A worker proves its pressure generation before it reports
...
### D-265 · 2026-09-16 · Renderer pressure is actor-local; its host row stays honestly unknown
**Decision.** Milestone F keeps renderer pressure inside the window: a serialized renderer controller samples what its environment can truthfully measure, releases only its own ephemeral file cache and T5's `ViewCache.releaseUnder`, and refuses only new whole-transcript hydration. No renderer→host pressure report, no new protocol method or reach class is added in F. The host's `desktop_renderer` row remains `expected 1 / answered 0 / unknown` while a window is connected, and diagnostics derive the headline from `aggregate(host, machine, project_worker)` labelled beside a separate "This window" state. The B1 native-report extension is recorded as an optional post-E3 follow-up, not part of F.
**Why.** A page is a `local_browser` actor and cannot call the native-only `resource/report`; the host cannot bind or verify a renderer claim the way D-262 binds a worker; a renderer can read private resident/heap but never PSS, which the protocol forbids presenting as `physical`. Inventing a second journal or an unverifiable report would violate the fail-closed evidence rule.
**Consequences.** D-261's renderer thresholds remain applied where PSS is measured (RP-1 role rows and the T15 harness). F's controller must not treat heap-only evidence as physical, must fence host directives by epoch reset on reconnect, and must migrate the three windowless `pi/session/entries` callers to bounded tails. `summary.level` is not the diagnostics headline while a window is connected.
...
### D-267 · 2026-09-16 · A slope the fit cannot resolve from zero repeats as a null, not as a sign
**Decision.** RP-2 repeatability keeps every predeclared threshold (same top owner, top-five overlap, Spearman ≥ 0.8 with tie-aware ranks at the declared `max(8 B, 1.25 %)` resolution, sign agreement and between-run CV ≤ 25 % for slopes, fixed ceilings, unchanged fixtures/workloads/timing) and adds one pre-registered resolution rule for slopes, applied only to runs performed after this decision: an ordinary-least-squares slope whose absolute value is at most twice its standard error is reported as `unresolved` with its noise floor (residual SD, SE, R², n) beside it. For the pair gate, two `unresolved` slopes of the same metric pass as an equivalent null (both runs prove no drift above that floor and the floor itself must agree within the same 25 % CV); one `unresolved` and one resolved slope fail; two resolved slopes keep the unchanged sign and CV gates. Every slope sample is taken after forced collection, as RP-2 already required.
**Why.** The first two complete post-containment runs at `6230793a`'s harness passed every ceiling, coverage, survivor and ranking gate and all four renderer slopes; the only failure was host post-retirement PSS at −55,120 vs +120,071 bytes/minute with relative standard errors of 433 % and 338 % and R² of 0.013 and 0.021 over the declared 30-second, six-point window. A sign gate on a slope indistinguishable from zero is a coin flip and would fail a perfectly flat host forever, which inverts the gate's purpose; the previous harness also computed renderer slopes from pre-instrumentation, pre-GC counters, which was a bug corrected in the same commit.
**Consequences.** M18-T15 reruns the unchanged full A/B after the rule lands with tests; `docs/resource-soak-baseline.md` reports resolved and unresolved slopes separately with their floors and never presents an unresolved slope as a rate. No threshold is relaxed and the rule is not applied to the two runs that motivated it. Owner `01a0aa7f-a32f-714d-99c8-18eecb4786d1`.

### D-268 · 2026-09-16 · No Electron/Tauri decision gate
**Decision.** Drop M18-T14 / RP-14. Electron remains the desktop shell; no packaged-Electron versus host+PWA comparison, Tauri sidecar spike or keep/migrate decision is part of the resource plan or the release path.
**Why.** The person directed it. The containment work (RP-1–RP-13, RP-2 repeat) already bounds the app on Electron; a shell migration study would spend effort on a direction that is not wanted.
**Consequences.** `docs/resource-and-loading-plan.md` marks RP-14 dropped and removes it from the dependency graph; M18's remaining path is T15 then closeout; M19-T1..T3 follow. Nothing else in the plan changes.

### D-269 · 2026-09-16 · Mixed slope resolution is a label, not a verdict
**Decision.** Amends D-267. When one run's slope is `resolved` and the other's `unresolved`, the pair is judged by the original predeclared gate from the T2 finding — same sign and between-run coefficient of variation ≤ 25 % on the two ordinary-least-squares point estimates — with both resolution labels, standard errors and R² reported beside the verdict. Both-unresolved pairs keep D-267's equivalent-null rule with the residual-SD floor gate; both-resolved pairs keep the unchanged sign and CV gate. Point estimates are retained in every report even when displayed as unresolved.
**Why.** D-267's mixed-resolution failure was the orchestrator's own addition, not a predeclared gate, and it dichotomised a continuous statistic at 2·SE: the final pair at executable SHA `21a15a2a` had pagination slopes of 188,384 (2.09 SE) and 177,217 bytes/page (1.88 SE) — the same estimate within a 4.3 % CV, passing the original gate — and failed only because one sat a hair above the label boundary. Every other gate (ceilings, coverage, survivors, all eleven ranking categories, the other four slopes) passed in both runs.
**Consequences.** The comparison for the existing pair is recomputed from the retained per-run reports without a rerun, and `docs/resource-soak-baseline.md` states both the D-267 verdict and the D-269 verdict with the numbers. No threshold, fixture, workload, timing or ceiling changes. Nothing in this decision weakens the T2 gate: it is exactly that gate for the mixed case.
- 2026-09-16 orchestrator `01a0a030`: final M18 audit `/tmp/m18-final-audit.md` (72 proven / 10 measurement-weak / 0 missing). Decisions D-135 onward have lived after `## Status edits log` since earlier sessions; D-258–D-269 follow that existing layout. Nothing moved or deleted; the `grep -n "^### D-"` recipe in AGENTS.md finds every decision in numeric order.

### D-270 · 2026-09-16 · Drift is files disagreeing with their own manifest; a new manifest is a new generation
**Decision.** Amends D-266 for fixed install roots. Tampering is detected by verifying the executable files against the inventory manifest that ships beside them in the install root, never against the pointer's remembered generation. The pointer records which generation was selected and activated (with the previous id kept as history) and never refuses a launch because the install root now carries a different, self-consistent manifest: that is an update that landed, and it activates on the next launch when no host is running, or through the park gate when one is. A launch refuses only when files disagree with their own manifest (tamper/partial install) or the manifest is missing/corrupt. Runtime rollback is not a product capability on a fixed install root (the plan already excludes automatic binary downgrade); a failed first launch of a new generation says so honestly and offers Try again, data restore (M19-T3) or reinstall — never "reinstall" as the answer to a good install.
**Why.** deb/rpm/npm install into one fixed root, so the previous generation's bytes do not survive an upgrade; verifying the old pointer target after an unattended upgrade refused every subsequent launch and reinstalling could not clear it (review `/tmp/m19-t2-review.md` B1/B2). The hazard D-266 targets — an old running host resolving a newer worker from changed files — is still closed: a running host pins the generation it launched with and refuses to spawn from a root whose manifest id differs.
**Consequences.** M19-T2 fix batch: verification is against the install-root manifest; pointer semantics as above; the desktop `failLaunch` path drops the unreachable rollback branch and its fixture; bounded verification per D-266 (manifest digest + entry files + changed rows, full only on mismatch) with the bundled Node hashed once per generation, not per spawn; the "new root" method list derives from `METHOD_POLICY`; an installed `.deb` install→upgrade→relaunch cycle becomes part of the evidence.
