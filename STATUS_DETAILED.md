...
| M14-T2 | Worker engine: pinned adapter, store, inspector, sign-in, import, companion module, host routing | in-progress | worker mcp-engine (01a091cf-27d3-73a4-a484-ea10ff6969e5, `agents/mcp-engine-161a7cc9`) | — | ready at `c49220a`; under review; env-hygiene commit in flight |
| M14-T3 | Settings → MCP servers | in-progress | worker mcp-settings-ui (01a091d1-20ed-73a4-a484-ea20d2b4199a, `agents/mcp-settings-ui-cba83f3f`) | — | reviewed (REQUEST CHANGES) and fixed at `46aa88e` (UI 1166; MCP suites 54, identity clean, re-run by the orchestrator); browser review against the real backend at integration |
| M14-T4 | Transcript: MCP tool rows and image results | in-progress | worker mcp-transcript (01a091d2-7bd9-73a4-a484-ea27a8d96475, `agents/mcp-transcript-568fde54`) | — | reviewed (APPROVE WITH FIXES) and fixed at `9e47b6f` (protocol 56, UI 1136; orchestrator re-ran the three touched suites, 40/40, and inspected the batch screenshots) |
...
| M15-T4 | Sidebar activity indicators: one indicator, owned by the session itself | in-progress | worker sidebar-and-slash-fixes (01a091e1-c6e5-73a4-a484-ea4576dad59a, `agents/sidebar-and-slash-fixes-84cd3548`) | — | reviewed (REQUEST CHANGES) and fixed at `5eb2008` (UI 1128, identity exit 0, orchestrator re-ran the three suites 37/37 and inspected the folded-branch screenshots); waits on its branch for the M14 tag (D-222) |
