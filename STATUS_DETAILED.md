...
#### M16-T59 notes
- 2026-09-14 claimed from the person's direct 0.6.4 report: reproduce switching back to a long conversation whose recent 40-message tail is loaded, then make the first settled viewport show the latest message. Work stays isolated on `fix/session-tail-reentry`; concurrent M18 planning and source owners remain untouched.
- 2026-09-14 reproduced red in `transcript-viewport.test.ts`: after A → B → A and a replacement tail with unchanged geometry, expected bottom 5,490 but remained at 0. Initial finding: `configure()` scheduled restoration without setting `windowDirty`; marking the path/revision dirty fixes that reduced sequence.
- 2026-09-14 independent diagnosis `run_6aad812d` found the full deterministic ordering: main navigation awaits the recent-tail read, so the returning path and new `historyRevision` reach `configure()` together. The different-path branch adopted that revision and restored the old window's `following:false` anchor; missing-anchor fallback mounted index 0 and native clamp made it stable. Saved places now carry their accepted window revision and are discarded only when that revision was replaced. Same-revision passive reading preservation remains unchanged. Focused 28/28 passes with the real resolving → path+revision sequence and later agent-output following.
- 2026-09-14 first browser invocation correctly reproduced the visible checkpoint-101/top state but used the prior compiled UI bytes; it is reproduction evidence, not fix validation. After rebuilding, `transcript-reentry.mjs` passes 4/4 at 1360/390 × dark/light (`/tmp/browser-check/run-lmvMEW`): return gap ≤4 px, newest row in the viewport, one `{tail:40}` read, checkpoints 101–120 retained, questions/explicit destinations/drafts preserved. `stream-start-follow.mjs` also passes 4/4 (`/tmp/browser-check/run-ovzidF`), including Send from history, every sampled stream frame at latest, and deliberate wheel opt-out.
- 2026-09-14 review `run_b1f38458`: revision ownership is the right invariant and passive starts/same-window anchors remain intact. Addressed both requested guards: same-path replacements with unchanged geometry are covered; `ensureVisible()` now explicitly supersedes arrival so a clamp cannot turn following back on during find/map/action navigation. Final evidence: focused viewport 29/29; UI 1,670/1 skipped; typecheck and identity pass; exact rebuilt bytes pass re-entry 4/4 (`/tmp/browser-check/run-xZ8b2P`, all placement gaps 0) and streaming-follow 4/4 (`/tmp/browser-check/run-gnWbi4`).
- 2026-09-14 done: v0.6.5 public from source `def7cee` and candidate `a9839b3`; source CI 34858465171; release run 34859087774 attempt 2; 12-asset inventory, digests, provenance, notes and Latest verified. Attempt 1's x64 clean-machine check hit a transient 251 ms result at its 250 ms pure-regex boundary; ARM64 passed, failed-job rerun passed unchanged and publication remained blocked until it did. https://github.com/youssefsiam38/laser/releases/tag/v0.6.5
...
| M16-T59 | A reopened conversation starts at its latest message | done | orchestrator-01a09ea5 | v0.6.5 candidate `a9839b3`; CI 34858465171; release 34859087774; 12 assets | see notes; D-257 |
...
### D-257 · 2026-09-14 · Ordinary session re-entry means latest
Decision: add M16-T59 and release 0.6.5. Returning to a conversation opens at the latest message in its bounded recent tail; an old viewport position from a prior visit never wins over that explicit navigation.
Why: 0.6.4 correctly requests the latest 40 messages, but main navigation awaits that read before publishing the returning path. The path and new window revision therefore reach the viewport together; it restored an anchor saved against the discarded window and stabilized at the new tail's first row.
Consequences: a saved reading place is valid only for the accepted history-window revision it was captured against. A replacement revision chooses latest and triggers placement even when dimensions are unchanged. Same-revision passive updates to the session already on screen continue to preserve reading position.
