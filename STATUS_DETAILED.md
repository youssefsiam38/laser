...
| M4-T7 | Keybindings + trust views | in-progress | lane-E | `pnpm -r build` exit 0 · `pnpm -r typecheck` exit 0 · `pnpm -r --no-bail test` 675/676 (the one failure is M11-T1's globals.css assertion, pre-existing); `packages/ui/src/components/settings/KeyboardTab.tsx`, `packages/ui/src/components/settings/TrustTab.tsx` | Trust view is complete and writes only through the host's registry (`actions.answerTrust`), never Pi's `trust.json` — `packages/host/src/trust.ts` reads that file and by design never writes it. Keyboard view lists the app's own bindings, read-only, and says so; Pi's `keybindings.json` binds Pi's TUI, which this app never loads, and editing it needs a `pi/keybindings/*` method that does not exist. See M4-T7 notes |
...
| M11-T6 | Themes persist where settings live | blocked | lane-E | — | Blocked on a protocol method. `pi/settings/set` refuses any key the pinned Pi does not define (`packages/worker/src/settings.ts` says so by name), so a piorbit theme cannot travel through it, and no host-owned preference channel exists. Today the theme persists per browser in `localStorage`, and Appearance offers Copy/Paste of the theme JSON as the manual path. The `pi/prefs/get|set` diff is in the lane-E report under REQUESTS |
