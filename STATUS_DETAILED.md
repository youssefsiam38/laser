...
| M13-T89 | Make pre-turn agent choice tentative until first send | in-progress | harness-lifecycle | — | coupled batch frozen `aba12f7b`, unmerged and approval withheld; D-191; see notes |
| M13-T90 | Align composer controls and recording affordance | in-progress | composer-layout | — | frozen `e7a5b89`; accepted staging only; D-188; see notes |
| M13-T91 | Show the session’s persisted agent in the header after start | in-progress | header-agent | — | code-approved `a8715dd`; final browser gate incomplete; D-188; see notes |
| M13-T92 | Replace stale Sending state with the queue state | in-progress | harness-lifecycle | — | backend `67bf567` on main; coupled batch `aba12f7b` unmerged; D-191; see notes |
| M13-T93 | Serialize child completion, engine settle and resume | blocked | harness-lifecycle | — | HLC010 needs person-approved awaitable extension-prompt seam; Q-8; see notes |
...
#### M13-T93 notes
- 2026-09-10 end-session blocker: coupled development commit `aba12f7b1912144d1d041f057aa643acf143aecd` is clean and frozen. HLC001/002/003/004/006/007 were independently resolved; HLC008 explicit provenance/default-model and HLC009 actual-state rollback were corrected in `c6a29fb`/`aba12f7b`. Latest reported worker evidence is 121 focused tests plus typecheck, identity and diff check; exact final backend confirmation is being recorded separately. Combined approval is withheld for HLC010.
...
Evidence: preliminary full verify passed at `6b69f5e` before header/neutral/lifecycle. Exact `adcbd66` focused checks passed: 58 UI, 11 worker fleet, identity and diff check. Header `a8715dd` passed five focused tests, types and identity. Coupled backend reported 121 focused worker tests, typecheck, identity and diff check; exact final backend confirmation is being recorded separately. Full current verify, browser and package gates were skipped at the person’s request, so release readiness is not established.
