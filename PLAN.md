...
| M13-T108 | Reopen empty sessions safely across worker retirement and restart | reproduce Beam and ordinary-project empty New Session reopen/load/first-send after worker retirement and host restart; fix canonical empty-session lifecycle without inventing transcripts or recreating arbitrary missing paths; retain identity/agent/model context where authoritative, preserve drafts, avoid duplicate stale-session errors; regress saved-session recovery and both session types; independent review and isolated tests |
