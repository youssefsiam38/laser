...
| M13-T93 | Serialize child completion, engine settle and resume | a child completion is recorded once, the harness waits for the actual session turn to settle before any ended-agent resume, and the run registry and live session cannot disagree or enter extra completion/follow-up loops; focused harness tests pass without overlapping pending-delivery driver/server ownership |
| M13-T94 | Release the next stable patch | M13-T89 through M13-T93 pass staged workspace and packaged checks; clean source CI precedes the immutable patch tag and verified publication workflow |
