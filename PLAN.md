...
Goal: implement `docs/resource-and-loading-plan.md` in dependency order. Authoritative transcripts stay host/worker-owned, actions remain host-first, dormant state is bounded without ending live work, and a previously seen conversation paints from a bounded device cache before authoritative reconciliation. Decisions D-255 and D-256.

Dependencies: M16-T16, M16-T21, M16-T22, M16-T29, M16-T30 and M16-T31 are done. Tasks below follow the RP dependency graph in `docs/resource-and-loading-plan.md`; RP numbers are preserved as task suffixes. D-257 splits RP-2's post-containment repeat baseline into M18-T15 so its safety refusal can unlock the fixes it measured without weakening the gate.
