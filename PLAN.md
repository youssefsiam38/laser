...
| M21-T1 | Protocol domain, revisions and transition rules | M21-T0 | closed schemas for every primary/supporting record, `ProjectWorkRef`, `RepositoryLink` and its state/change targets, states, dependency edges, stale propagation and transition tests; no session owner field |
| M21-T2 | Stable project identity and canonical store | M21-T1 | transactional project-partitioned store, migrations, integrity, stable repository ids across relocation/worktrees, project removal retention, worktree parent resolution, blobs, bounds and crash tests |
...
| M21-T6 | Embedded workspace shell | M21-T5 | `/spec`, `/research`, `/design`, `/plan` from any session with the project picker in projectless Chat and the command text as the whole input (D-352); Specs entry, tabs, list/detail/inspector composition, preserved conversation, deep links and every empty/loading/error/stale/offline state at desktop and phone widths |
