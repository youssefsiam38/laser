...
| M16-T25 | The best MCP client there is | every recommendation in the MCP client best-practices guide (2026-07-28) implemented, not cherry-picked: progressive discovery is the DEFAULT for every server with a per-server Advanced override; a context-share threshold decides injection; catalog/inspect/execute layers with multiple detail levels; host-side definition cache honouring `ttlMs`/`cacheScope` and invalidated by `list_changed`; tools grouped by server; prompt-cache-preserving injection (append after the breakpoint or one stable call-tool meta-tool; disconnects treated as conversation boundaries); dynamic server enable/disable from the model with a registry of available servers; programmatic tool calling with a real sandbox, per-call authorization inside a script run, no network, no credentials in generated code, resource limits and output filtering, `isError: true` surfaced as a thrown exception; research of how Claude Code and Codex implement MCP recorded with what we adopt and what we reject |
