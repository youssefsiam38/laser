...
- **Project-owned lifecycle**: Specs, Research, Designs, Plans and Tasks are
  durable project entities in one embedded workspace. They do not belong to a
  session; any session may mention them, and execution sessions link back to
  them without becoming their authority.
...
- Specs, Research, Designs, Plans and Project Tasks belong to a project, never
  to a session. Sessions carry typed references to immutable revisions.
- React is Laser's design-workspace engine, not the target framework. The
  canonical design tree is framework-neutral; native validation and production
  code use the project's actual framework and design system.
...
| M13-T122 | Agent definitions are Markdown files | custom agents live as `<name>.md` (YAML frontmatter + instructions body) under `<stateDir>/agents/` (global) and `<project>/.laser/agents/` (project, trusted projects only); the host watches both, a hand edit is live within a second, a broken file is an `AgentWarning` (field `file`), the Agents page writes the same files; `agents.json` v1 migrates once (agents → files, other state stays in `agents.json` v2) with the old file kept as a backup; a project agent is offered only to that project's sessions and shadows a same-named global one there; `defaultAgent` is always global |
| M13-T123 | Laser's core instructions prepend every custom agent | `packages/worker/src/agents/core-instructions.md` is shipped in the worker's dist and the desktop package and rendered (same template fields as target `agent`) ahead of the agent's instructions — its own or the shipped default — for every custom agent unless `excludeCoreInstructions`; never for a built-in; provenance attributes the core block separately; the worker selects global + its own project's definitions from the snapshot |
| M13-T124 | Agents page: scope, file and core-instructions controls | new-agent form chooses Global / This project; list shows scope and the definition file path; "Exclude Laser's Core Instructions Prompt" checkbox (unchecked by default) in the custom agent's Instructions section, absent for built-ins; a project agent is listed only while that project is open; browser matrix green in both themes and widths |
| M13-T125 | The agent labels its own activity | every tool the engine offers except `start_agent`, `complete_agent_run` and `inspect_fleet` carries an optional `label` parameter (≤25 chars, D-277); the worker strips it before the tool runs, keeps it in the stored call, and never records it as an argument; `start_agent` rows use `subagent_name`; Namer names new sessions only — its tool-labelling path, prompt fields and `lasercode/namer/label` message are gone; core-instructions asks agents to fill `label`; tests cover injection for built-in, harness, background and MCP tools, stripping, and a missing/over-long label never refusing a call |
| M13-T126 | Activity rows read the agent's label | running tool rows (tool, MCP, group aggregate) show `toolCallLabel(name, args)` and fall back to the neutral copy; `label` is hidden from every args disclosure and JSON view; old sessions render unchanged; the `namerLabels` view state is removed; browser matrix green in both themes/widths with a streaming labelled tool |
| M13-T129 | A parent's message reopens a released child | `send_agent_message` resolves its child from the durable run record like every other agent verb; a child the host released while idle (RP-4) is reopened from its canonical session file through `SessionHost.reopen` and takes the message as a new run; `no_such_agent_session` is reserved for an id no run under the caller ever had, `agent_session_unavailable` names a child that exists but cannot be reopened, and neither ever advises `start_agent` for a merely cold session; harness regression test, `docs/agents.md` rule |
| M16-T61 | The Chat tab never waits | the Chat tab opens on an unstarted composer at once — first send creates the chat session and delivers the message in one motion, exactly as the Code tab's project landing; reopening a remembered chat keeps the composer live and queues the send until the session is ready (no "Preparing Chat…", no blocked send); which chat is open is remembered for this app run only, so the first visit after launch is a fresh chat; a new reply, question or approval in the other tab's current session marks that tab with an attention dot (accessible name updated), cleared when the tab is viewed; browser matrix green |
| M16-T62 | A trimmed conversation always reads back | the copy "Earlier messages are not loaded here. Open this conversation again to read them." never renders; a view whose older turns were released (RP-5b §7) keeps a working "Load earlier messages" path that re-mints its cursor from the authority on demand, with no read cap; reloading is idempotent and cheap; tests cover repeated reads and a release mid-read |
| M16-T63 | The @ picker selects folders and understands people's paths | choosing a folder in the `@` picker inserts it as the mention (Enter/click), `/` still descends; `~`, `~/x`, `./`, `../`, absolute paths and Windows spellings (`C:\`, backslashes, `%USERPROFILE%`) resolve in the query, the picker and the inserted mention; host explorer resolves `~` against the home directory and refuses paths that leave the allowed roots with a sentence; tests per convention |
| M16-T64 | Load more loads, and says how many | every click of "Load more" in the sessions list either reveals rows or says why not — never a silent no-op; the control reads "Load 7 more" / "Load 3 more" from what the catalog knows is left (or "Load more" only while the count is unknown, with the count filled in as soon as a probe answers); cursor/reveal races fixed with a test that clicks twice quickly |
...
| M16-T29 | Long conversations stay fast after use | depends on M16-T16 and M16-T24; repeated short↔long and long↔long switches remain responsive after paging to old history and after a session grows live; bound mounted transcript work independently of loaded history without deleting canonical data; preserve within-session reading position, drafts, focus, disclosures, approvals, find, versions and original-entry actions; ordinary session re-entry resets to the latest ~40 messages with older-page loading available again, not the previous expanded history/reading position; explicit navigation targets override that default (D-236); compare before/after browser median and p95 at 40, 240 and 2,000 loaded messages for sustained browsing/live growth/outgoing cleanup, plus equivalent recent-tail re-entry after each history size; desktop/phone, both themes; implementation design approved from a measured reproduction (D-231), re-entry policy revised by the person (D-236) |
| M16-T28 | `@` is a file explorer, not a guess | typing `@` opens a global path-aware file/folder picker: `@/` lists the filesystem root, absolute paths and parent traversal use OS-account access, ignored/generated folders remain discoverable (D-232); `@name` and `@./name` filter the current directory by prefix, `@server/` lists that directory, `@a/b/c` walks as it is typed; folders continue the path instead of completing it, files complete the mention; keyboard throughout, both widths and themes; large directories bounded and never blocking the composer |
| M16-T30 | The hidden window costs nothing | depends on M16-T21; the desktop window keeps Chromium's default background throttling (no `backgroundThrottling: false`); with the window hidden in the tray, agents keep running, native notifications still arrive and withdraw, and on return the renderer shows the correct live state (streaming text, run status, elapsed labels, catalog, drafts) without a burst of stale animation; a reconnect that fell due while hidden completes promptly on show; desktop startup resolves the identity and the login-shell environment concurrently; the desktop log records a startup timeline (process entered, window created, ready-to-show, host ready, page loaded) for evidence |
| M16-T31 | The first conversation loads only what it needs | depends on M16-T21; a production bundle analysis is recorded in `docs/perf-bundle.md`; KaTeX and its CSS load only when a message actually contains math; feature surfaces not needed for a first conversation (appearance editor, MCP configuration/gallery, agent editor, API request inspector, source details, conversation map) are separate chunks loaded on first use; no lazy chunk is preloaded before it is needed; one Shiki highlighter instance serves every fence and grammars still load on first use with the full catalog retained; the host marks Vite's hashed `assets/*` immutable so Chromium keeps the bundle's bytecode cache between launches; initial JavaScript bytes and parse/execute time measured before and after in the browser harness; no visible behaviour, motion or search fidelity changes |
| M16-T32 | Streaming touches only the streaming row | depends on M16-T11 and M16-T21; a React-profiler measurement in the browser harness proves that a streamed delta commits only the active message row (settled history, sidebar, fleet, workbench: zero renders per delta); no repeated long tasks over 50 ms during streaming with the transcript window loaded; renderer heap and DOM node count return toward baseline after repeated long↔short session switches; regressions found are fixed only from the traces, preserving every delta, approvals, partial tool output and the exact final transcript; results recorded in `docs/perf-streaming.md` |
| M16-T33 | The package carries only what runs | depends on M10-T3; the packaged dependency tree drops proven-unreachable files (workspace `src/`, `test/`, `public/`, `tsconfig*` and `*.tsbuildinfo` of `@lasercode/*` packages, and any dependency payload proven unreachable by reading its loader, `exports` and `files`) without excluding executable TypeScript by extension; every removal is justified in `packages/desktop/electron-builder.yml` comments; `node packages/desktop/scripts/clean-machine.mjs` passes on the packaged Linux build with a real session, the model list and the MCP check; unpacked and installer sizes recorded before and after |
| M16-T34 | The log store is bounded in bytes and visible | depends on D-242; a byte budget bounds retained request/response/tool bodies, enforced at ingestion (not only on a timer), with row and age limits kept; full bodies stay for the most recent 50 provider requests per session and older rows keep a summary (model, message count, size, timing) with the body released (D-245); space is really reclaimed (incremental vacuum) and proven by a temporary store shrinking in a test; pruning never releases a body a retained entry still points at; Settings shows the store size (`pi/logs/stats.bytes`) and a Clear written for a person, tokens only; vacuum never blocks the request path measurably |
| M16-T35 | The API request modal does not blink while the session streams | the inspector freezes the target it was opened with (at/beforeAt/entryId resolved once at open), re-queries are additive (current entries and selection kept, loader only when nothing is shown yet), the payload effect keys on `entry.id` + `detailRef.ref`; a browser check modelled on `logs-flicker.mjs` reproduces red then green: with the modal open while a turn streams and settles, no loader reappears, the selection does not change, no re-query/refetch of an unchanged capture; Refresh still offers a new capture and opening on another message shows that message; no debounce, no hidden loader |
| M16-T36 | Phone dark 2,000-message re-entry meets its budget | depends on M16-T29; returning to a 2,000-message conversation at 390 px dark meets 300 ms median / 500 ms p95 typing readiness on the same build with motion on, without relaxing the budget or removing the transition (reduced motion remains a fallback that loses only movement); 20 paired samples redone on the final bytes with `transcript-window-phone.mjs` |
| M16-T37 | The release orchestrator resumes after the tag | depends on D-223; a resume after the tag exists completes verification and checkpoint stages although remote main advanced to the candidate the release created; the pre-push fast-forward check is unchanged; a test in `scripts/release/test` covers that ordering |
| M16-T38 | Split the log store and the release orchestrator | `packages/host/src/logstore.ts` split along its section comments (bodies, reclaim, summary) and `scripts/release/release.mjs` likewise, semantics identical, every test unchanged and green |
| M16-T39 | One named selector family for the current session | every `s.current ? s.open[s.current]?.X` read in `packages/ui/src` goes through named selectors in `runtime/presentation-state.ts`; a test pins that no component reads entries/leaf through `samePresentationView` |
| M16-T40 | The release orchestrator can abandon an untagged candidate | `release.mjs` offers an explicit `--abandon` for a checkpoint below `tag-pushed` whose exact-SHA CI failed: archives the checkpoint, releases the lock, never touches the tag or remote main; refused at or after `tag-pushed`; a test covers it |
| M16-T41 | Release 0.6.2 | the Electron performance lanes and the open-problem fixes are public as a verified release |
| M16-T42 | Reading upwards keeps the reader's place when a page arrives | depends on M16-T29; when an earlier page is inserted above the anchor, the viewport moves by the inserted rows' height before the next paint and the window for that render is computed around the anchor; one wheel notch at the top reads a message or two, never lands on the top of the new page; `scroll-up-repeat.mjs` (paced `SLOW=1` and burst) passes on the matrix |
| M16-T43 | Prose after a formula does not crash the window | the math plugin is held as state through a lazy initializer that returns it; a message rendered after one that loaded KaTeX renders; the error boundary logs the JavaScript stack beside the component stack |
| M16-T44 | Review batch: worker | findings #2 #5 #6 #8 #12 #13 #14 #25 #34 #35 #36 #37 #38 of the 25130e9 review fixed with a test each; worker suite green |
| M16-T45 | Review batch: host | findings #1 #9 #10 #11 #23 #24 #29 #30 #31 #32 #33 fixed with a test each; host suite green |
| M16-T46 | Review batch: relay, crypto, protocol, companion | findings #3 #26 #27 #28 #7 #39 #40 #41 #42 fixed with a test each; suites green |
| M16-T47 | Review batch: UI runtime, dialogs, PWA | findings #4 #18 #19 #22 #43 #44 #46 #47 #48 fixed with a test each; UI suite green |
| M16-T48 | Review batch: UI components | findings #15 #16 #17 #45 #49 #50 fixed with a test each; UI suite and matrix green |
| M16-T49 | Review batch: desktop | findings #20 #21 fixed with a test each; desktop suite green |
| M16-T51 | A thinking or model change never locks the composer | on a conversation that has started, `pi/thinking/set`/`pi/model/set` apply at once (the engine takes them for the next turn) instead of waiting behind a queued steer's admission lease; only a pristine session waits for the first-turn fence; the composer's preparation gate is per session, so a change still waiting for one conversation never leaves another's composer inert; worker and UI tests red/green |
| M16-T52 | Forking a parent re-keys its children | after a parent session forks, every child entry's `parentPath`/`rootPath` and every run's `parent.sessionPath` follow it; `inspect_fleet` from the forked parent still lists its children; test |
| M16-T53 | Settings changes keep their order against session moves | `pi/model/set`/`pi/thinking/set` are serialised with fork/navigate/compact on the same session without waiting on a queued steer's lease; a change captured before a move reports the moved session's truth; `feature/set` refusal names the project and the waiting child; tests |
| M16-T54 | Scrolling up is steady in 0.6.3 | the person reports scroll-up still buggy after 0.6.3: reproduce against the paced/browser harness, fix the concrete defect(s) with red-first tests, paced matrix green; hotfix release 0.6.4 |
| M16-T55 | A new stream returns to latest | sending from an earlier reading position moves to the new prompt before its answer streams and follows the growing answer; a reader who only scrolls up remains undisturbed; red browser proof and matrix |
| M16-T56 | Projects own their Bash setup | Settings replaces Environment with a project-wide Projects tab; every known project is a collapsible with exactly one optional command run before each Bash command in the project's parent sessions, children, worktrees and background tasks; empty disables it and a non-zero setup status blocks the requested command; UI/worker/background tests and browser matrix |
| M16-T57 | Compaction ends visibly | after compaction finishes or fails, the session and composer leave Compacting immediately and remain correct after reload; reproduce the stale state, fix its owning transition, and cover worker, reducer and browser behavior as applicable |
| M16-T58 | Agents never stop without an owner | a live child agent cannot become terminal because of an unexplained abort, client disconnect or idle retirement; every ending names its real initiator, transient provider failures recover without losing the run, and the observed released-build failures have red-first worker/host regressions |
| M16-T59 | A reopened conversation starts at its latest message | returning to a previously opened long session may load only its recent 40-message tail, but the first settled viewport shows the end of that tail rather than its beginning; switching between long sessions is covered red-first in the viewport and real browser at desktop/phone widths and both themes; release 0.6.5 |
| M16-T50 | Release 0.6.3 | scroll-up place keeping, the window crash and the review batch public as a verified release |
| M16-T27 | The session the app just created is the session it shows | the intermittent failure in `catalog-arrival.test.tsx` is explained and fixed at its cause, not by retrying or loosening the test: when a session created outside the app arrives in the catalog while its project screen is up and a prompt is routed to it, the view really becomes current; a stress run of the suite passes repeatedly |
| M16-T26 | The first click is not a cold start | a project's worker is ready before the person opens a session in it: readiness is prepared on the signals that precede the click (the project is selected, its group is visible/expanded, the pointer rests on a row, the app restores a remembered project), bounded so an unopened project never pays for it; one worker per project and every retirement/lifecycle rule intact; a live run is never disturbed; measured on the forensics harness: short-session open from cold 744 ms → ≤ 200 ms, with the cost of the pre-warm itself reported |
| M16-T25 | The best MCP client there is | the MCP client best-practices guide (2026-07-28), except dynamic server management which the person explicitly rejected (D-230): progressive discovery is the DEFAULT for every server with a per-server Advanced override; a context-share threshold decides injection; catalog/inspect/execute layers with multiple detail levels; host-side definition cache honouring `ttlMs`/`cacheScope` and invalidated by `list_changed`; tools grouped by server; prompt-cache-preserving injection (append after the breakpoint or one stable call-tool meta-tool; disconnects treated as conversation boundaries); programmatic tool calling with a real sandbox, per-call authorization inside a script run, no network, no credentials in generated code, resource limits and output filtering, `isError: true` surfaced as a thrown exception; research of how Claude Code and Codex implement MCP recorded with what we adopt and what we reject |
| M16-T24 | Click to conversation, accounted for millisecond by millisecond | an end-to-end attribution of the time between clicking a session row and the conversation being readable, on the real built app through `scripts/browser-check`, at 4 / 240 / 2,000 messages and with a cold and a warm worker: every phase named and measured (click handling, destination resolution, `session/load` inside the engine, entries read and parse, store hydration, projection, React commit, markdown/highlighting, fonts/images, paint), with the three largest causes proven by removing them in an experiment, not inferred; a written verdict on what must change and where, and whether M16-T16's tail-first design is sufficient or the engine's own load is the wall |
| M16-T23 | One browser-check harness every agent uses | `scripts/browser-check/` — one documented entry point that starts an isolated host + stub provider + Chrome from the built app, seeds a chosen fixture (empty, short, long, tools/reasoning, agents, MCP), exposes a small scripting API (`open(path)`, `viewport(width)`, `theme(dark|light)` set through the app's own preference, `shot(name)`, `touch`, reduced motion, `rpc(method, params)`), writes every artifact outside the checkout, and tears everything down; scrubs `PI_*`/credentials and never touches the person's agent dir or browser; documented in `AGENTS.md` as the way to do browser acceptance, with a `--matrix` mode that runs 1360/390 × dark/light in one call |
| M16-T23 | One browser-check harness every agent uses | `scripts/browser-check/` — one documented entry point that starts an isolated host + stub provider + Chrome from the built app, seeds a chosen fixture, exposes a small scripting API (open, viewport, theme through the app's own preference, shot, touch, reduced motion, rpc), writes artifacts outside the checkout, tears everything down, scrubs credentials, and offers a `--matrix` mode for 1360/390 × dark/light; `AGENTS.md` names it as the way to do browser acceptance |
...
| M16-T77 | Autonomous goal recovery | Missing goal tools repaired safely and restore contention retryable, without pause/spin, duplicate continuation or weakening explicit cancellation. Pinned policy regressions and real-engine evidence pass. Scope clarification D-290: restore-contention acceptance is split to dropped M16-T80; T77 retains tool repair and adds ordinary-message usability during tool failure. |
...
| M16-T79 | Publish the agreed repairs | Depends on T70, T73–T78, T81, T83 and T84 (T82 deferred to 0.9.3 under D-301). Independent reviews resolved, staged identity/full verification green, exact-source CI and routine release transaction complete; public asset inventory, digests, provenance, notes and Latest verified. |
| M16-T80 | Multi-workflow goal restore | Split from T77: restore contention retryable, without pause/spin, duplicate continuation or weakening explicit cancellation. Dropped under D-290: no second workflow-mutex participant exists in the product; revisit only with a supported competing workflow. |
| M16-T81 | Earlier history survives live paging | Investigate the reported live child session read-only; reproduce upward paging/history disappearance with live appends, compaction and bounded/stale-cursor recovery. Earlier-message requests must advance backwards without silently replacing the reader's loaded window with the newest tail; preserve anchor, branch/revision fences and resource bounds. Deterministic regressions and real pointer/keyboard/touch browser evidence pass. |
| M16-T82 | Active-chat images remain accessible | Valid requested images remain viewable despite background/offscreen cache pressure, ordinary admission limits, live revision movement and session revisit. No permanent disabled memory placeholder for recoverable data. Prioritize active/visible/user-requested content, retain genuine safety and identity protections, and prove decoded images/captions/opening/retry across code-guided parent-owned acceptance scenarios, beyond code review. Coordinate sequential implementation with T81's history/residency owner. |
| M16-T83 | Reading upwards is continuous and the scrollbar is honest | Continuous wheel, trackpad, touch and keyboard reading upwards through a long, tool-heavy conversation never shows a section twice, never moves the reader back down, and never leaves the reader at the top of a freshly arrived page; measured anchor delta stays below one pixel per frame while pages arrive and rows are measured. The scroll range reflects the unloaded earlier conversation from producer counts and measured turn heights, so the thumb size and position mean what they show; reading into that reserve loads the pages on demand behind a designed loading state, without jumps. Regressions reproduce on the pre-fix source with the real-session and synthetic lanes of `scroll-up-repeat`, then pass on pointer, keyboard and real touch at both widths and themes. Depends on T81's history contract; must not reintroduce tail replacement or ambient trims. |
| M16-T84 | Full reply, reasoning and message bodies read as Markdown | The full-body viewer opened from a reply, reasoning or prompt renders the body with the transcript's Markdown renderer (headings, emphasis, lists, code with highlighting, tables, links, math), in both themes and widths, with the person's detail preferences; Find, Copy, Download, wrap, Home/End and keyboard reading keep working on the rendered document; a Plain text toggle shows the raw body; bodies larger than the renderer can hold at once fall back to the paged plain viewer with that said in the person's words. Reasoning summaries that arrive as separate segments render as separate paragraphs, never glued into one line. Tool output, requests and JSON keep the terminal/plain viewer. Interaction tests plus a browser check at both widths and themes. |
| M16-T85 | Reading upwards never pushes the reader back | Scrolling up through a long real conversation moves only upwards: no commit writes `scrollTop` against the person's own movement, an arriving page keeps a mounted anchor row's measured position within one pixel, and a reader inside the unloaded estimate sees rows appear where the placeholders were without any scroll write. Compensation is measured from a stable-keyed row's rect in the layout phase, never from estimated geometry or `scrollHeight` deltas, and exactly one system adjusts per commit. A page whose boundary row is not a rendered entry is kept. Deterministic replay of the real session's shape (20+ pages, mixed measured/estimated heights, bounded estimate) proves `scrollTop` never increases; D-287 follow behaviour unchanged. |
| M16-T86 | A chosen file mention is finished | Choosing a file, folder or agent with `@` ends that query: the picker does not reopen while the person keeps typing, the chosen mention reads as one tag with its name and kind in both themes and widths, and the submitted text is exactly the mention form it always was. Editing inside a finished mention returns it to ordinary text; a fresh `@` opens the picker again; inner-space queries, `/`, Tab, Enter and Escape are unchanged; Backspace only deletes (D-299). Interaction tests cover several mentions in one message, editing, pasting and the payload. |
| M16-T87 | Transcript scrolling rebuilt on a proven virtualizer | Replace the bespoke window/anchor arithmetic with `@tanstack/react-virtual` (MIT, exact-pinned) per `docs/transcript-virtualization.md`: stable message-id keys, measured rows, eager same-pass anchor resolution on prepend, follow-on-append only from the end, one authority for scroll adjustment. Laser keeps history paging and its fences, the placeholder for unloaded history as measured items, pins for selection/focus/Find/approvals, `content-visibility`, reduced motion. Done when reading up a long real conversation shows no row appearing or disappearing under the eye, no row arriving before the reader reaches it, every older page reachable, the reading position never moving back, and Find/selection/deep links/Jump to latest unchanged. |
| M16-T95 | New Session is immediately usable | Main-window New Session shows the existing project/Chat landing with an editable focused composer on the next render. Empty-session reuse, `session/new` and worker start run in the background; first Send joins that in-flight work and never allocates twice or loses the draft. A newly created session is adopted without `session/load`. A background failure leaves the draft and keeps Send retryable. A late result after navigation does not steal the destination. D-127 reuse (current/newest genuinely unstarted session, including draft and model) is preserved. Existing-history opening is unchanged. Beam bubble prepare-on-open is unchanged (D-334). |
| M16-T99 | A new chat is local until the person speaks | Opening a new session paints the finished landing on the next frame and then never changes it on its own. Nothing an empty state shows — the workspace name, the eyebrow, the sentence, the suggestions, the composer, the microphone — waits for or reacts to a response: the session request goes out silently and its arrival causes no remount, no reflow and no text swap. The microphone is present and pressable immediately; its provider and permission checks run on the press, and only a real failure is ever shown, in the person's language. The landing is identical before and after the session exists, proven frame by frame in a browser at both widths and both themes (D-341). |
| M16-T100 | Scrolling up survives a compaction | A base revision captured before a compaction or branch-summary record, whose fold still proves a byte-identical prefix with its leaf on the branch, resolves to its state with `barrier` set: `before`/`beforeEntry` pages are served against it by both the live worker and the durable host reader, while a suffix merge on that base is still replaced. The transcript's upward reading no longer dies with -32007 after an auto-compaction; host/worker tests cover the barrier and the moved-branch refusal |
| M16-T97 | A long transcript fills itself | Opening a conversation leaves no estimated space on screen: the newest page is requested in user-anchored turns (`HISTORY_FIRST_PAGE_TURNS`/`HISTORY_EARLIER_PAGE_TURNS`, already in the protocol), and paging continues without a gesture until at least two screens of real rows sit above the reading position. A page that legitimately shrinks to a handful of rows keeps paging instead of stopping. Reading position, focus and `prefers-reduced-motion` behaviour are unchanged, and paging stays bounded by an explicit byte/page budget. A background refresh whose base went stale (a compaction, or more entries appended in one turn than the worker retains checkpoints for) accepts the replacement page rather than putting the view into a refusal the person did not cause (D-340). |
| M16-T98 | Other versions without the whole transcript | A person reaches the other versions of a prompt from that prompt, through a bounded history window for an entry's siblings, never by loading the entire tree. `{ all: true }` leaves every person-facing path — versions, the message menu, Find, Edit/Fork/Jump and background refresh — so a conversation larger than one page can no longer show a control that always fails, and the API-caller sentence about reading a page at a time never reaches a person. The new window carries a schema round-trip sample and router coverage, and is served by both the live worker and the durable host reader (D-340). |
| M16-T96 | Unread means a new message, not a file flush | The hollow session-row dot (`finished_unread`) follows catalog message count and turns that ended while nobody was looking, not jsonl mtime. A look records the count; a later mtime with the same count stays idle. Opening or leaving a session without a new message does not bring the dot back. The visible row goes idle as soon as the transcript is acknowledged. `waiting_for_input` and unseen errors stay marked. Old `attention.json` without a count keeps today’s mtime behaviour until the next look (D-335). |
...
| M15-T8 | Compact before rejecting a smaller fallback model | When an access failure opens fallback and an otherwise usable candidate is blocked only because the current conversation exceeds its context window, auto-compaction (when enabled) runs once under that candidate and the same accepted turn continues. Success never renders the fallback-exhausted warning; failure/cancellation/manual model selection settles once without loops, duplicate tools, replayed messages or concurrent requests. Candidates blocked by credentials, settings, cooldowns or non-transient failures are unchanged. |
...
| MX-T9 | Resource containment and instant conversation architecture plan | `docs/resource-and-loading-plan.md` records measured evidence, dependency-ordered implementation slices, acceptance gates, deployment implications and the Electron/Tauri decision boundary without adding external-agent control |
| MX-T10 | Bundled runtime package-manager verification | the bundled-style real `npm`/`npx` test is self-contained and passes without relying on an unrelated installed `/opt` tree; `pnpm verify` has no unexplained clean-base exception |

## M17 · The coding experience developers stay for

Goal: close the gaps `docs/ux-audit-coding-agents.md` measured against the field — the conversation owns the window, a child's question is answerable where it is shown, changes are reviewable and reversible, permissions can be set once, and every surface speaks to a person. Ordered by what a developer feels most; each task stays inside D-140/D-147 and invariant 6b.
...
Goal: implement `docs/resource-and-loading-plan.md` in dependency order. Authoritative transcripts stay host/worker-owned, actions remain host-first, dormant state is bounded without ending live work, and a previously seen conversation paints from a bounded device cache before authoritative reconciliation. Decisions D-255 through D-270 apply.

Dependencies: M16-T16, M16-T21, M16-T22, M16-T29, M16-T30 and M16-T31 are done. Tasks below follow the RP dependency graph in `docs/resource-and-loading-plan.md`; RP numbers are preserved as task suffixes. D-257 splits RP-2's post-containment repeat baseline into M18-T15 so its safety refusal can unlock the fixes it measured without weakening the gate. D-258 adds RP-5b as M18-T16 after unchanged post-containment evidence proved the approved whole-view pin exemption could let one current message cross the fixed renderer ceiling; T16 blocks T15 and the final T8 renderer gate. D-260 adds the RP-4 route-lease correction as M18-T18 after real unloads exposed a host check-then-act race; T18 also blocks T15. D-261/D-262 settle the full-scale pressure thresholds and worker-generation contract; D-263 settles the remaining pressure-row semantics. D-264 gives the final capability-consumption and post-containment recovery/update obligations explicit owners instead of leaving them as prose.

| Task | Title | Acceptance |
| --- | --- | --- |
| M18-T1 | RP-1 process identity and correct accounting | Host-owned `(pid,startTime)` inventory, role/owner attribution, platform-correct private-memory metrics, demand-driven bounded history and hostile-argv redaction; Linux reconciles with `smaps_rollup`, Electron cross-checks `app.getAppMetrics()`, unavailable is never zero. |
| M18-T2 | RP-2 controlled reproduction and initial heap attribution | Credential-free scratch/browser scenarios cover 50 sessions, long paging/streaming/images, 10 children/200 commands, projects, hide/restore, slow consumers and detach/retire; bounded measurement distinguishes retained heap/native/shared/pre/post-GC peaks, names concrete owners, refuses B at a safety breach and records a sanitized finding that assigns the blocking lifetime work without weakening fixtures or ceilings. |
| M18-T3 | RP-3 Advanced resource diagnostics | Advanced desktop/phone surface shows role-separated current/peak private memory, CPU, elapsed, bounded history and cache/session/task counts; redacted export and owner-routed controls cannot bypass lifecycle APIs; complete browser matrix. |
| M18-T4 | RP-4 bounded worker session lifetime | Explicit attachment ownership plus safe LRU runtime unload and a global replay-byte budget; active work/questions/runs/tasks/queues pin runtimes, replay-floor eviction forces snapshot resync, reopen preserves canonical state. |
| M18-T5 | RP-5 bounded renderer session lifetime | Durable/light session state is separated from byte/count-bounded hydrated transcript/image/search/projection state; current, Beam, waiting and running sessions are pinned; drafts, focus, scroll, approvals and find paging survive rehydrate. |
| M18-T6 | RP-6 bounded task and transcript-delivery lifetime | Terminal commands release process handles/tails into bounded durable logs while recent `task_output` remains readable; delivery paths are connection/scope reference-counted and detach/reopen races lose no terminal update or question. |
| M18-T7 | RP-7 bounded provider logging and transport pressure | Cursor-based frame decoder is linear and bounded at 16 MiB; large captures are chunked/redacted/released visibly; host/client/relay/output queues have byte high-water marks without dropping protocol state, questions or terminal events. |
| M18-T8 | RP-8 memory-pressure policy and safety ceilings | Measured role thresholds apply the documented safe release order before conservative heap ceilings; pressure never cancels active work, refusals explain what was released, normal largest fixtures stay below warning, runaway allocation is contained and worker loss is harness-attributed. |
| M18-T9 | RP-9 durable session revision contract | Opaque environment/session/branch-bound revision survives unchanged restart and invalidates on visible history/branch/metadata changes; `baseRevision` gates deltas, stale merges are refused, protocol round-trip/router inventory pass, revision is available without worker spawn. |
| M18-T10 | RP-10 bounded device tail cache | Atomic, corrupt-safe, per-session/global count/byte/age LRU stores only recent renderable tails and lightweight UI state; desktop uses OS-backed encryption where available, browser storage is described honestly, enterprise policy can disable it, relay stores none, environment/account isolation and clear controls are proven. |
| M18-T11 | RP-11 immediate paint and authoritative reconciliation | Navigation selects and paints a valid cached tail provisionally within one fixture frame, reconciles by revision/delta or atomic replacement, fences all mutations until authority, preserves draft/focus/reading intent, and leaves truthful readable retry state offline. |
| M18-T12 | RP-12 worker-free authoritative reads | One bounded snapshot/page shape routes to a live owning worker or a read-only host projection without spawning; live wins over disk, cursors/revisions/canonical identities agree, all environment deployments share the shape, and reads never mutate session files. |
| M18-T13 | RP-13 remote/cloud/enterprise policy | Environment descriptor declares capabilities, cache retention, RPC scopes, local-only operations and diagnostics; host-bound per-method authorization runs before worker creation, pairing only narrows grants, policy prevents caching, switches isolate state, audits name actor+method without content. |
| M18-T14 | RP-14 Electron/Tauri decision gate | Dropped by D-268: Electron stays; no shell comparison or migration decision. |
| M18-T15 | RP-2 post-containment repeat baseline | After T4–T8 and T16, two consecutive clean full runs of the unchanged RP-2 fixtures stay below the fixed ceilings with zero survivors; all nine scenarios complete, retained-owner rankings/slopes/peak ranges satisfy the predeclared category-aware repeatability gates, unsupported metrics remain explicit and `docs/resource-soak-baseline.md` records sanitized evidence. |
| M18-T16 | RP-5b bounded single-conversation hydration and rendering | Every hydrated view, including current/pinned/running, retains at most 1.5 MiB inside the 4 MiB renderer budget; live and settled oversized text/reasoning/tool/image bodies become explicit bounded excerpts backed by revision-fenced host/worker range reads, message ordinals/actions/drafts/focus/scroll/search survive, no active work is stopped, and unchanged full scenarios 4–5 remain below the renderer warning/ceiling. |
| M18-T17 | RP-4 impossible naming-pin correction | A first prompt waiting without an available Namer is never reported as active retirement-blocking work; an actual naming opportunity remains safe, all real work pins remain unchanged, credential-free prompted workers retire naturally, and T15 scenario 9 reaches zero guards/survivors without weakening its fixture. |
| M18-T18 | RP-4 path-routed mutation authority | A host route lease serializes safe runtime unload against ensure-open plus path-routed actions, rechecks authority after worker replacement, never resends a possibly delivered mutation, preserves every worker pin/fence and missing-transcript refusal, and unchanged scenario 5 completes its 200-command workload across repeated real unloads. |
| M18-T19 | Consume environment capabilities in client affordances | Client controls and diagnostics derive availability from the authenticated environment descriptor's scopes, `localOnly` methods and diagnostics capability; downgraded/remote environments hide or explain unsupported actions before invocation, with no duplicated policy table and desktop/phone tests. |
| M18-T20 | Catalog pages and cold workers stop paying for what nobody looks at | `pageCatalog` no longer exempts a `finished_unread` child row from the page quota (a root's own `finished_unread`, a question, an error and live runs still pin), so a refresh is a page and not the whole catalog; every worker spawn points `NODE_COMPILE_CACHE` at `<stateDir>/compile-cache/node-<version>` unless the launch environment chooses otherwise, so a cold worker's ~2,200-file import is served from V8's cache |

## M19 · Runtime recovery and update activation

Goal: implement the supporting recovery and update obligations in `docs/resource-and-loading-plan.md` after resource containment, without replacing the reviewed release orchestrator or rewriting the person's desired configuration after a failed launch.

Dependencies: M18 is done. These tasks strengthen runtime activation and migration recovery; they do not relax release verification.

| Task | Title | Acceptance |
| --- | --- | --- |
| M19-T1 | Launch identity, safe mode and bounded repair | Every spawned host/worker announces a per-launch identity before readiness; child/module failures carry structured ownership; failed launch leaves desired feature configuration intact; first-party safe mode starts the minimal feature set against the same data; repeated repair uses a bounded durable ledger and then asks the person. |
| M19-T2 | Immutable generation activation and correlated updates | Feature/runtime generations are staged immutably and atomically selected; update readiness requires migrations complete and long-running roots parked at an explicit activation gate; one update ID survives reconnect and determines the result, never reconnection alone. |
| M19-T3 | Migration snapshot and interrupted-restore recovery | State migrations define an integrity-checked snapshot/restore boundary and durable interrupted-restore marker; failure resumes or rolls back without mixing generations or losing the person's intact prior state. |
...
| M19-T6 | An installed update does not disable the running generation | The app and host bind worker launches to immutable bytes retained for the generation they started with, so a self-consistent fixed-root update can land without making unopened conversations fail. The running generation keeps normal session/new, load, prompt and child-agent behavior without mixed binaries; a full restart activates the installed generation through the existing verification/migration gates. Retention and cleanup are bounded, crash-safe and proven by a real fixed-root install→update cycle. |
| M19-T7 | Public global configuration lives in `~/.laser` | `configDir` is a first-class launch identity, defaulting to the account home’s `.laser` directory while overridden agent roots remain isolated. A single forward-only schema 1→2 migration moves only validated non-secret settings, keybindings, MCP definitions and global agent Markdown through the existing snapshot/restore engine; exact conflicts refuse without overwrite, legacy files remain intact and read-only, old binaries refuse schema 2, every runtime consumer uses the new root, external edits are conflict-safe, project `.laser` behavior is unchanged, and isolated focused/full/packaged acceptance proves no real home or installer hook is touched (D-338). |
...
is [`docs/source-control-leap.md`](docs/source-control-leap.md); this table is
the index, and the spec is the contract. It absorbs M17-T5, M17-T6, M17-T7,
M17-T8 and M17-T16, which stay in their milestone marked absorbed.
...
Specs, Research, Designs, Plans and Tasks are durable project entities that any
permitted session can mention without owning them.

The binding specification is
[`docs/project-lifecycle-leap.md`](docs/project-lifecycle-leap.md); this table is
the dependency-ordered implementation index, and the specification is the
product, domain, UX and security contract.
...
M21 implementation starts after the person accepts M20's sandbox.

### Dependency-ordered implementation

| Task | Title | Depends on | Acceptance |
| --- | --- | --- | --- |
| M21-T0 | Binding product contract and dependency plan | M20 person acceptance for implementation, not for planning | the binding specification and D-329–D-333 plus D-345 settle ownership, storage, nouns, gates, repository provenance, design engine and surfaces; ledger and one-screen status updated |
| M21-T1 | Protocol domain, revisions and transition rules | M21-T0 | closed schemas for every primary/supporting record, `ProjectWorkRef` with its person-facing `key` projection (D-355), `RepositoryLink` and its state/change targets, states, dependency edges, stale propagation and transition tests; no session owner field |
| M21-T2 | Stable project identity and canonical store | M21-T1 | transactional project-partitioned store with per-project monotonic key allocation (D-355), migrations, integrity, stable repository ids across relocation/worktrees, project removal retention, worktree parent resolution, blobs, bounds and crash tests |
| M21-T3 | Host authority, methods, policy and event stream | M21-T2 | complete method inventory implemented with optimistic concurrency, idempotency, project sequence/reconcile, method scopes/reach, worker-free reads and authorization tests |
| M21-T4 | Bounded bodies, search and derived projections | M21-T2, M21-T3 | ranged blob/text reads, value-only search, revision-fenced projections, derived release states, durable-write refusal at quota and resource-containment tests over large fixtures |
| M21-T5 | UI client store, reconcile and deep links | M21-T3, M21-T4 | project-scoped cache survives reconnect, rejects stale events, restores exact revision links and never aliases projects after relocation |
| M21-T6 | Embedded workspace shell | M21-T5 | the leap's "Embedded workspace" structure (D-355): Project work control with live counts, Work backlog with filters and saved views, Needs you queue, Recent, + Create, kind-aware detail/inspector frame, keys and type badges everywhere they appear, kind colour tokens in the theme set; `/spec`, `/research`, `/design`, `/plan` from any session with the project picker in projectless Chat and the command text as the whole input (D-352); preserved conversation, deep links and every empty/loading/error/stale/offline state at desktop and phone widths |
| M21-T7 | Specs and Research experience | M21-T6 | create/edit/revise/archive Specs and Research; brief/full-spec distinction; Research detail as question tree, findings and source panel per `docs/research-phase.md`; source/provenance UI, cross-links, search and Markdown reading/editing complete |
| M21-T8 | Comments, reviews and approval gates | M21-T3, M21-T6, M21-T7 | semantic anchors, threads, addressed/resolved roles, blocking rules, three digest-bound gates, invalidation and Approval Card interaction tests; Enter never approves |
| M21-T9 | Cross-session and cross-project mentions | M21-T3, M21-T4, M21-T5, M16-T86 | one mention adapter stores typed pinned refs, ranks current project, handles projectless/cross-project reads, routes mutations to the owner and reloads/searches without duplicating content |
| M21-T10 | Design Index: static facts, synthesis, review, storage | M21-T1, M21-T4 | per `docs/design-phase.md`: L0 parsers (manifests, CSS family, Tailwind/CSS-in-JS as text, docgen family, stories/docs as text, routes/templates), L1 synthesis on the Design-index profile, eras, DTCG tokens, review actions preserved across re-index, `.laser/design/index.json` + `review.json`, digest caches, bounded fleet Command; parse-only guarantee tested (no child processes, no network) |
| M21-T11 | DesignTree, canvas, primitive kit, prototypes and Sketch | M21-T10 | validated non-executable tree with declarative flow/state actions, stable node ids, DOM infinite canvas with frames and edges, Shadow DOM frames with index tokens as custom properties, primitive kit skinned by the index, fixtures, Prototype mode, sandboxed Sketch frame with `write_sketch`/`ground_sketch` and the Sketch→Tree ladder (D-354); round-trip/migration tests without React fields in protocol |
| M21-T12 | Static host grounding and source selection | M21-T10, M21-T11 | route/template → `HostPage` outline and file list, person-supplied reference image, `InsertionRegion` with structural path + text hash and orphaning, Conform/Island strategy recorded with reasons; no runner, no execution (D-353) |
| M21-T13 | Design workspace and anchored review | M21-T8, M21-T11, M21-T12 | Index/Foundation/Screens/Flows/Review sections, in-context canvas with host outline and strategy chip, inspector with index entry and review state, comments, before/after revisions, Mapped/Proposed labels with unreviewed chips, all applicable state matrices in both themes and widths |
...
| M21-T16 | Plans and Tasks workspace | M21-T6, M21-T15 | plan document/graph, Task detail, the Tasks-only Board with real drag transitions that refuse illegal moves by key (D-355), dependencies, blockers, attempts and evidence use adopted elements; no fabricated progress, schedule or fleet row |
| M21-T17 | Model tools and execution linking | M21-T9, M21-T15, M13, M26-T1 | compact tool surface over worker bridge conforming to `docs/agent-tool-contract.md`, `/design implement @Design` hand-off packet (D-354), context packet, feature gating, visible existing/new session choice, agent-run linking, wrong-project refusal and session deletion resilience |
...
| M21-T19 | Verification and convergence | M21-T13, M21-T18 | Spec/Design/Plan/Task verifier, Native evidence only from accepted M20 checkpoint previews linked `verified_at` (D-353), browser matrices, accessible evidence, deviation workflow, affected-only stale graph and final report; blocking failures cannot read Done |
...
| M21-T21 | Import, export and repository publication | M21-T3, M21-T7, M21-T10, M21-T15 | deterministic previewed adapters and manifests include repository links; optional project publication records exact `published_as` state, explicit conflict handling, provenance/licence retention and no second live authority |
| M21-T22 | Security, privacy, relay and resource hardening | M21-T3–M21-T21 | threat model, trust gates, sandbox/egress controls, method-policy coverage, hostile payloads, quotas, phone/relay capability matrix and no credentials/source markup leakage |
...
| M21-T26 | Research phase: adapters, tools and loop | M21-T1, M21-T3, M21-T4, M26-T1 | `docs/research-phase.md`: Research body schema and `SourceRef`; `web`, `project`, `repository`, `package`, `document` adapters with digest cache and budgets (`scholarly`, `tracker` may follow); `search_sources`, `read_source`, `record_finding`, `resolve_question` under the tool contract; host-enforced confidence and citation rules; `/research`, Research… on Spec/Design and palette entry; engine-instruction playbook; injection text returned as data; adapter disabled ⇒ tool absent; tests for every rule in the contract |
| M21-T25 | Release the project lifecycle leap | M21-T24, M21-T26 | staged identity/full/package gates, clean source CI, exact version/tag, user-authorized routine release, complete assets/digests/provenance/notes and Latest verification |
...
| M26-T5 | Publish tool contract milestone | M26-T2, M26-T3, M26-T4 | reviewed tool-only source, identity/full/package gates, exact-source CI, immutable tag, complete public assets/digests/provenance and Latest verification before downstream milestone releases (D-360) |
