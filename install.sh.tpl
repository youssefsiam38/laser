#!/bin/sh
# {{name}} installer — one command, no root, no Node, no package manager.
#
...
#
#   curl -fsSLo {{name}}-install.sh \
#     https://raw.githubusercontent.com/{{repository}}/v0.1.0/install.sh \
#     && sh {{name}}-install.sh --version v0.1.0
#
...
REPO_DEFAULT="{{repository}}"
PRODUCT="{{name}}"
BINARY="{{binary}}"
APP_ID="{{appId}}"
URL_SCHEME="{{urlScheme}}"
DESKTOP_NAME="{{desktopFileName}}"
...
# `{{binary}}` on PATH, and a menu entry that TryExec hides.
ROLLBACK_APP=""
...
scratch_dir() {
  if [ -z "$SCRATCH" ]; then
    SCRATCH=$(mktemp -d "${TMPDIR:-/tmp}/{{name}}-install.XXXXXX") ||
...
usage() {
  cat <<'EOF'
{{name}} installer — installs the {{displayName}} desktop app for the current user.
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --repo)
      shift
      [ $# -gt 0 ] || die "--repo needs an owner/name, for example --repo {{repository}}"
      REPO="$1"
      ;;
    --repo=*) REPO="${1#--repo=}" ;;
    --from)
...
#
# --signer-workflow is what makes this mean "built by {{name}}'s release
...
# it already carries the MimeType line that makes {{urlScheme}}:// links work, the
...
install_desktop_entry() {
  ...
  mkdir -p "$APPS_DIR"
  {
    ...
    printf '{{desktopInstalledByKey}}=install.sh\n'
  } >"$ide_out.new"
...
# Claim {{urlScheme}}:// links. Install only — on uninstall the entry is gone and the
...
install_appdir() {
  ...
  # {{name}}'s own launcher is the entry point, in every format. An AppImage
  ...
  #     that means {{name}} starts unsandboxed and says nothing;
  #   - it is `#!/usr/bin/env bash`, so a machine without bash cannot start the
  #     app at all;
  #   - it rewrites PATH, LD_LIBRARY_PATH and XDG_DATA_DIRS for {{name}} and for
...
so you can run this again once the package is gone. To remove it by hand:
  ...
  removed_data=0
  kept_data=0
  for data_dir in {{shellDataDirs}}; do
    [ -d "$data_dir" ] || continue
