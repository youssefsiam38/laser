...
describe("WorkerServer · mcp/*", () => {
  ...
  it("saves, lists, inspects, pings, calls, disconnects and removes one server", async () => {
    ...
    const listed = await call<{ servers: McpServerState[] }>("mcp/list", { cwd, view: "project" });
    ...
    const listedWhileInspecting = await call<{ servers: McpServerState[] }>("mcp/list", { cwd, view: "project" });
    ...
    const afterDisconnect = await call<{ servers: McpServerState[] }>("mcp/list", { cwd, view: "project" });
  ...
  it("inspects an unsaved definition without saving it", async () => {
    ...
    expect((await call<{ servers: McpServerState[] }>("mcp/list", { cwd, view: "project" })).result?.servers).toEqual([]);
  }, 60_000);
  ...
  it("takes its status from the newest session snapshot and ignores a shutdown's empty one", async () => {
    ...
    let listed = await call<{ servers: McpServerState[] }>("mcp/list", { cwd, view: "project" });
    ...
    listed = await call<{ servers: McpServerState[] }>("mcp/list", { cwd, view: "project" });
  ...
  it("detects importable configurations and refuses a conflict unless told to replace", async () => {
    ...
    const detected = await call<{ sources: Array<{ id: string; servers: Array<{ name: string; inlineSecrets: string[]; conflicts: string[] }> }> }>("mcp/import/detect", { cwd, scope: "project" });
    ...
    const applied = await call<{ imported: string[]; servers: McpServerState[] }>("mcp/import/apply", { cwd, source: "project-mcp-json", names: ["imported"], scope: "project" });
    ...
    const again = await call("mcp/import/apply", { cwd, source: "project-mcp-json", names: ["imported"], scope: "project" });
    expect(again.error?.message).toMatch(/already exists/);
    const replaced = await call<{ imported: string[] }>("mcp/import/apply", { cwd, source: "project-mcp-json", names: ["imported"], scope: "project", replace: true });
