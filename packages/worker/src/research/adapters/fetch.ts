 ...
 *   reading anything.
 * - **No private network.** A literal loopback, link-local or RFC1918 address
 *   is refused outright. This is a floor, not a full SSRF defence: name
 *   resolution belongs to the platform, and the worker never proxies a fetch
 *   for the model to an address it chose.
...
export function checkFetchTarget(value: string): URL {
  ...
  if (host === "localhost" || host.endsWith(".localhost") || host === "::1" || PRIVATE_V4.test(host) || /^f[cd][0-9a-f]{2}:/i.test(host) || /^fe80:/i.test(host)) {
    throw new ResearchRefused(
      "private_address",
      `${host} is on this machine or its private network, and research only reads sources a person could open from the public internet.`,
...
export function createResearchFetcher(impl: typeof fetch = fetch): ResearchFetcher {
  return async (request: ResearchFetchRequest, signal?: AbortSignal): Promise<ResearchFetchResult> => {
    const url = checkFetchTarget(request.url);
