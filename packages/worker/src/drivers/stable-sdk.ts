...
export class StableSdkDriver implements SessionDriver {
  ...
  async open(options: DriverOpenOptions): Promise<SessionState> {
    ...
    const createRuntime: CreateAgentSessionRuntimeFactory = async ({ cwd, sessionManager, sessionStartEvent }) => {
      ...
      // MCP servers (docs/mcp.md): the engine is loaded only for a project
  ...
  private commitPreparedFirstTurn(): void {
    ...
    try {
      // `open()` activated against the pristine runtime. The accepted
      // agent/model tuple may have replaced that runtime, so resolve from the
      // model that will actually receive this turn before the provider request
      // starts. Inside the try: acceptance must stay monotonic even if chain
      // resolution ever throws.
