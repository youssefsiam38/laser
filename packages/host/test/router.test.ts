...
import type { RuntimeActivationGate } from "../src/runtime-activation.js";
...
describe("Router · runtime activation", () => {
  it("routes all three methods only for a native caller and preserves exact update ids", async () => {
    ...
    const state = { updateId, generationId, phase: "parked" as const, blockers: { conversations: 0, agents: 0, questions: 0, approvals: 0, commands: 0, mutations: 0, workers: 0 } };
    const activation = {
      enter: vi.fn(() => () => undefined),
      prepare: vi.fn(async () => state),
      status: vi.fn(async () => state),
      cancel: vi.fn(async () => ({ ...state, phase: "cancelled" as const })),
    } as unknown as RuntimeActivationGate;
    const h = harness({ activation });
    try {
      for (const [method, params] of [
        ["pi/runtime/activation/prepare", { updateId, generationId }],
        ["pi/runtime/activation/status", { updateId }],
        ["pi/runtime/activation/cancel", { updateId }],
      ] as const) {
        await expect(h.router.handle({ jsonrpc: "2.0", id: 1, method, params }, LOCAL_ACCESS)).resolves.toHaveProperty("result.updateId", updateId);
      }
      expect(activation.prepare).toHaveBeenCalledExactlyOnceWith(updateId, generationId);
      expect(activation.status).toHaveBeenCalledExactlyOnceWith(updateId);
      expect(activation.cancel).toHaveBeenCalledExactlyOnceWith(updateId);
      await expect(h.router.handle({ jsonrpc: "2.0", id: 2, method: "pi/runtime/activation/prepare", params: { updateId, generationId } }, BROWSER_ACCESS))
        .resolves.toMatchObject({ error: { code: ErrorCodes.Unsupported } });
      expect(activation.prepare).toHaveBeenCalledTimes(1);
    } finally { h.cleanup(); }
  });
});
  ...
  it("refuses only a windowless live transcript before cache and routing", async () => {
    ...
    try {
      ...
      const anchored = await h.router.handle(
        { jsonrpc: "2.0", id: 3, method: "pi/session/entries", params: { path: PATH_A, window: { beforeEntry: "entry-40", limit: 40 } } },
        LOCAL_ACCESS,
      );
      expect(anchored).toHaveProperty("result");
      expect(h.workerRequests.at(-1)?.params).toEqual({ path: PATH_A, window: { beforeEntry: "entry-40", limit: 40 } });
