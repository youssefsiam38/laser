 ...
 */
export const MEMORY_PRESSURE_EVENT_ID = /^mp_\d{1,15}$/;
 ...
 * against. The value is a {@link ResourceMeasure}, so "we could not read it"
 * is a first-class answer with a reason, and never a zero.
 */
export interface MemoryPressureInput {
  kind: MemoryPressureInputKind;
  value: ResourceMeasure;
  warningBytes?: number;
...
export interface MemoryPressureActionResult {
  ...
  /** How much this step gave back. Absent when it gave nothing back. */
 ...
 */
export const memoryPressureMeasureSchema: z.ZodType<ResourceMeasure> = z.union([
...
export const memoryPressureEventSchema = memoryPressureActionResultSchema.extend({
...
export const memoryPressureSummarySchema = z
  .object({
    level: memoryPressureLevelStateSchema,
    roles: z.array(memoryPressureRoleStateSchema).max(MEMORY_PRESSURE_ROLES_MAX),
    refusing: z.array(memoryPressureRefusalSchema).max(MEMORY_PRESSURE_REFUSALS_MAX),
    totals: z
      .object({
        events: count,
