...
import { explorerListingSchema, type ExplorerEntry } from "@lasercode/protocol";
...
export function useDirectoryPage(cwd: string | undefined, query: string, active = true): DirectoryPageState {
  ...
  const [response, setResponse] = useState<{ key: string; offset: number; attempt: number; result?: ReturnType<typeof explorerListingSchema.parse>; failed?: boolean }>();
  useEffect(() => {
    if (!cwd || !active || directory === undefined || prefix === undefined) { setResponse(undefined); return; }
    let cancelled = false;
    const timer = setTimeout(() => {
      void client.request("pi/project/browse", { path: directory, explorer: { mode: "explorer", cwd, prefix, offset, limit: PAGE_SIZE } }).then(
        (reply) => {
          if (cancelled) return;
          const parsed = explorerListingSchema.safeParse(reply);
