...
function EntriesRefresh() {
  ...
  useEffect(() => {
    ...
    if (path && old.path === path && old.running && !running) void actions.refreshEntries();
  }, [actions, path, running]);
  return null;
}

/**
 * Upward reading fetches the next complete turn page, for as long as there is
 * one: a person scrolling up is never stopped short of the conversation's
 * start. Nothing here asks for the whole conversation, and nothing here loads
 * "other versions" — those belong to each message's own version picker, which
 * reads a bounded window of that message's siblings (D-340).
 */
export function HistoryControls() {
  ...
  const [loading, setLoading] = useState<"earlier" | null>(null);
  const [rereading, setRereading] = useState(false);
  const [announcement, setAnnouncement] = useState("");
  const deferred = useLaserState(s => Boolean(s.current && s.open[s.current]?.trimmed));
  const load = useCallback(async (continuous = false) => {
    ...
    if (busy.current || refusal || (!deferred && !history?.before)) return;
    busy.current = true;
    ...
    pending.current = { focused: root.current?.contains(document.activeElement) ? document.activeElement : null, page: true };
    let loaded = false;
    try {
      const page = await actions.loadEarlierEntries();
      loaded = page.accepted;
      if (continuous) spend(page);
      if (loaded) setAnnouncement("Earlier messages loaded.");
  ...
  }, [actions, controller, deferred, history?.before, refusal, spend]);
  useLayoutEffect(() => {
    ...
    const frame = requestAnimationFrame(() => { if (!budget.current.exhausted && controller.needsPrefetch()) void load(true); });
  ...
  useEffect(() => {
    ...
    const prefetch = () => { rearm(); if (controller.needsPrefetch()) void load(true); };
    const scroll = () => {
      const top = viewport.scrollTop;
      const upwards = top < lastTop;
      lastTop = top;
      rearmIfMoved();
      if (upwards && (controller.needsPrefetch() || (deferred && top < viewport.clientHeight / 2))) void load(true);
    };
    ...
    const wheel = (event: WheelEvent) => { rearm(); if (event.deltaY < 0 && (controller.needsPrefetch() || viewport.scrollTop <= 0)) void load(true); };
    let touchY: number | undefined;
    const touchstart = (event: TouchEvent) => { touchY = event.touches?.[0]?.clientY; };
    const touchmove = (event: TouchEvent) => {
      const y = event.touches?.[0]?.clientY;
      rearm();
      if (y !== undefined && touchY !== undefined && y > touchY + 8 && (controller.needsPrefetch() || viewport.scrollTop <= 0)) void load(true);
      touchY = y;
    };
    const keydown = (event: KeyboardEvent) => {
      rearm();
      if ((event.key === "ArrowUp" || event.key === "PageUp" || event.key === "Home") && (controller.needsPrefetch() || viewport.scrollTop <= 0)) void load(true);
    };
  ...
  if (!deferred && !history?.before && !requestedHistory.current && !refusal) return null;
  return <div ref={root} className="flex flex-wrap items-center justify-center gap-2 py-2 text-sm text-ink-2" aria-busy={loading !== null || rereading}>
    ...
    {/* Earlier messages arrive by scrolling up (and through the button above,
        which is the same thing for a keyboard). Other versions of a prompt are
        the prompt's own affair: its version picker reads them. */}
