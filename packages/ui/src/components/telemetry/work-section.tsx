...
export function WorkSection({
  ...
  const ranked = work ? [...work.tools.ranked, ...(work.tools.other > 0 ? [{ name: "other", count: work.tools.other }] : [])] : [];
...
function ToolRankRow({ row, max }: { row: TelemetryToolRank; max: number }) {
  ...
  return (
    <li className="flex min-w-0 items-center gap-2">
      ...
      <span className="w-8 shrink-0 text-end font-mono text-xs text-ink-3 tnum">{count(row.count)}</span>
