...
import { hasApiCost, noApiCostText, partialApiSubtotalText, spendHeader } from "./format.js";
...
export function SpendSection({
  ...
  const partial = partialApiSubtotalText(spend);
    ...
    <TelemetrySection id="spend" title="Spend" icon={icon} number={spendHeader(spend)} open={open} onOpenChange={onOpenChange}>
      {api ? (
        <>
          ...
          />
          {partial ? <FigureNote slot="telemetry-partial-spend">{partial}</FigureNote> : null}
        </>
      ) : (
