...
export function ConversationSearch({ query, hits, activeIndex, onQueryChange, onStep, onClose, inputRef, className, ...props }: Omit<ComponentProps<"div">, "children"> & {
  ...
  return <div data-slot="conversation-search" role="search" aria-label="Find in conversation" className={cn("z-20 shrink-0 px-3 py-2", className)} onKeyDown={e => { if (e.key === "Escape") { e.preventDefault(); e.stopPropagation(); onClose(); } }} {...props}>
    <div className={cn(paper, "mx-auto flex max-w-(--measure-thread) flex-col overflow-hidden rounded-xl")}>
      <div className="flex min-w-0 items-center gap-1 px-2 py-1">
        ...
        <input ref={inputRef} value={query} onChange={e => onQueryChange(e.target.value)}
          aria-label="Find in conversation" placeholder="Find in conversation" maxLength={200}
