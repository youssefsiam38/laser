...
const selectClass = "h-9 rounded-md border border-line bg-surface px-2 text-sm text-ink outline-none focus-visible:ring-2 focus-visible:ring-live";
...
function LineList({ label, rows, placeholder, maxLength = LINE_MAX, onChange }: { label: string; rows: DraftRow[]; placeholder: string; maxLength?: number | undefined; onChange(rows: DraftRow[]): void }) {
  return (
    <div className="flex flex-col gap-2">
      {rows.map((row, index) => (
        <div key={row.id} className="flex items-center gap-2">
          <Input aria-label={`${label} ${index + 1}`} value={row.text} maxLength={maxLength} placeholder={placeholder} onChange={(event) => onChange(rows.map((item) => item.id === row.id ? { ...item, text: event.target.value } : item))} />
  ...
  return (
    <div className="grid gap-3">
      <Section title="Shape the decision" detail="Name the problem, intended outcomes, and what is deliberately outside the work.">
        ...
        <MarkdownAuthoringField label="Problem" editorKey="spec:problem" value={draft.problem} maxLength={PROJECT_WORK_TEXT_MAX} placeholder="What is happening now, and why is it worth changing?" onChange={(problem) => onChange({ ...draft, problem })} onCreateShortcut={onCreateShortcut} />
      ...
      <Section title="Requirements and acceptance" detail="Make the contract concrete now, or leave it as a purposeful brief.">
        {draft.requirements.map((row, index) => (
          <div key={row.id} className="rounded-md border border-line bg-surface-2 p-2">
            ...
            <MarkdownAuthoringField label={`Requirement ${index + 1}`} editorKey={`spec:requirement:${row.id}`} value={row.text} maxLength={PROJECT_WORK_TEXT_MAX} onChange={(text) => onChange({ ...draft, requirements: draft.requirements.map((item) => item.id === row.id ? { ...item, text } : item) })} onCreateShortcut={onCreateShortcut} />
          </div>
        ))}
        ...
        {draft.acceptance.map((row, index) => (
          <div key={row.id} className="rounded-md border border-line bg-surface-2 p-2">
            ...
            <MarkdownAuthoringField label={`Acceptance criterion ${index + 1}`} editorKey={`spec:acceptance:${row.id}`} value={row.text} maxLength={PROJECT_WORK_TEXT_MAX} onChange={(text) => onChange({ ...draft, acceptance: draft.acceptance.map((item) => item.id === row.id ? { ...item, text } : item) })} onCreateShortcut={onCreateShortcut} />
          </div>
        ))}
      ...
      <Section title="Long-form document" detail="Optional supporting Markdown; the structured contract above remains authoritative.">
        <MarkdownAuthoringField label="Document" editorKey="spec:document" value={draft.document} maxLength={PROJECT_WORK_MARKDOWN_MAX} onChange={(document) => onChange({ ...draft, document })} onCreateShortcut={onCreateShortcut} />
      </Section>
    </div>
  );
}
  ...
  return (
    <div className="grid gap-3 md:grid-cols-2">
      ...
      <Section title="References and intent" detail="Record the direction in your own words; this does not claim repository grounding.">
        <MarkdownAuthoringField label="Direction notes" editorKey="design:notes" value={draft.notes} maxLength={PROJECT_WORK_TEXT_MAX} placeholder="References, constraints, or qualities to preserve" onChange={(notes) => onChange({ ...draft, notes })} onCreateShortcut={onCreateShortcut} />
      </Section>
    </div>
  );
}
