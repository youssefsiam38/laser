/**
 * ANSI interpreter for `stream` panels with `encoding: "ansi"`. SGR
 * (colour, bold, dim, italic, underline, inverse) is interpreted into spans;
 * every other escape — cursor movement, OSC titles and hyperlinks, private
 * modes — is stripped. Nothing is ever passed through (docs/ux-panels.md,
 * "ANSI is the sharp one"). Pure.
 *
 * Tested in test/panels/ansi.test.ts.
 */
