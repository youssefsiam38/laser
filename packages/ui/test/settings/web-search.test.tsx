...
it("requires an explicit sharing action and can revoke without model logout", async () => {
  ...
  await click("Allow for web search");
  expect(mocks.request).toHaveBeenCalledWith("web-search/configure", { cwd: "/project", change: { action: "configure", provider: "openai", connection: { source: "shared", sharedProvider: "openai" } } });
...
it("changes availability without changing any saved connection", async () => {
  ...
  expect(mocks.request).toHaveBeenCalledWith("feature/set", { id: "web-search", scope: "global", enabled: true });
  expect(mocks.request.mock.calls.some(([method]) => method === "web-search/configure")).toBe(false);
});
