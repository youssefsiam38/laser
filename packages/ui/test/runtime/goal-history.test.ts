...
describe("durable goal presentation", () => {
  ...
  it("keeps rejected completion tools and ordinary look-alike messages visible", () => {
    ...
    expect(projectMessages({ blocks: [{ kind: "user", id: "u", text: "Goal mode is active. Normal user text", images: 0 }], goals, running: false, dialogs: [] }).messages[0]?.content).toEqual([{ type: "text", text: "Goal mode is active. Normal user text" }]);
  });
