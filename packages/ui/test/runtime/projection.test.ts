...
type Part = { type: string; result?: unknown; artifact?: unknown; [key: string]: unknown };
...
describe("projectMessages — streaming order", () => {
  ...
  it("uses block ids as message ids so a streaming turn keeps identity", () => {
    const blocks: Block[] = [
      { kind: "user", files: [], id: "b1", text: "hi", images: [] },
  ...
  it("keeps image bytes out of the prose", () => {
    const { messages } = projectMessages({
      blocks: [{ kind: "user", files: [], id: "b1", text: "look", images: [{ type: "image", mimeType: "image/png", data: "cGlj" }] }],
      running: false,
      dialogs: [],
    });
  ...
  it("carries createdAt when a block was stamped", () => {
    ...
    const { messages } = projectMessages({
      blocks: [{ kind: "user", files: [], id: "b1", at, text: "hi", images: [] }],
      running: false,
      dialogs: [],
    });
...
describe("projectMessages — running status", () => {
  it("marks only the last streaming text part running, and only on the tail message", () => {
    const blocks: Block[] = [
      ...
      { kind: "user", files: [], id: "b2", text: "again", images: [] },
...
describe("shareProjectedMessages", () => {
  it("reuses the previous array when nothing changed", () => {
    const a = projectMessages({
      blocks: [{ kind: "user", files: [], id: "b1", text: "hi", images: [] }],
      running: false,
      dialogs: [],
    }).messages;
