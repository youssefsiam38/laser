...
/** The inspector body is a lazy chunk behind the dialog frame: render, then let Suspense resolve it. */
...
it("defers multi-megabyte JSON formatting until requested, then shares exact text across search and copy",async()=>{
  ...
  await mount(<ApiRequestDialog target={{kind:"log",entry:{...entry,detail:payload}}} onClose={()=>{}}/>);
...
it("loads exact message-linked requests and exposes tools, parameters, full JSON and close",async()=>{
  ...
  const close=vi.fn();
  await mount(<ApiRequestDialog target={{kind:"message",path:"/session",entryId:"user-1"}} onClose={close}/>);
...
it("shows source markers with keyboard/touch details without duplicating instruction search or changing Markdown",async()=>{
  ...
  await mount(<ApiRequestDialog target={{kind:"log",entry:captured}} onClose={()=>{}}/>);
...
it("follows an origin, opens inline contributions without a file action, and closes sources before find or the dialog", async () => {
  ...
  const close = vi.fn();
  await mount(<ApiRequestDialog target={{ kind: "log", entry: captured }} onClose={close}/>);
  await act(async () => { await new Promise(resolve => setTimeout(resolve, 30)); });
...
it("labels timestamp fallback and excludes captures linked to a different message",async()=>{
  ...
  await mount(<ApiRequestDialog target={{kind:"message",path:"/session",entryId:"old",at:entry.at,beforeAt:"2026-09-06T10:01:00.000Z"}} onClose={()=>{}}/>);
...
it("explains summary-only logging without inventing an API body",async()=>{
  ...
  await mount(<ApiRequestDialog target={{kind:"log",entry:summary}} onClose={()=>{}}/>);
...
async function search(value:string) {
  ...
  if(!document.querySelector('[role="search"] input')) await act(async()=>document.querySelector<HTMLButtonElement>('[aria-label="Search request"]')!.click());
  await act(async()=>{
...
it("highlights instructions in place, preserves focus, wraps matches and clears search before closing the modal",async()=>{
  const close=vi.fn();
  await mount(<ApiRequestDialog target={{kind:"log",entry:{...entry,detail:{instructions:"A needle here. Another needle there."}}}} onClose={close}/>);
  await act(async()=>document.querySelector<HTMLButtonElement>('[aria-label="Search request"]')!.click());
...
it("finds across Markdown spans and scopes keyboard shortcuts to the request inspector",async()=>{
  await mount(<ApiRequestDialog target={{kind:"log",entry:{...entry,detail:{instructions:"An Ap**ple** instruction",other:{Apple:"outside"}}}}} onClose={()=>{}}/>);
...
it("counts payload occurrences once across Instructions, Full request and Full JSON views",async()=>{
  ...
  await mount(<ApiRequestDialog target={{kind:"log",entry:{...entry,detail}}} onClose={()=>{}}/>);
