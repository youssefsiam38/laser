...
const shell = { layout: "desktop", sessionsOpen: true, fleetOpen: false, telemetryOpen: false, historyOpen: false, toolsOpen: false, addProjectOpen: false, canCreate: true,
  setSessionsOpen: vi.fn(), setFleetOpen: vi.fn(), setTelemetryOpen: vi.fn(), setHistoryOpen: vi.fn(), setToolsOpen: vi.fn(), setAddProjectOpen: vi.fn(), toggleSessions: vi.fn(), toggleFleet: vi.fn(), toggleTelemetry: vi.fn(), openHistory: vi.fn(), newSession: vi.fn(), showChat: vi.fn(), returnToChat: vi.fn() } satisfies ShellContextValue;
...
it("omits denied new, compact and fork palette commands while keeping read navigation", async () => {
  ...
  const store = createStateStore({ ...initialState, current: path, open: { [path]: opened }, sessions: [], environment: testDescriptor({ actor: { class: "paired_device", id: "phone" }, scopes: ["handshake", "read", "diagnostics"] }) });
  await act(async () => root.render(<LaserStoreProvider store={store}><TooltipProvider><WorkbenchProvider><ShellContext.Provider value={shell}><CommandPaletteDialog open onOpenChange={vi.fn()} /></ShellContext.Provider></WorkbenchProvider></TooltipProvider></LaserStoreProvider>));
