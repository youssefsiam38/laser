...
export default async function historyActions(check) {
  ...
  await reset();
  assert.equal((await check.rpc('pi/session/entries', { path: check.fixture.path })).entries.filter(e => e.type === 'message').length, 240);
  assert((await page.locator('[data-message-id]').count()) < 80);
  ...
  // On a phone, forking can trigger the supported install reminder. A real
  // focus action runs its existing dismissal handler; inputValue alone does not.
  await composer.click();
  assert.equal(await composer.inputValue(), prompt);
  assert.equal(activeMessages(await check.rpc('pi/session/entries', { path: wire.at(-1).path })).length, 10);
  assert((await page.locator('[data-message-id]').count()) <= 10);
  ...
  try {
    ...
    writeFileSync(join(check.root, 'version-failure.json'), JSON.stringify({ requests, wire, mounted: await page.locator('[data-message-id]').evaluateAll(nodes => nodes.map(node => ({ id: node.dataset.messageId, text: node.textContent }))) }, null, 2));
    throw error;
  }
  ...
  await page.waitForFunction(expected => document.querySelector('textarea[aria-label="Message"]')?.value === expected, prompt);
  assert.equal(await composer.inputValue(), prompt);
  assert.equal(activeMessages(await check.rpc('pi/session/entries', { path: check.fixture.path })).length, 10);
  assert((await page.locator('[data-message-id]').count()) <= 10);
  noAllSince(count);
  ...
  const beforeRetry = (await check.rpc('pi/session/entries', { path: check.fixture.path })).leafId;
  await reply.getByRole('button', { name: 'Try again', exact: true }).click();
  await until(async () => {
    const current = await check.rpc('pi/session/entries', { path: check.fixture.path });
    const active = activeMessages(current);
    return current.leafId !== beforeRetry && active.length === 12 && active.at(-1).message.role === 'assistant';
  }, 'retry to persist its own new six-turn branch', 120000);
...
function activeMessages({ entries, leafId }) {
  const byId = new Map(entries.map(entry => [entry.id, entry]));
  const active = [], seen = new Set();
  for (let id = leafId; id && !seen.has(id);) {
    seen.add(id); const entry = byId.get(id); if (!entry) break;
    if (entry.type === 'message') active.push(entry);
    id = entry.parentId;
  }
